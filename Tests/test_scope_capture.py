...
if table_name != '':
    ...
    logger = lab_core.logger(
        channel_master_or_group=master,
        database=database_name,
        use_threads=True)
    table_name = input(
        "What's the name of this scope shot (database table name):") if table_name is None else table_name
...
for channel in range(1, 5):
    if ch_enable_status[channel]:
        ...
        if smooth_factor[channel] is not None:
            data.smooth_y(
                window=smooth_factor[channel],
                extrapolation_window=None,
                iterations=poles[channel])

        GX.add_trace(data=data,
                     color=scope_colors[channel],
                     legend="{}: {}{}/DIV".format(
                         scope_legend[channel], yscale**-1, yunits)
                     )
        GX.add_ref_marker(
            ylocation=yoffset * yscale,
            marker_color=scope_colors[channel],
            use_axes_scale=True)
