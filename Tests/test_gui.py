...
m = lab_core.master()
ic = lab_core.integer_channel(
    name='format_tester',
    size=8,
    write_function=debug)
...
ic3 = lab_core.integer_channel(
    name='another_bit',
    size=1,
    read_function=lambda: 1 if random.random() < 0.0001 else 0)
...
print(
    m.get_all_channels_set(
        categories=['bits']) | m.get_all_channels_set(
            categories=['format']) -
    m.get_all_channels_set(
        categories=['preset']))
