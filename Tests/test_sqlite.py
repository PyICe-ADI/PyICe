...
csv = db.csv(
    output_file='die_temp.csv',
    elapsed_time_columns=True,
    append=False)

# Option 2 - use time_delta_query to construct SQL statement and have
# SQLite make the elapsed time calculation.
# output scaled from seconds to minutes
db.query("SELECT {}, * FROM temp_sensor".format(db.time_delta_query(time_div=60)))
csv2 = db.csv(output_file='die_temp2.csv')

# shouldn't change anything if all logged data is scalar. In this case,
# expands Hameg fuse linking list (len 3).
...
for ws_name, ws in list(sheets.items()):
    ...
    chart_setup = {'name': '{} vs {}'.format(y_axis, x_axis),
                   # x-axis using named range
                   'categories': '={}!{}'.format(ws_name, x_axis),
                   # y-axis using named range
                   'values': '={}!{}'.format(ws_name, y_axis),
    ...
    print(
        "Adding chart {name} to sheet {ws_name}.".format(
            ws_name=ws_name,
            **chart_setup))
