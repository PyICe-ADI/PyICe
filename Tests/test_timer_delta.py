...
class TestTimerDelta:
    def test_delta_does_not_accumulate_with_read_before_pause(self):
        ...
        # Each delta should be ~0.1s, NOT accumulating
        assert 0.05 < data1['delta_s'] < 0.2
        assert 0.05 < data2['delta_s'] < 0.2
        assert 0.05 < data3['delta_s'] < 0.2
        # Specifically, interval 3 should NOT be ~0.3s (accumulated)
        assert data3['delta_s'] < 0.25
    ...
    def test_delta_accumulates_without_read(self):
        ...
        # The delta is ~0.2s because no read advanced last_time between intervals
        assert data['delta_s'] > 0.15