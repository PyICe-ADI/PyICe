...
h = comparator(
    rising_threshold=3.6,
    falling_threshold=3.3,
    write_overshoot=0.1)
m = lab_core.master()
i_f = m.add_channel_virtual('input_force', write_function=h.write)
i_f.write(0)
i_s = m.add_channel_virtual_caching(
    'input_sense',
    read_function=lambda: i_f.read() /
    1000.0)
...
i_f_int = m.add_channel_virtual(
    'input_force_integer',
    write_function=h2.write,
    integer_size=10)
i_f_int.add_format(
    format_name='dummy',
