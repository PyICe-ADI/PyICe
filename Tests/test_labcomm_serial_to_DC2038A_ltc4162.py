...
# logging.basicConfig(level=logging.ERROR) # PyICe.lab_core creates the
# root logging object.
...
#    chip.populate_from_file(xml_file, access_list=['user_i2c_mode'], use_case=None)
    ...
    while not debug or 'q' not in input(
            "Press ENTER to launch PyICe GUI or 'q' then ENTER to exit. ").lower():
...
def test_labcomm():
    ...
    LTC_usbserialports = [
        port for port in comport_list if port.vid is not None and int(
            port.vid) == 0x1272 and int(
            port.pid) == 0x8005]
    ...
    if len(LTC_usbserialports) < 1:
        ...
        for port in LTC_usbserialports:
            print(
                " {:>12s} : vid:pid=0x{:04x}:0x{:04x}".format(
                    port.device, int(
                        port.pid), int(
                        port.vid)))
    ...
    the_xml_file = os.path.join(
        os.path.dirname(
            os.path.abspath(__file__)),
        "LTC4162-BASE-local-copy.xml")
