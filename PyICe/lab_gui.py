...
class channel_wrapper(object):
    ...
    def get_default_format(self):
        if self.get_formats():
            if self._channel.get_format() is not None:
                ...
                # Default to first user (non-generic) format if not otherwise
                # specified.
                return self.get_formats()[4]
            else:
                try:
    ...
    def unformat(self, data, format, use_presets):
        ...
        if hasattr(self._channel, 'unformat'):
            ...
            # convert string to number if underlying channel doesn't do
            # format/unformat
            try:
                data = int(data)
            except ValueError:
...
class display_item(QtWidgets.QLabel, channel_wrapper):
    ...
    def __init__(self, channel_object):
        ...
        self.highlight = QtCore.QPropertyAnimation(
            self, QtCore.QByteArray("alpha".encode("utf-8")))
        ...
        # safety yellow is more visible, so can be faded further
        self.highlight.setEndValue(40)
        ...
        if self.get_description():
            # put a paragraph in to make it rich text and get the wrapping
            ttip += "<p>{}</p>".format(escape(self.get_description()))
        if len(self.get_attributes()) != 0:
            ttip += '<p><table border="0.2">'
            ttip += '<tr><th>Attribute Name:</th><th>Attribute Value:</th></tr>'
            for attribute in self.get_attributes():
                ttip += '<tr><td>{}</td><td>{}</td></tr>'.format(
        ...
        if self.get_presets_dict():
            ...
            for preset, value in self.get_presets_dict().items():
                if self.get_preset_description(preset) is not None:
                    ...
                    preset_desc = ''
                ttip += '<tr><td>{}</td><td>{}</td>{}</tr>'.format(
                    preset, value, preset_desc)
            ttip += '</table></p>'
        ttip += '<p><table border="0.2"><tr><th>tag:</th><td>{}</td></tr></table></p>'.format(
            self.get_tag())
    ...
    def _set_alpha(self, value):
        ...
        if self._iir_setting == "Disabled":
            ...
            try:
                color = [255,
                         # Fade red to yellow.
                         min(int(round(255.0 * self._iir_rampto / self._iir_setting)), 255),
                         0
                         ]
            except TypeError:
    ...
    def get_formatted_data(self, raw_data=None):
        ...
        if self._use_presets_read and not isinstance(
                formatted_data, numbers.Number):
            units = ""
        ...
        debug_logging.debug(
            "GUI display item %s formatted raw data to %s.",
            self.get_name(),
            formatted_data)
    ...
    def update_from_dict(self, data_dict):
        if self.get_name() in list(data_dict.keys()):
            ...
            if hasattr(self._channel, "format") and not isinstance(
                    data_dict[self.get_name()], lab_core.ChannelReadException):
                # channel read data may be pre-formatted by underlying channel.
                # need to un-do before reformatting with GUI settings
                raw_data = self._channel.unformat(data_dict[self.get_name(
                )], format=self._channel.get_format(), use_presets=self._channel.using_presets_read())
            else:
                raw_data = data_dict[self.get_name()]
            debug_logging.debug(
                "GUI display item %s received data %s (%s raw).",
                self.get_name(),
                data,
                raw_data)
            try:
                ...
                if not numpymissing and isinstance(raw_data, numpy.ndarray):
                    self._data_changed = (
                        raw_data != self._prev_raw_data).any()
                else:
                    print(type(raw_data))
                    raise e
            ...
            try:
                if self._iir_setting == "Disabled":
                    ...
                    if hasattr(self._channel, "format") and self._channel.get_format(
                    ) is not None and self._channel.get_format()['signed']:
                        # WARNING: Can't just average signed number!
                        raw_signed = self._channel.twosComplementToSigned(
                            raw_data)
                        iir_signed = self._channel.twosComplementToSigned(
                            self._iir_data)
                        filt_signed = raw_signed + \
                            (1 - 1.0 / self._iir_rampto) * \
                            (iir_signed - raw_signed)
                        self._current_raw_data = self._channel.signedToTwosComplement(
                            filt_signed)
                    else:
                        self._current_raw_data = raw_data + \
                            (1 - 1.0 / self._iir_rampto) * \
                            (self._iir_data - raw_data)
    ...
    def contextMenuRequested(self, point):
        ...
        if len(self.get_presets()) and self._channel.is_writeable():
            ...
            for preset in self.get_presets():
                action_write_preset = QtWidgets.QAction(
                    preset, write_preset_menu)
                action_write_preset.connect(
                    action_write_preset,
                    SIGNAL("triggered()"),
                    lambda preset=preset: self.write_preset(preset))
        ...
        if len(self.get_write_history()) and self._channel.is_writeable():
            ...
            for item in reversed(self.get_write_history()):
                action_write_hist = QtWidgets.QAction(
                    str(item), write_recent_menu)
                action_write_hist.connect(action_write_hist, SIGNAL(
                    "triggered()"), lambda item=item: self.write([(self.get_name(), item)]))
        ...
        if self._channel.is_writeable():
            ...
            self.connect(action_write, SIGNAL("triggered()"),
                         self, SLOT("display_write_dialog()"))
        ...
        self.connect(
            action_print_history,
            SIGNAL("triggered()"),
            self.print_history)
        menu.addAction(action_print_history)
        # print tooltip menu item - some tooltips are so big, they don't fit on
        # the screen
        action_print_tooltip = QtWidgets.QAction("Print Info", menu)
        self.connect(
            action_print_tooltip,
            SIGNAL("triggered()"),
            self.print_tooltip)
        ...
        if len(self.get_formats()):
            ...
            action_set_format = QtWidgets.QAction(
                "None", format_menu, checkable=True)
            if self._format is None:
                action_set_format.setChecked(True)
            self.connect(
                action_set_format,
                SIGNAL("triggered()"),
                lambda: self.set_format(None))
            format_menu.addAction(action_set_format)
            for reg_format in self.get_formats():
                action_set_format = QtWidgets.QAction(
                    reg_format, format_menu, checkable=True)
                if reg_format == self._format:
                    action_set_format.setChecked(True)
                action_set_format.connect(
                    action_set_format,
                    SIGNAL("triggered()"),
                    lambda reg_format=reg_format: self.set_format(reg_format))
                format_menu.addAction(action_set_format)
            menu.addMenu(format_menu)
        # iir filter sub menu
        iir_menu = QtWidgets.QMenu("IIR Filter")
        for iir in ["Disabled", 2, 4, 8, 16, 32, 64, 128, 256, 512,
                    1024, 2048, 4096, 8192, 16384, 32768, 65536, "Average"]:
            action_set_iir = QtWidgets.QAction(
                str(iir), iir_menu, checkable=True)
            if iir == self._iir_setting:
                action_set_iir.setChecked(True)
            action_set_iir.connect(
                action_set_iir,
                SIGNAL("triggered()"),
                lambda iir=iir: self.set_iir(iir))
            iir_menu.addAction(action_set_iir)
        menu.addMenu(iir_menu)
        # use presets read menu item
        action_use_read_presets = QtWidgets.QAction(
            "Use Read Presets", menu, checkable=True)
        menu.addAction(action_use_read_presets)
        if self._use_presets_read:
            action_use_read_presets.setChecked(True)
        self.connect(
            action_use_read_presets,
            SIGNAL("toggled(bool)"),
            self,
            SLOT("setUseReadPresets(bool)"))
        # use presets write menu item
        if self._channel.is_writeable():
            action_use_write_presets = QtWidgets.QAction(
                "Use Write Presets", menu, checkable=True)
            menu.addAction(action_use_write_presets)
            if self._use_presets_write:
                action_use_write_presets.setChecked(True)
            self.connect(
                action_use_write_presets,
                SIGNAL("toggled(bool)"),
                self,
                SLOT("setUseWritePresets(bool)"))
        # flash on change menu item
        action_flash_on_change = QtWidgets.QAction(
            "Flash on Change", menu, checkable=True)
        menu.addAction(action_flash_on_change)
        if self._flash:
            action_flash_on_change.setChecked(True)
        self.connect(
            action_flash_on_change,
            SIGNAL("toggled(bool)"),
            self,
            SLOT("setFlash(bool)"))
        # print on change menu item
        action_print_on_change = QtWidgets.QAction(
            "Print on Change", menu, checkable=True)
        menu.addAction(action_print_on_change)
        if self._print:
            action_print_on_change.setChecked(True)
        self.connect(
            action_print_on_change,
            SIGNAL("toggled(bool)"),
            self,
            SLOT("setPrint(bool)"))
    ...
    def print_history(self):
        ...
        for idx, [timestamp, raw_data, read_count] in enumerate(self._history):
            ...
            if len(self._history) != self._history_length + 1:
                ...
                if idx == 0:
                    # Indicate measurment time is after signal change time by
                    # uncertain period.
                    prefix = "+"
            else:
                if idx == 0:
                    # Skip oldest reading with uncertain transisition time
                    # after dequeue fills.
                    continue
            t_rel = time_fmt.format(
                prefix="-", postfix="", **hms((request_time - timestamp).total_seconds()))

            try:
                t_duration = time_fmt.format(
                    prefix=prefix, postfix=" ", **hms((self._history[idx + 1][0] - timestamp).total_seconds()))
            except IndexError:
                # t_duration = " --:--:--.---"
                t_duration = time_fmt.format(
                    prefix=prefix, postfix="+", **hms((request_time - timestamp).total_seconds()))
            row_col_data.append(["{:02d}:".format(idx),
                                 t_rel,
                                 t_duration,
    ...
    def print_tooltip(self):
        ...
        for line in lines:
            ...
            fields = [clean_unicode(field) for field in fields]
            if len(fields) == 1:
                infotxt += textwrap.fill(fields[0],
                                         width=max_width,
                                         subsequent_indent='  ')
            else:
                if fields[-1] == '':
                ...
                for i, field in enumerate(fields):
                    if i < len(fields) - 1:
                        ...
                        # spacer = '.' * (col_widths[i] - len(field)) #Lower 7
                        # bits of Ascii might be safer. Period mark is bolder
                        # than Middle Dot.
                    else:
                        spacer = ''
                    if i == len(col_widths) - 1:
                        indent = ' ' * \
                            reduce(lambda x, y: x + y, col_widths[:-1])
                        field = textwrap.fill(
                            field,
                            width=max_width,
    ...
    def display_write_dialog(self):
        if not self._wcd:  # not yet created
            ...
            if self._wcd.isHidden():
                # window was created, then closed. Re-sync with display_item
                # settings
                self._wcd.update_format(self._format)
                self._wcd.set_value(
                    self.format(
    ...
    def _create_write_dialog(self):
        self._wcd = write_channel_dialog(
            self._channel,
            self._current_raw_data,
            self._format,
            self._use_presets_write)
    ...
    def write(self, data):
        ...
        if self._channel.is_readable():
            self.SI_request_read_channel_list.emit(
                [data_pair[0] for data_pair in data])
    ...
    def save(self, ds_parent):
        ...
        # Always save, even if None. Allows auto-format selection to be un-done
        # in config.
...
class write_channel_dialog(QtWidgets.QDialog, channel_wrapper):
    ...
    def init_interface(self):
        ...
        self.value_text_box = QtWidgets.QLineEdit(
            str(self.format(self._current_raw_data, self._format, self._use_presets_write)))
        ...
        if len(self.get_formats()):
            ...
            self.formats_combo.setCurrentIndex(
                self.formats_combo.findText(
                    "None" if self._format is None else self._format))
            ...
            self.link_format_checkbox = QtWidgets.QCheckBox(
                'Link Format', self)
    ...
    def increment(self):
        if self.get_value() == "None":
            ...
            # special handling for default formats. others should tolerate
            # float conversion.
            value = self.unformat(
                self.get_value(),
                self._format,
        ...
        if self._format in ('hex', 'bin'):
            ...
            try:
                result = self.format(
                    result, self._format, self._use_presets_write)
            except Exception as e:
                print(e)
                print(traceback.format_exc())
                return
    ...
    def update_use_presets_write(self, state):
        ...
        self._use_presets_write = state
        self.set_value(
            str(self.format(unformatted_data, self._format, self._use_presets_write)))
    ...
    def update_format(self, format):
        ...
        try:
            ...
            # non-None object and is therefore always truthy.  Use isChecked()
            # for bool semantics.
            if hasattr(
                    self, 'link_format_checkbox') and self.link_format_checkbox.isChecked():
                old_value = self.unformat(
                    self.get_value(), self._format, self._use_presets_write)
    ...
    def load(self, ds_parent):
        ...
        if dcwd_ds:
            ...
            if self._current_raw_data is None:
                ...
                # default to more recently read data than saved default if
                # available
...
class display_item_group(QtWidgets.QWidget):
    ...
    def __init__(self, channel_group_object):
        ...
        self.setPalette(self.palette)
        self._busy_alpha = 0
        self.busy_highlight = QtCore.QPropertyAnimation(
            self, QtCore.QByteArray("busy_alpha".encode('utf-8')))
    ...
    def populate_from_channel_group(self, channel_group_object):
        ...
        for channel in channel_group_object:
            ...
            for tag in channel.get_tags():
                ...
                di.SI_request_write_channel_list.connect(
                    self.write_channel_list)
    ...
    def build_interface(self):
        ...
        for di in self.display_items:
            if di.displayed():
                if column_height > 0 and column_height + di.sizeHint().height() + \
                        self._grid.verticalSpacing() > self.parentWidget().size().height() - 60:
                    # self.frameGeometry().height()
                    # 60 offset may have something to do with
                    # grid.contentsMargins() and layout placement, possibly
                    # another grid.verticalSpacing() too. Used to prevent frame
                    # from oversizing and creating vertical scroll
                    y_pos = 0
                    x_pos += 1
                    self.columnWidths.append(0)
                    column_height = 0
        ...
        if di.displayed():
            if di.sizeHint().width() > self.columnWidths[di.column_placement]:
                self._grid.setColumnMinimumWidth(
                    di.column_placement, di.sizeHint().width())
    ...
    def contextMenuRequested(self, point):
        ...
        self.connect(
            tab_flash_on_change,
            SIGNAL('triggered()'),
            lambda: self.flash_on_change(True))
        menu.addAction(tab_flash_on_change)
        tab_no_flash_on_change = QtWidgets.QAction(
            "Don't Flash on Change", menu)
        self.connect(
            tab_no_flash_on_change,
            SIGNAL('triggered()'),
            lambda: self.flash_on_change(False))
        ...
        menu.addSeparator()
        self.tagged_sort = QtWidgets.QAction(
            "Categorized Sort", menu, checkable=True)
        self.tagged_sort.setChecked(self._tagged_sort)
        self.connect(
            self.tagged_sort,
            SIGNAL('toggled(bool)'),
            self.set_tagged_sort)
        ...
        # dead code that silently accumulated a duplicate slot connection on
        # every right-click.
    ...
    def wheelEvent(self, QWheelEvent):
        ...
        if modifiers == QtCore.Qt.ControlModifier:
            ...
            try:
                ...
                # qt6: angleDelta() returns QPoint; .y() extracts vertical
                # scroll component
                delta = QWheelEvent.angleDelta().y()
            # independent of mouse resolution, but doesn't support
            # super-mega-rapid-zoom
            self.change_font_size(1 if delta > 0 else -1)
        else:
            # allow signal to bubble up to scroll function (hold meta/alt with
            # wheel)
            QWheelEvent.ignore()
    ...
    def load(self, ds_parent):
        ...
        for di_ds in dig_ds:
            ...
            for di in self.display_items:
                if di.get_name() == di_name and di.get_tag(
                ) == di_ds.get_child('display_item')['tag']:
                    di.load(di_ds)
...
class display_tag_group(QtWidgets.QWidget):
    ...
    def init_interface(self):
        ...
        # add the display_item_group at the top
        layout.addWidget(self.scroll_area, 0)
        x_pos = 0
        y_pos = 0
        for tag_name in self.get_tags():
            new_tag_check_box = QtWidgets.QCheckBox(
                str(tag_name), tag_sel_group_widget)
            ...
            self.connect(
                ...
                self._on_change)
            # new_tag_check_box.checkStateChanged.connect(self._on_change)  #
            # qt6
            y_pos += 1
        ...
        try:
            ...
            for tag in tag_ds:
                ...
                if tag_name in list(self._check_boxes.keys()):
                    self._check_boxes[tag_name].setCheckState(
                        QtCore.Qt.Checked)
        except BaseException:
            print(
                "Warning: Loading a older saved configuration. Category/tag settings not loaded.")
    ...
    def select_all(self, select):
        ...
        for tag in list(self._check_boxes.values()):
            tag.setCheckState(
                QtCore.Qt.Checked if select else QtCore.Qt.Unchecked)
        self.suppress_update = False
        self.SI_stateChanged.emit()
        # self.emit(QtCore.SIGNAL('stateChanged()'))
...
class tab_view(QtWidgets.QWidget):
    ...
    def __init__(self, channel_group_object, parent):
        ...
        self.SI_passive_observer_data.connect(
            self.dig.receive_passive_channel_data)
    ...
    def init_interface(self):
        ...
        self.scroll_area.setWidgetResizable(True)
        # add the display_item_group at the top
        layout.addWidget(self.scroll_area, 100)
        # add the display_tag_group at the bottom
        layout.addWidget(self.dcg, 0)
        self.setLayout(layout)
        self.show()
    ...
    def wheelEvent(self, QWheelEvent):
        ...
        if modifiers == QtCore.Qt.NoModifier:
            # remap mouse wheel vertical scroll to horizontal scroll, since the
            # only scroll bar is horizontal.
            scroll_bar = self.scroll_area.horizontalScrollBar()
            try:
                delta = QWheelEvent.delta()  # qt5: returns int (degrees/8)
                scroll_bar.triggerAction(
                    scroll_bar.SliderSingleStepAdd if delta > 0 else scroll_bar.SliderSingleStepSub)
            except AttributeError:
                # qt6: angleDelta() returns QPoint; .y() is vertical component
                delta = QWheelEvent.angleDelta().y()
                # SliderSingleStepAdd/Sub must be accessed via class, not
                # scroll_bar instance, in Qt6
                scroll_bar.triggerAction(QtWidgets.QAbstractSlider.SliderSingleStepAdd if delta >
                                         0 else QtWidgets.QAbstractSlider.SliderSingleStepSub)
        else:
            QWheelEvent.ignore()
...
class tab_group(QtWidgets.QTabWidget):
    ...
    def connect_tv(self, tv_widget):
        ...
        tv_widget.SI_request_write_channel_list.connect(
            self.write_channel_list)
        self.SI_passive_observer_data.connect(
            tv_widget.receive_passive_channel_data)
    ...
    def disconnect_tab(self, tab_view_widget):
        if tab_view_widget is not None:
            ...
            if new_tab_index is None:
                dbgmsg = "tab_group.disconnect_tab id={}".format(
                    id(tab_view_widget))
            else:
                dbgmsg = "tab_group.disconnect_tab #{}".format(new_tab_index)
            debug_logging.debug(dbgmsg)
            self.SI_channel_data_ready.disconnect(
                tab_view_widget.receive_channel_data)
            self.SI_tab_use_write_presets.disconnect(
                tab_view_widget.tab_use_write_presets)
            self.SI_tab_use_read_presets.disconnect(
                tab_view_widget.tab_use_read_presets)
            self.SI_tab_flash_on_change.disconnect(
                tab_view_widget.tab_flash_on_change)
            self.SI_tab_change_font_size.disconnect(
                tab_view_widget.tab_change_font_size)
            self.SI_resize_main_window.disconnect(
                tab_view_widget.resize_main_window)

    def connect_tab(self, tab_view_widget):
        if tab_view_widget is not None:
            new_tab_index = self.find_widget_in_tabs(tab_view_widget)
            if new_tab_index is None:
                dbgmsg = "tab_group.connect_tab id={}".format(
                    id(tab_view_widget))
            else:
                dbgmsg = "tab_group.connect_tab #{}".format(new_tab_index)
            debug_logging.debug(dbgmsg)
            self.SI_channel_data_ready.connect(
                tab_view_widget.receive_channel_data)
            self.SI_tab_use_write_presets.connect(
                tab_view_widget.tab_use_write_presets)
            self.SI_tab_use_read_presets.connect(
                tab_view_widget.tab_use_read_presets)
            self.SI_tab_flash_on_change.connect(
                tab_view_widget.tab_flash_on_change)
            self.SI_tab_change_font_size.connect(
                tab_view_widget.tab_change_font_size)
            self.SI_resize_main_window.connect(
                tab_view_widget.resize_main_window)
            ...
            class TabSwitchingAgent(object):
                ...
                def try_to_disconnect_old_tab(self):
                    ...
                    try:
                        ...
                        if callable(
                                self.the_tab_group.background_call) and self.tries_so_far < self.retries:
                            ...
                            # chance that our next attempt to disconnect will
                            # succeed.
                            self.the_tab_group.background_call(
                                self.try_to_disconnect_old_tab)
                    else:
                        if self.tries_so_far > 1:
                            debug_logging.info(
                                "  ...retrying successfully disconnected old tab.")
                        else:
                            debug_logging.debug(
                                "  Disconnected old tab successfully")
                        self.the_tab_group.background_call(
...
class gui_logger(QtCore.QObject):
    ...
    def log(self):
        if self._logger:
            ...
            # cannot directly use the logger here, send it to the back ground
            # worker to actually read
    ...
    def _disp_save_dialog(self):
        fname, _ = QtWidgets.QFileDialog.getSaveFileName(
            None, 'Connect Database', '.', "SQLite Files (*.sqlite);;All files (*.*)", options=QtWidgets.QFileDialog.DontConfirmOverwrite)
        if fname:
    ...
    def _append_table(self):
        self._logger = lab_core.logger(
            self._channel_group,
            database=self._get_dbase_filename())
        self._logger.append_table(self._get_table_name())
        self._logger.set_journal_mode(
            journal_mode='WAL',
            synchronous='NORMAL',
            timeout_ms=10000)
        self.connect_dialog.close()
...
class logger_view(QtWidgets.QWidget):
    ...
    def init_interface(self):
        ...
        # add the display_item_group at the top
        layout.addWidget(scroll_area, 100)
        # add the display_tag_group at the bottom
        layout.addWidget(self.dcg, 0)
        self.setLayout(layout)
        self.resize(800, 600)
...
class background_worker(QtCore.QThread):
    ...
    def run(self):
        try:
            while self.running:
                try:
                    # write is first so if a read follows a write the new value
                    # will be set
                    ...
                    if data_type == 'write':
                        ...
                        debug_logging.warning(
                            'background worker received command it does not recognize: ({}, {})'.format(
                                repr(data_type), repr(data)))
                except Exception as e:
                    debug_logging.warning(
                        "**** lab_gui.background_worker.run() caught exception {} but keeps running".format(e))
                    debug_logging.warning(traceback.format_exc())
        except Exception as e:
            debug_logging.warning(
                "**** lab_gui.background_worker.run() caught exception {} and exits".format(e))
            debug_logging.warning(traceback.format_exc())
        finally:
...
class ltc_lab_gui_main_window(QtWidgets.QMainWindow):
    ...
    def __init__(self, channel_group, background_call=None):
        ...
        self.overflow_highlight = QtCore.QPropertyAnimation(
            self, QtCore.QByteArray("overflow_alpha".encode("utf-8")))
    ...
    def _set_overflow_alpha(self, value):
        ...
        self.palette.setColor(self.backgroundRole(), color)
        self.setPalette(self.palette)
    overflow_alpha = QtCore.Property(
        int, _get_overflow_alpha, _set_overflow_alpha)
    # def _get_busy_alpha(self):
    #     return self._busy_alpha
    # def _set_busy_alpha(self, value):
    #     self._busy_alpha = value
    ...
    def create_menu_bar(self):
        ...
        file_save_default.setShortcut(
            QtGui.QKeySequence(
                QtCore.Qt.CTRL | QtCore.Qt.Key_S))
        file_save_default.triggered.connect(
            lambda: self.save_file('default.guicfg'))
        ...
        file_menu.addSeparator()
        self.file_passive = QtWidgets.QAction(
            "Passive Observer", file_menu, checkable=True)
        file_menu.addAction(self.file_passive)
        self.file_passive.toggled.connect(
            self.set_passive_observer_mode)  # Bool
        self.file_close = QtWidgets.QAction("Close", file_menu)
        self.file_close.setShortcut(
            QtGui.QKeySequence(
        ...
        tab_use_write_presets = QtWidgets.QAction(
            "Use Write Presets", tab_menu)
        tab_use_write_presets.triggered.connect(
            lambda: self.SI_tab_use_write_presets.emit(True))
        tab_menu.addAction(tab_use_write_presets)
        tab_no_write_presets = QtWidgets.QAction(
            "Don't Use Write Presets", tab_menu)
        tab_no_write_presets.triggered.connect(
            lambda: self.SI_tab_use_write_presets.emit(False))
        ...
        tab_use_read_presets.triggered.connect(
            lambda: self.SI_tab_user_read_presets.emit(True))
        tab_menu.addAction(tab_use_read_presets)
        tab_no_read_presets = QtWidgets.QAction(
            "Don't Use Read Presets", tab_menu)
        tab_no_read_presets.triggered.connect(
            lambda: self.SI_tab_user_read_presets.emit(False))
        tab_menu.addAction(tab_no_read_presets)
        tab_menu.addSeparator()
        tab_flash_on_change = QtWidgets.QAction("Flash on Change", tab_menu)
        tab_flash_on_change.triggered.connect(
            lambda: self.SI_tab_flash_on_change.emit(True))
        tab_menu.addAction(tab_flash_on_change)
        tab_no_flash_on_change = QtWidgets.QAction(
            "Don't Flash on Change", tab_menu)
        tab_no_flash_on_change.triggered.connect(
            lambda: self.SI_tab_flash_on_change.emit(False))
        tab_menu.addAction(tab_no_flash_on_change)
        tab_menu.addSeparator()
        increase_font_size = QtWidgets.QAction("Increase Font Size", tab_menu)
        increase_font_size.setShortcut(
            QtGui.QKeySequence(
                QtCore.Qt.CTRL | QtCore.Qt.Key_Plus))
        increase_font_size.triggered.connect(
            lambda: self.SI_change_font_size.emit(1))
        tab_menu.addAction(increase_font_size)
        decrease_font_size = QtWidgets.QAction("Decrease Font Size", tab_menu)
        decrease_font_size.setShortcut(
            QtGui.QKeySequence(
                QtCore.Qt.CTRL | QtCore.Qt.Key_Minus))
        decrease_font_size.triggered.connect(
            lambda: self.SI_change_font_size.emit(-1))
        tab_menu.addAction(decrease_font_size)
        tab_menu.addSeparator()
        read_all = QtWidgets.QAction("Read All", tab_menu)
        read_all.setShortcut(
            QtGui.QKeySequence(
                QtCore.Qt.CTRL | QtCore.Qt.Key_R))
        # Wrap in lambda so _active_tab is evaluated at trigger time, not at menu-bar creation time.
        # Without the lambda, the action permanently targets whichever tab was
        # active during __init__.
        read_all.triggered.connect(
            lambda: self._tg._active_tab.dig.request_read_all())
        ...
        logger_disconnect = QtWidgets.QAction(
            "Disconnect Database", logger_menu)
        ...
        logger_select_channels = QtWidgets.QAction(
            "Select Channels...", logger_menu)
        logger_select_channels.triggered.connect(
            self._gui_logger.display_select_channels)
        logger_menu.addAction(logger_select_channels)
        logger_log = QtWidgets.QAction("Log Once", logger_menu)
        logger_log.setShortcut(
            QtGui.QKeySequence(
                QtCore.Qt.CTRL | QtCore.Qt.Key_L))
        logger_log.triggered.connect(self._gui_logger.log)
        logger_menu.addAction(logger_log)
        logger_menu.addSeparator()
        self.logger_background_log = QtWidgets.QAction(
            "Enable Background Logging", logger_menu, checkable=True)
        self.logger_background_log.toggled.connect(
            self.enable_background_logging)
        logger_menu.addAction(self.logger_background_log)
        self.logger_change = QtWidgets.QAction(
            "Enable Logging on Change", logger_menu, checkable=True)
    ...
    def enable_change_logging(self, enable_log):
        if not self._gui_logger._logger:
            ...
            self.SI_channel_data_ready.connect(
                self._gui_logger.log_data_if_changed)
        else:
            try:
                self.SI_channel_data_ready.disconnect(
                    self._gui_logger.log_data_if_changed)
    ...
    def _disp_save_dialog(self):
        fname, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, 'Save file', '.', "Gui Configuration files (*.guicfg);;All files (*.*);;XML files (*.xml)")
        if fname:
            self.save_file(fname)

    def _disp_open_dialog(self):
        fname, ftype = QtWidgets.QFileDialog.getOpenFileName(
            self, 'Open file', '.', "Gui Configuration files (*.guicfg);;All files (*.*);;XML files (*.xml)")
        if len(fname) > 0:
            debug_logging.info(
                "Loading GUI configuration from {}".format(fname))
            self.load_file(fname)
        else:
            debug_logging.info(
                "No GUI configuration file specified for loading.")

    def _disp_dump_old_dialog(self):
        fname, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, 'Save Dump file', '.', "Text Files (*.txt);;All files (*.*)")
        if fname:
            self.dump(fname)
    ...
    def _disp_dump_new_dialog(self):
        ...
        if fname:
            ...
            channel_name_list = [channel.get_name()
                                 for channel in self._channel_group]
    ...
    def dump(self, file_name):
        with open(file_name, 'w') as f:
            for name, value in sorted(
                    list(self._data.items()), key=lambda item: item[0]):
                f.write('{}: {}\n'.format(name, value))
            f.close()
    ...
    def set_passive_observer_mode(self, passive):
        if not passive:
            if self.file_passive.isChecked():
                self._tg.SI_request_read_channel_list.disconnect(
                    self.show_passive_error)
                self._tg.SI_request_write_channel_list.disconnect(
                    self.show_passive_error)
                self._gui_logger.SI_request_background_call.disconnect(
                    self.show_passive_error)
                self.SI_passive_observer_data.disconnect(
                    self._tg.receive_passive_channel_data)
            self.file_passive.setChecked(False)
            self._tg.SI_request_read_channel_list.connect(
                self.read_channel_list)
            self._tg.SI_request_write_channel_list.connect(
                self.write_channel_list)
            self.SI_channel_data_ready.connect(self._tg.receive_channel_data)
            self._gui_logger.SI_request_background_call.connect(
                self.receive_background_call_request)
            ...
            # PySide6, which would leave the connection live and cause infinite
            # recursion.
            self.file_passive.toggled.disconnect(
                self.set_passive_observer_mode)
            if not self.file_passive.isChecked():
                ...
                self._tg.SI_request_write_channel_list.disconnect(
                    self.write_channel_list)
                self.SI_channel_data_ready.disconnect(
                    self._tg.receive_channel_data)
                self.SI_request_background_call.disconnect(
                    self.receive_background_call_request)
            ...
            self.SI_passive_observer_data.connect(
                self._tg.receive_passive_channel_data)
            self.setWindowTitle(
                '{} - PASSIVE OBSERVER MODE'.format(self._channel_group.get_name()))
...
# QApplication is a singleton per Qt docs
...
class ltc_lab_gui_app(QObject):
    ...
    def __init__(self, channel_group, passive=False,
        ...
        self.worker.start()
        # need to clone in case there are remote objects; they wont play nice
        # with qt threads
        self._channel_group = channel_group.clone(
            name="GUI flat channel group")
        self.main_window = ltc_lab_gui_main_window(
            channel_group, background_call=self.worker.background_call)
        # icon path is relative to calling script, so we need to find an
        # absolute path back to PyICe
        ...
        self.worker.SI_channel_data_ready.connect(
            self.main_window.receive_channel_data)
        self.worker.SI_dump_data_ready.connect(
            self.main_window.receive_dump_data)
        self.SI_queue_overflow.connect(
            self.main_window._queue_overflow_highlight)
        self.main_window.SI_request_background_call.connect(
            self.worker.background_call)
        self.main_window.SI_request_read_channel_list.connect(
            self.worker.read_channel_list)
        self.main_window.SI_request_dump_channel_list.connect(
            self.worker.dump_channel_list)
        self.main_window.SI_request_write_channel_list.connect(
            self.worker.write_channel_list)
        self.main_window.SI_close_main.connect(QApp.exit)
        if passive:
            self.SI_passive_observer_data.connect(
                self.main_window.receive_passive_channel_data)
    ...
    def passive_data(self, data_dict):
        try:
            ...
            # was SI_queue_overflow().emit() — calling the signal as a function
            # returns None, then .emit() on None raises AttributeError
