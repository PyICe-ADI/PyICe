...
class lab_util_locator:
    '''Complicated deep blue Python to elminate the old lab_utils file from the upper level of PyICe.
                ...
                if mod_ext == ".py":
                    ...
                    for (name, member) in inspect.getmembers(_this_module):
                        ...
                        if name in self._members and self._members[name] is not member:
                            raise Exception(f'lab_utils namespace conflict: {name} from {member.__module__} and {self._members[name].__module__}.')
    ...
    def __getattribute__(self, name):
        ...
        if name in ["_members", "__all__", "__class__"]:
            return super(lab_util_locator, self).__getattribute__("__dict__")[name]
        if name.startswith("_"):
            return globals()[name]
        return self._members[name]

sys.modules[__name__] = lab_util_locator()