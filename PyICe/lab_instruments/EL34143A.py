...
class EL34143A(scpi_instrument):
    ...
    def _add_channel_curr_range(self, channel_name, curr_range):
        ...
        new_channel.set_description(
            self.get_name() + ': ' + self._add_channel_curr_range.__doc__)
        ...
        new_channel.add_preset(
            "AUTO", "Determine appropriate range when necessary.")
    ...
    def _add_channel_volt_range(self, channel_name, volt_range):
        ...
        new_channel = channel(
            channel_name + '_range',
            write_function=self._SetVoltRange)
        new_channel.set_description(
            self.get_name() + ': ' + self._add_channel_volt_range.__doc__)
    ...
    def _add_channel_pow_range(self, channel_name, pow_range):
        ...
        new_channel = channel(
            channel_name + '_range',
            write_function=self._SetPowRange)
        new_channel.set_description(
            self.get_name() + ': ' + self._add_channel_volt_range.__doc__)
    ...
    def _add_channel_res_range(self, res_range):
        ...
        new_channel = channel(channel_name,
                              read_function=lambda: self._SetResRange)
        new_channel.set_description(
            self.get_name() + ': ' + self._add_channel_volt_range.__doc__)
    ...
    def _SetCCCurrent(self, current):
        ...
        if self.curr_range == 'AUTO':
            present_range = self._find_current_range(
                float(self.GetCCCurrent()))
            # Only change the range if necessary
            if not self.curr_ranges[present_range][0] <= current <= self.curr_ranges[present_range][1]:
                new_range = self._find_current_range(current)
                # Sending both range and value simultaneously SHOULD bypass
                # conflicts.
                self.get_interface().write(
                    f'CURR:RANG {new_range};CURR {current}')
            else:
    ...
    def _SetCVVoltage(self, voltage):
        ...
        if self.volt_range == 'AUTO':
            present_range = self._find_voltage_range(
                float(self.GetCVVoltage()))
            # Only change the range if necessary
            if not self.volt_ranges[present_range][0] <= voltage <= self.volt_ranges[present_range][1]:
                new_range = self._find_voltage_range(voltage)
                if not self.volt_ranges[new_range][0] <= float(self.GetCVVoltage(
                    # Only step the voltage if necessary
                )) <= self.volt_ranges[new_range][1]:
    ...
    def _SetCWPower(self, power):
        ...
        if self.pow_range == 'AUTO':
            ...
            # Only change the range if necessary
            if not self.pow_ranges[present_range][0] <= power <= self.pow_ranges[present_range][1]:
                new_range = self._find_power_range(power)
                if not self.pow_ranges[new_range][0] <= float(self.GetCWPower(
                    # Only step the power if necessary
                )) <= self.pow_ranges[new_range][1]:
