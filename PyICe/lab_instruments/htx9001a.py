...
class htx9001a(htx9001):
    ...
    def add_channel_irelay(self, channel_name, irelay_number):
        ...
        if self.irelay_pins[irelay_number] in self.initialized_pins:
            raise Exception(
                f"irelay number {irelay_number} already used in another channel!")
        new_channel = integer_channel(
            channel_name,
            size=1,
            write_function=lambda data: self._write_irelay(
                irelay_number,
                data))
    ...
    def _add_channel_pwm_frequency(self, channel_name, pin):
        def set_pwm_frequency(value):
            ...
            if value < flow or value > fhigh:
                raise Exception(
                    f"Invalid HTX9001A frequency {value}. Must be between {flow} and {fhigh} Hz.")
    ...
    def _add_channel_pwm_enable(self, channel_name, pin):
        def set_pwm_enable(value):
            ...
            if value not in [0, 1]:
                raise Exception(
                    f'Bad value for HTX9001A pwm_enable: {value}. Try one of: 0,"0",False,1,"1",True.')
