...
class u2300a_scope(scpi_instrument, delegator):
    ...
    def __init__(self, interface_visa, force_trigger=False,
        ...
        self._waveform_channel_types = [
            'ain_single_ended_bipolar',
            'ain_diff_bipolar',
            'ain_single_ended_unipolar',
            'ain_diff_unipolar']
        self.reset()
    ...
    def add_channel_timeout(self, channel_name):
        ...
        new_ch.set_description(
            self.get_name() + ': ' + self.add_channel_timeout.__doc__)
    ...
    def add_channel_time(self, channel_name):
        ...
        new_ch.set_description(
            self.get_name() + ': ' + self.add_channel_time.__doc__)
    ...
    def add_channel_ain_single_ended_bipolar(
        ...
        new_ch.set_delegator(self)
        new_ch.set_description(
            self.get_name() + ': ' + self.add_channel_ain_single_ended_bipolar.__doc__)
        new_ch._set_type_affinity('PyICeBLOB')
        return self._add_channel(new_ch)
    ...
    def add_channel_ain_diff_bipolar(
        ...
        new_ch.set_delegator(self)
        new_ch.set_description(
            self.get_name() + ': ' + self.add_channel_ain_diff_bipolar.__doc__)
        new_ch._set_type_affinity('PyICeBLOB')
        return self._add_channel(new_ch)
    ...
    def add_channel_ain_single_ended_unipolar(
        ...
        new_ch.set_delegator(self)
        new_ch.set_description(
            self.get_name() + ': ' + self.add_channel_ain_single_ended_unipolar.__doc__)
        new_ch._set_type_affinity('PyICeBLOB')
        return self._add_channel(new_ch)
    ...
    def add_channel_ain_diff_unipolar(
        ...
        new_ch.set_delegator(self)
        new_ch.set_description(
            self.get_name() + ': ' + self.add_channel_ain_diff_unipolar.__doc__)
        new_ch._set_type_affinity('PyICeBLOB')
        return self._add_channel(new_ch)
    ...
    def set_trigger(self, trigger_channel, mode='POST', delay_count=None,
        ...
        if trigger_channel is None:
            ...
            # Selects the external digital trigger (EXTD_AI_TRIG) pin as the
            # triggering source.
            ...
            self.get_interface().write(
                f'TRIGger:DTRiGger:POLarity {polarity_condition}')
            self._trigger_source = 'EXTD'
        elif trigger_channel == 'exta' or trigger_channel == 'EXTA':
            # Selects the external analog trigger (EXTA_TRIG) pin as the
            # triggering source.
            ...
            self.get_interface().write(
                f'TRIGger:ATRIGger:HTHReshold {high_threshold}')
            ...
            self.get_interface().write(
                f'TRIGger:ATRIGger:LTHReshold {low_threshold}')
            # TRIGger:TYPe <mode>
            ...
            self.get_interface().write(
                f'TRIGger:ATRIGger:CONDition {polarity_condition}')
            ...
            # MID mode not allowed in SONE trigger mode. See programmer manual
            # page 190 and conflicting guidance in user manual page 77.
            assert mode in ['POST']
            self.get_interface().write(f'TRIGger:TYPe {mode}')
            # TRIGger:ATRIGger:HTHReshold <value>
            # This command sets the high-threshold voltage of the AI trigger control.
            ...
            raise Exception(
                "I don't know what to do. Perhaps driver is incomplete.")
        self.check_errors()
    ...
    def add_channels_trigger(self, base_name):
        ...
        def trigger_config_ch_write(write_channel, value):
            ...
            for other_channel in write_channel.get_attribute(
                ...
                vals[other_channel.get_attribute(
                    'trigger_config_type')] = other_channel.read()
        ...
        def trigger_arm_ch_write(value):
            ...
            if value == "ARM":
                dwell_time = {"PRE": 1.02 * self.acq_time_ch.read(),   # Alot time to acquire (at least) the entire record.
                              ...
                              # trigger.
                              "POST": 0,
                              # Alot time to acquire (at least) the first half
                              # of the record.
                              "MID": 0.51 * self.acq_time_ch.read()
                              }
                self.arm_trigger()
        ...
        trigger_arm_ch.set_description(
            self.get_name() + ': ' + self.add_channels_trigger.__doc__)
        trigger_arm_ch.add_preset(
            preset_value='ARM',
            preset_description='Arms the DAQ to take a new reading.')
        self._add_channel(trigger_arm_ch)

        trigger_source_ch = channel(name=f'{base_name}_source')
        trigger_source_ch.set_attribute(
            'trigger_config_type', 'trigger_channel')
        trigger_source_ch.set_attribute('u2300a_type', 'trigger_control')
        trigger_source_ch._write = lambda val: trigger_config_ch_write(
            trigger_source_ch, val)
        trigger_source_ch.set_description(
            self.get_name() + ': ' + self.add_channels_trigger.__doc__)
        trigger_source_ch.add_preset(
            preset_value=None,
            preset_description='Immediate triggering')
        trigger_source_ch.add_preset(
            preset_value='EXTD',
            preset_description='Selects the external digital trigger (EXTD_AI_TRIG) pin as the triggering source.')
        trigger_source_ch.add_preset(
            preset_value='EXTA',
            preset_description='Selects the external analog trigger (EXTA_TRIG) pin as the triggering source.')
        for ain_ch in self._configured_channels.values():
            trigger_source_ch.add_preset(
                preset_value=ain_ch,
                preset_description=ain_ch.get_name())
        ...
        self.trigger_mode_ch._write = lambda val: trigger_config_ch_write(
            self.trigger_mode_ch, val)
        self.trigger_mode_ch.set_description(
            self.get_name() + ': ' + self.add_channels_trigger.__doc__)
        self.trigger_mode_ch.add_preset(
            preset_value='POST',
            preset_description='(Post-trigger): Continuous recording stops when trigger condition is met and data from time: (trigger_event - acq_time) to the trigger_event is returned. Trigger ARM delay will be set equal to acq_time (plus a little) to allow the minimum time needed for the entire buffer to populate.')
        self.trigger_mode_ch.add_preset(
            preset_value='PRE',
            preset_description='(Pre-trigger): Recording begins when the trigger condition is met and data from the time of the trigger_event to the time (trigger_event + acq_time) is returned. Recording is causal with trigger, no need for extra ARM delay.')
        self.trigger_mode_ch.add_preset(
            preset_value='MID',
            preset_description='(Mid-trigger): Continuous recording extends from half the acquisition time before the trigger event to half the acquisition after the trigger event. Trigger ARM delay will be set equal to acq_time/2 (plus a little) to allow the minimum time needed to populate the first half of the buffer.')
        self.trigger_mode_ch.add_preset(
            preset_value='DEL',
            preset_description='(Delay-trigger): Input is acquired when the delay count reaches zero. The delay count starts immediately after the trigger condition is met.')
        ...
        trigger_delay_count_ch.set_attribute(
            'trigger_config_type', 'delay_count')
        trigger_delay_count_ch.set_attribute('u2300a_type', 'trigger_control')
        trigger_delay_count_ch._write = lambda val: trigger_config_ch_write(
            trigger_delay_count_ch, val)
        trigger_delay_count_ch.set_description(
            self.get_name() + ': ' + self.add_channels_trigger.__doc__)
        ...
        trigger_polarity_condition_ch.set_attribute(
            'trigger_config_type', 'polarity_condition')
        trigger_polarity_condition_ch.set_attribute(
            'u2300a_type', 'trigger_control')
        trigger_polarity_condition_ch._write = lambda val: trigger_config_ch_write(
            trigger_polarity_condition_ch, val)
        trigger_polarity_condition_ch.set_description(
            self.get_name() + ': ' + self.add_channels_trigger.__doc__)
        trigger_polarity_condition_ch.add_preset(
            preset_value='AHIG',
            preset_description='Above-High-Level triggering selected. The trigger signal is generated when the analog signal is higher than the high-threshold voltage.')
        trigger_polarity_condition_ch.add_preset(
            preset_value='BLOW',
            preset_description='Below-Low-Level triggering selected. The trigger signal is generated when the analog signal is lower than the low-threshold voltage.')
        trigger_polarity_condition_ch.add_preset(
            preset_value='WIND',
            preset_description='Window (inside region) triggering selected. The trigger signal is generated when the analog signal falls within the range of the high-threshold and low-threshold voltages.')
        trigger_polarity_condition_ch.add_preset(
            preset_value='POS',
            preset_description='(Positive-edge triggering): The trigger signal is generated when a rising edge is detected in the digital signal.')
        trigger_polarity_condition_ch.add_preset(
            preset_value='NEG',
            preset_description='(Negative-edge triggering): The trigger signal is generated when a falling edge is detected in the digital signal.')
        ...
        trigger_high_threshold_ch.set_attribute(
            'trigger_config_type', 'high_threshold')
        trigger_high_threshold_ch.set_attribute('u2300a_type', 'cccc')
        trigger_high_threshold_ch._write = lambda val: trigger_config_ch_write(
            trigger_high_threshold_ch, val)
        trigger_high_threshold_ch.set_description(
            self.get_name() + ': ' + self.add_channels_trigger.__doc__)
        ...
        trigger_low_threshold_ch.set_attribute(
            'trigger_config_type', 'low_threshold')
        trigger_low_threshold_ch.set_attribute(
            'u2300a_type', 'trigger_control')
        trigger_low_threshold_ch._write = lambda val: trigger_config_ch_write(
            trigger_low_threshold_ch, val)
        trigger_low_threshold_ch.set_description(
            self.get_name() + ': ' + self.add_channels_trigger.__doc__)
        ...
        trigger_source_ch.set_attribute(
            'trigger_related_channels',
            [
                self.trigger_mode_ch,
                trigger_delay_count_ch,
                trigger_polarity_condition_ch,
                trigger_high_threshold_ch,
                trigger_low_threshold_ch])
        self.trigger_mode_ch.set_attribute(
            'trigger_related_channels',
            [
                trigger_source_ch,
                trigger_delay_count_ch,
    ...
    def add_channel_acquisition_time(self, name):
        ...
        self.acq_time_ch.set_attribute(
            'u2300a_type', 'ain_acquisition_control')
        self.acq_time_ch.set_description(
            self.get_name() + ': ' + self.add_channel_acquisition_time.__doc__)
    ...
    def _set_acquisition_time(self, acquisition_time,
        ...
        if acquisition_time == 'MAX':
            ...
            point_count = acquisition_time * \
                self._compute_sample_rate(sample_rate, channel_count)
        point_count = max(int(point_count), 1)
        assert point_count * \
            channel_count <= self._ai_channels['max_record_length']
        self.get_interface().write(
            f'ACQuire:POINts {point_count}')  # This is per-channel!
    ...
    def add_channel_sample_rate(self, name):
        ...
        sample_rate_ch.set_description(
            self.get_name() + ': ' + self.add_channel_sample_rate.__doc__)
    ...
    def _compute_sample_rate(self, sample_rate, channel_count):
        ...
        assert sample_rate * channel_count <= (
            self._ai_channels['max_sample_rate'] if channel_count > 1 else self._ai_channels['max_sample_rate_single'])
    ...
    def _set_range(self, channel, sig_range, polarity):
        ...
        self.get_interface().write(
            f'ROUTe:CHANnel:POLarity {polarity}, (@{channel.get_attribute("internal_address")})')
        assert sig_range in self._ai_channels[polarity]['range']
        channel.set_attribute('sig_range', sig_range)
        self.get_interface().write(
            f"ROUTe:CHANnel:RANGe {sig_range}, (@{channel.get_attribute('internal_address')})")
    ...
    def _scale_fn(self, channel):
        ...
        return lambda x, lsb=gains['lsb'], offset=offset, shift=self._ai_channels['bit_offset']: lsb * (
            x >> shift) + offset
    ...
    def arm_trigger(self, channel_list=None):
        ...
        scan_internal_addresses = [
            channel.get_attribute('internal_address') for channel in filter(
                lambda ch: ch.get_attribute(
                    'u2300a_type') in self._waveform_channel_types,
                channel_list)]
        scan_internal_addresses = list(
            set(scan_internal_addresses))  # remove duplicates #TODO why??
        if isinstance(self._trigger_source, channel):
            # Move the trigger channel to front of scanlist. Won't work if
            # repeated in scanlist!!
            scan_internal_addresses.remove(
                self._trigger_source.get_attribute('internal_address'))
            scan_internal_addresses.insert(
                0, self._trigger_source.get_attribute('internal_address'))
        # Special dance to make sure setup is always allowable in any order.
        # May be optimizable later with some experimentation
        self._set_sample_rate(sample_rate=3,
                              channel_count=len(scan_internal_addresses))
        self._set_acquisition_time(
            acquisition_time=1e-6,
            sample_rate=3,
            channel_count=len(scan_internal_addresses))
        cmd = "ROUTe:SCAN (@"
        ...
        self._set_sample_rate(
            sample_rate=self._ain_configuration['sample_rate'],
            channel_count=len(scan_internal_addresses))
        self._set_acquisition_time(
            acquisition_time=self._ain_configuration['acquisition_time'],
            sample_rate=self._ain_configuration['sample_rate'],
            channel_count=len(scan_internal_addresses))
        self.check_errors()
    ...
    def read_delegated_channel_list(self, channel_list):
        ...
        if resp == 'YES':
            ...
            # I don't know why the binary drivers aren't working right. I get a
            # response only 9 points long... Workaround for now rather than
            # debug.
        ...
        for ch in channel_list:
            if ch.get_attribute('u2300a_type') == 'ain_time':
                ...
                if not numpy_missing:
                    results[ch.get_name()] = fromiter(
                        map(lambda idx: idx / self._sample_rate, int_points), dtype=dtype('<d'))
                else:
                    results[ch.get_name()] = list(
                        map(lambda idx: idx / self._sample_rate, int_points))
            elif ch.get_attribute('u2300a_type') in self._waveform_channel_types:
                if int_data is not None:
                    ch_idx = self._last_scan_internal_addresses.index(
                        ch.get_attribute('internal_address'))
                    ch_data = (
                        int_data[i] for i in range(
                            ch_idx, len(int_data), len(
                                self._last_scan_internal_addresses)))
                    if not numpy_missing:
                        results[ch.get_name()] = fromiter(map(lambda point, scale_f=ch.get_attribute(
                            'scale_fn'): scale_f(point), ch_data), dtype=dtype('<d'))
                    else:
                        results[ch.get_name()] = list(
                            map(lambda point, scale_f=ch.get_attribute('scale_fn'): scale_f(point), ch_data))
                else:
                    # failed trigger
    ...
    def get_all_settings(self):
        ...
        result["atrigger_source"] = self.get_interface().ask(
            "TRIGger:ATRIGger:SOURce?")
        result["atrigger_threshold"] = self.get_interface().ask(
            "TRIGger:ATRIGger:HTHReshold?")
        result["ltrigger_threshold"] = self.get_interface().ask(
            "TRIGger:ATRIGger:LTHReshold?")
        result["channel_polarity"] = self.get_interface().ask(
            f"ROUTe:CHANnel:POLarity? (@101:{max(self._ai_channels['single_ended'])})")
        result["channel_range"] = self.get_interface().ask(
            f"ROUTe:CHANnel:RANGe? (@101:{max(self._ai_channels['single_ended'])})")
        result["rout_channel_stype"] = self.get_interface().ask(
            f"ROUTe:CHANnel:STYPe? (@101:{max(self._ai_channels['single_ended'])})")
...
class u2300a_datalogger(u2300a_scope):
    ...
    def _setup(self):
        ...
        self._set_sample_rate(
            sample_rate=self._ain_configuration['sample_rate'],
            channel_count=len(scan_internal_addresses))
        self.data_buffer = {ia: collections.deque()
    ...
    def read_delegated_channel_list(self, channel_list):
        ...
        if time.time() - self.time_since > 0.5 or not data_in_waiting or self.stopping:
            while True:
                ...
                if resp == 'DATA':
                    # Indicates that at least one block of data is completed
                    # and ready to be read back.
                    self.get_interface().write('WAVeform:DATA?')
                    # I don't know why the binary drivers aren't working right.
                    # I get a response only 9 points long... Workaround for now
                    # rather than debug.
                    raw_data = self.get_interface().read_raw()
                    header = raw_data[:10]
                    raw_data = raw_data[10:]
                    ...
                    # fmt_str = '<' + 'h' * self._point_count *
                    # len(self._last_scan_internal_addresses)        ### Manual
                    ...
                    for i, a in enumerate(self._last_scan_internal_addresses):
                        ch_data = (
                            int_data[i] for i in range(
                                i, len(int_data), len(
                                    self._last_scan_internal_addresses)))
                        # just unscaled integer readings; rescale later!
                        self.data_buffer[a].extend(ch_data)
                    self.time_since = time.time()
                    self.stopping = False
                    break
                elif resp == 'FRAG':
                    if data_in_waiting:
                        break
                    # Fragment, indicates that the instrument has started to
                    # acquire data, but has yet to complete a single block of
                    # data.
                elif resp == 'EPTY':
                    if data_in_waiting:
                        break
                    ...
                    # Indicates that the buffer is full and the acquisition is
                    # stopped.
    ...
    def _read_assist(self, channel_list):
        # TODO: what if a channel address appears twice or more in the
        # scanlist??
        results = results_ord_dict()
        try:
            for ch in channel_list:
                if ch.get_attribute('u2300a_type') == 'ain_time':
                    ...
                    results[ch.get_name()] = ch.get_attribute('scale_fn')(
                        self.data_buffer[ch.get_attribute('internal_address')].popleft())
                else:
                    raise Exception("U2300A: Don't know what to do yet.")
            self.point_idx += 1
...
class u2300a_DVM(scpi_instrument, delegator):
    ...
    def add_channel_ain_single_ended_bipolar(
        ...
        self._set_sigtype(new_ch, sig_mode='NRS')  # Return to AI_SENSE
        new_ch.set_attribute('scale_fn', self._scale_fn(new_ch))
        new_ch.set_display_format_function(
    ...
    def _set_range(self, channel, sig_range, polarity):
        ...
        channel.set_attribute('polarity', polarity)
        self.get_interface().write(
            f'SENSe:VOLTage:POLarity {polarity}, (@{channel.get_attribute("internal_address")})')
        ...
        channel.set_attribute('sig_range', sig_range)
        self.get_interface().write(
            f"SENSe:VOLTage:RANGe {sig_range}, (@{channel.get_attribute('internal_address')})")
    ...
    def read_delegated_channel_list(self, channel_list):
        ...
        ch_str = f'(@{",".join(channels)})'
        # SORT???
        data = [float(v) for v in self.get_interface().ask(
            f'MEASure:VOLTage:DC? {ch_str}').split(',')]
        results = results_ord_dict()
...
class u2331a_base():
    ...
    _ai_channels = {}
    _ai_channels['single_ended'] = range(
        101, 164 + 1)  # Available channel numbers
    _ai_channels['differential'] = range(
        101, 132 + 1)  # Avaiable channel numbers
    # 12-bit, lsbyte first (little-endian), packed to left.
    _ai_channels['bit_offset'] = 4
    # 1e6 available when multiplexing.
    _ai_channels['max_sample_rate'] = 1000000
    # 3e6 available in single-channel mode.
    _ai_channels['max_sample_rate_single'] = 3000000
    _ai_channels['max_record_length'] = 8000000  # Common to all models.
    _ai_channels['unipolar'] = {}
    # max/min/lsb weight settins for each ain range
    _ai_channels['unipolar']['range'] = {}
...
if __name__ == '__main__':
    ...
    vif = m.get_visa_interface(
        'USB0::0x0957::0x1518::TW56510006::0::INSTR',
        timeout=3)
    ...
    if False:
        ...
        pandas_df = pandas.DataFrame(
            zip(data['time'], data['foo1'], data['foo2']))
