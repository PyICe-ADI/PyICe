...
class AD5693R(instrument):
    ...
    def __init__(self, interface_twi, addr7):
        ...
        # send dac codes to this address for auto update.
        self.dac_reg = 0x30
        self.control_reg = 0x40      # sets up gain and power down modes.
        self.gain_code = 0x0800    # 0 to 5V rather than 0 to 2.5V.
        # start with output on, this sets and r to GND for disable mode.
        ...
        # In lieu of using the reset bit which nukes the I2C port.
        self._set_voltage(0)
    ...
    def _update_controlreg(self):
        ...
        self.twi.write_register(
            addr7=self.addr7,
            commandCode=self.control_reg,
            data=swap_endian(
                self.impedance | self.refenable | self.gain_code,
                elementCount=2),
            data_size=16,
            use_pec=False)
    ...
    def _set_outputz(self, z):
        if z in [0, "0"]:
            ...
            raise Exception(
                f'AD5693R impedance setting must be one of: {["0", "1k", "100k", "z"]}')
        self._update_controlreg()
