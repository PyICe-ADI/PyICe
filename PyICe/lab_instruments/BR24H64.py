...
class BR24H64(instrument):
    ...
    def _write_location(self, location, data):
        ...
        while tries:
            try:
                tries -= 1
                # time.sleep(0.0035) # Max write delay value from the datasheet
                # - Doesn't appear to be needed ???
                self.twi.write_register(addr7=self.addr7, commandCode=location >> 8, data=(
                    location & 0xff) + (data << 8), data_size=16, use_pec=False)
                return
                ...
                if not tries:
                    raise twi_interface.i2cIOError(
                        f"BR24H64 EEPROM write communication failed at ADDR7:{hex(self.addr7)}.") from e
        ...
        while tries:
            try:
                ...
                self.twi.write_register(
                    addr7=self.addr7, commandCode=location >> 8, data=(
                        location & 0x00ff), data_size=8, use_pec=False)
    ...
    def read_dictionary(self, verbose=False):
        ...
        if verbose:
            print_banner(
                f"Reading BR24H64 EEPROM media from device at ADDR7:{hex(self.addr7)}.", "Please Wait..")
        ...
        for character in file:
            # for-loop returns integers instead of bytestrings for some reason
        ...
        if verbose:
            print_banner(
                f"BR24H64 EEPROM at ADDR7:{hex(self.addr7)} reading complete.")
        return data
    ...
    def write_dictionary(self, data_dict, verbose=False):
        ...
        for key in data_dict:
            file += self.STX + \
                key.encode("ASCII") + self.US + \
                data_dict[key].encode("ASCII") + self.ETX
