...
class agilent_3034a(oscilloscope):
    ...
    def __init__(self, interface_visa, force_trigger=False,
        ...
        if reset:
            ...
            # Get to a known state for full automation if so desired.
            self.reset()
            # Sleep after a reset. Scope might crash without this.
            time.sleep(1)
        ...
        # Dummy Channel fills, make sure time channel reads don't crash before
        # acquisition
        self.time_info = {}
    ...
    def resync_scope(self):
        ...
        self.reset()
        self.delay(1)
        # Turn off the softkey menu so that channel info displays on the
        # bottom.  This maybe could have worked in just the _init_ as once it's
        # done once it appears to stick through reseting and clearing the
        # scope.
        self.get_interface().write(':SYSTem:MENU OFF')
        self.get_interface().write(":WAVeform:FORMat BYTE")
        # Maximum number of points by default (scope must be stopped)
        self.get_interface().write(":WAVeform:POINts:MODE RAW")
        enabled_channels = []
    ...
    def add_Ycontrol_Yreadback_channels(self, name, number):
        ...
        self.add_channel_Yrange_readback(
            name=f"{name}_Yrange_readback", number=number)
        self.add_channel_Yoffset_readback(
    ...
    def add_Xcontrol_Xreadback_channels(self, prefix):
        ...
        self.add_channel_Xposition_readback(
            name=f"{prefix}_Xposition_readback")
        self.add_channel_Xreference_readback(
    ...
    def add_trigger_channels(self, prefix):
        ...
        self.add_channel_trigger_glitch_lessthan(
            name=f"{prefix}_glitch_lessthan")
        self.add_channel_trigger_glitch_polarity(
            name=f"{prefix}_glitch_polarity")
        self.add_channel_trigger_glitch_qualifier(
            name=f"{prefix}_glitch_qualifier")
        self.add_channel_trigger_glitch_greaterthan(
            name=f"{prefix}_glitch_greaterthan")
        self.add_channel_trigger_runt_polarity(name=f"{prefix}_runt_polarity")
        self.add_channel_trigger_runt_qualifier(
            name=f"{prefix}_runt_qualifier")
        ...
        self.add_channel_trigger_runt_level_high(
            name=f"{prefix}_trigger_runt_level_high")
        self.add_channel_trigger_runt_level_low(
            name=f"{prefix}_trigger_runt_level_low")
        self.add_channel_trigger_HFReject(name=f"{prefix}_trigger_HFReject")
        self.add_channel_trigger_pattern(name=f"{prefix}_pattern")
        self.add_channel_trigger_pattern_format(
            name=f"{prefix}_pattern_format")
        self.add_channel_trigger_pattern_greaterthan(
            name=f"{prefix}_pattern_greaterthan")
        self.add_channel_trigger_pattern_lessthan(
            name=f"{prefix}_pattern_lessthan")
        self.add_channel_trigger_pattern_qualifier(
            name=f"{prefix}_pattern_qualifier")
    ...
    def add_channel_timebase(self, name):
        ...
        def compute_x_points(self):
            ...
            xpoints_gen = map(
                ...
                self.time_info["origin"],
                range(
                    self.time_info["points"]))
            return fromiter(xpoints_gen, dtype=dtype('<d'))
        new_channel = channel(
            name, read_function=lambda: compute_x_points(self))
        new_channel.set_delegator(self)
        self._add_channel(new_channel)
        new_channel._set_type_affinity('PyICeBLOB')
    ...
    def _read_scope_time_info(self):
        ...
        for scope_channel_number in range(1, 5):
            enable_status[scope_channel_number] = int(self.get_interface().ask(
                # Is the channel enabled?
                f":CHANnel{scope_channel_number}:DISPlay?"))
        an_enabled_channel = [chnum for chnum, enstatus in enable_status.items(
            # Pick the first enabled channel
        ) if enstatus][0]
        # Set the first enabled channel as waveform source
        self.get_interface().write(
            f':WAVeform:SOURce CHANnel{an_enabled_channel}')
        self.time_info["points"] = int(self.get_interface().ask(
            # int(preamble[2])
            ":WAVeform:POINts?"))
        self.time_info["increment"] = float(self.get_interface().ask(
            # float(preamble[4])
            ":WAVeform:XINCrement?"))
        self.time_info["origin"] = float(self.get_interface().ask(
            # float(preamble[5])
            ":WAVeform:XORigin?"))
        self.time_info["reference"] = float(self.get_interface().ask(
            # float(preamble[6])
            ":WAVeform:XREFerence?"))
        self.time_info["scale"] = self.time_info["increment"] * \
    ...
    def add_all_timebase_trigger_aquisition_channels(self, prefix="scope"):
        ...
        self.add_channel_pointcount_readback(
            name=f"{prefix}_points_count_readback")
    ...
    def _set_runmode(self, value):
        ...
        if value in ["RUN", "SINGLE"]:
            ...
            # is written.
            timeout = 9 + math.ceil(1.5 * xrange)
            timeout_time = time.time() + timeout
            # Asking AER? clears AER. If SINGLE/SINGLE or RUN/RUN is written
            # without a trigger in between, AER won't be high, and the timeout
            # exception is raised. Note: SINGLE/RUN/SINGLE/RUN etc. CAN be
            # written without triggers in between and AER goes high each time.
            while not int(self.get_interface().ask(":AER?")):
                self.delay(0.01)
                if time.time() > timeout_time:
                    raise Exception(
                        f'Agilent 3034a: AER is still low {timeout}s after writing two {value} commands in a row to the scope without a scope trigger in between')
        else:  # Must be STOP
            self.delay()
        ...
        while (True):
            if self.scope_stopped():
                ...
                if remaining_time < last_remaining_time:
                    ...
                    print(
                        f'Agilent Scope waiting for trigger: {remaining_time}\r',
                        end="")
                self.delay(0.05)
    ...
    def add_channel_probe_gain(self, name, number):
        ...
        new_channel = channel(
            name, write_function=lambda value: _set_probe_gain(
                number, value))
        new_channel._set_value(
            self.get_interface().ask(
                f":CHANnel{number}:PROBe?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
    ...
    def add_channel_Yrange(self, name, number):
        ...
        new_channel = channel(
            name, write_function=lambda value: _set_Yrange(
                number, value))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
    ...
    def add_channel_Yoffset(self, name, number):
        ...
        new_channel = channel(
            name, write_function=lambda value: _set_Yoffset(
                number, value))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
    ...
    def add_channel_impedance(self, name, number):
        ...
        new_channel = channel(
            name, write_function=lambda value: _set_impedance(
                number, value))
    ...
    def add_channel_trigger_pattern_qualifier(self, name):
        ...
        def _set_trigger_pattern_qualifier(value):
            ...
            self.delay()
        new_channel = channel(
            name, write_function=_set_trigger_pattern_qualifier)
        ...
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:GLITch:QUALifier?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_pattern_format(self, name):
        ...
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:PATTern:FORMat?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_pattern_greaterthan(self, name):
        ...
        def _set_trigger_pattern_greaterthan(value):
            ...
            self.delay()
        new_channel = channel(
            name, write_function=_set_trigger_pattern_greaterthan)
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:PATTern:GREaterthan?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_pattern_lessthan(self, name):
        ...
        def _set_trigger_pattern_lessthan(value):
            ...
            self.delay()
        new_channel = channel(
            name, write_function=_set_trigger_pattern_lessthan)
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:PATTern:LESSthan?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_pattern_range(self, name):
        ...
        new_channel = channel(
            name,
            write_function=lambda less_than,
            greater_than: _set_trigger_pattern_range(
                less_than,
                greater_than))
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:PATTern:RANGe?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_glitch_greaterthan(self, name):
        ...
        def _set_trigger_glitch_greaterthan(value):
            ...
            self.delay()
        new_channel = channel(
            name, write_function=_set_trigger_glitch_greaterthan)
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:GLITch:GREaterthan?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_glitch_lessthan(self, name):
        ...
        def _set_trigger_glitch_lessthan(value):
            ...
            self.delay()
        new_channel = channel(
            name, write_function=_set_trigger_glitch_lessthan)
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:GLITch:LESSthan?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_glitch_level(self, name):
        ...
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:GLITch:LEVel?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_glitch_source(self, name):
        ...
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:GLITch:SOURce?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_glitch_polarity(self, name):
        ...
        def _set_trigger_glitch_polarity(value):
            ...
            self.delay()
        new_channel = channel(
            name, write_function=_set_trigger_glitch_polarity)
        new_channel.add_preset("POSITIVE", "")
        new_channel.add_preset("NEGATIVE", "")
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:GLITch:POLarity?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_glitch_qualifier(self, name):
        ...
        def _set_trigger_glitch_qualifier(value):
            ...
            self.delay()
        new_channel = channel(
            name, write_function=_set_trigger_glitch_qualifier)
        new_channel.add_preset("GREATERTHAN", "")
        new_channel.add_preset("LESSTHAN", "")
    ...
    def add_channel_trigger_glitch_range(self, name):
        ...
        new_channel = channel(
            name,
            write_function=lambda less_than,
            greater_than: _set_trigger_glitch_range(
                less_than,
                greater_than))
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:GLITch:RANGe?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_runt_polarity(self, name):
        ...
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:RUNT:POLarity?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_runt_qualifier(self, name):
        ...
        new_channel.add_preset("RANGE", "")
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:RUNT:QUALifier?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_runt_source(self, name):
        ...
        new_channel._set_value(
            self.get_interface().ask(":TRIGger:RUNT:SOURce?"))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (None,))
        return new_channel
    ...
    def add_channel_trigger_runt_level_high(self, name):
        ...
        def _set_trigger_level_high(value):
            ...
            self.get_interface().write(
                f":TRIGger:LEVel:HIGH {value}, {source}")
            self.delay()
    ...
    def add_channel_meas_frequency(self, name, number):
        ...
        def _get_frequency_measurement(number):
            ...
            value = float(
                self.get_interface().ask(
                    f":MEASure:FREQuency? CHANnel{number}"))
            if scope_was_not_stopped:
                self.get_interface().write("RUN")
            return value
        new_channel = channel(
            name, read_function=lambda: _get_frequency_measurement(number))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
    ...
    def add_channel_meas_period(self, name, number):
        ...
        def _get_period_measurement():
            ...
            value = float(
                self.get_interface().ask(
                    f":MEASure:PERiod? CHANnel{number}"))
    ...
    def add_channel_meas_dutycycle(self, name, number):
        ...
        def _get_dutycycle_measurement(number):
            ...
            if scope_was_not_stopped:
                self._set_runmode('RUN')
            return result
        new_channel = channel(
            name, read_function=lambda: _get_dutycycle_measurement(number))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
    ...
    def add_channel_meas_risetime(self, name, number):
        ...
        def _get_risetime_measurement(number):
            ...
            return result
        new_channel = channel(
            name, read_function=lambda: _get_risetime_measurement(number))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
    ...
    def add_channel_meas_falltime(self, name, number):
        ...
        def _get_falltime_measurement(number):
            ...
            return result
        new_channel = channel(
            name, read_function=lambda: _get_falltime_measurement(number))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
    ...
    def add_channel_meas_delay(self, name, first_channel, second_channel):
        ...
        def _get_delay_measurement(first_channel, second_channel):
            ...
            return float(self.get_interface().ask(":MEASure:DELay?")) if float(
                self.get_interface().ask(":MEASure:DELay?")) != 9.9e37 else float("Inf")
        new_channel = channel(
            name, read_function=lambda: _get_delay_measurement(
        ...
        self.add_channel_meas_define_abs_thresh(
            name=f"{name}_thresh1", number=first_channel)
        self.add_channel_meas_define_abs_thresh(
            name=f"{name}_thresh2", number=second_channel)
        new_channel.set_attribute(
            'dependent_physical_channels', (first_channel, second_channel))
        return new_channel
    ...
    def add_channel_meas_pwidth(self, name, number):
        ...
        def _get_pwidth_measurement(number):
            ...
            return result
        new_channel = channel(
            name, read_function=lambda: _get_pwidth_measurement(number))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
    ...
    def add_channel_meas_nwidth(self, name, number):
        ...
        def _get_nwidth_measurement(number):
            ...
            return result
        new_channel = channel(
            name, read_function=lambda: _get_nwidth_measurement(number))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
    ...
    def add_channel_meas_vmax(self, name, number):
        ...
        def _get_vmax_measurement(number):
            ...
            return result
        new_channel = channel(
            name, read_function=lambda: _get_vmax_measurement(number))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
    ...
    def add_channel_meas_vaverage(self, name, number):
        ...
        def _get_vaverage_measurement(number):
            ...
            self.delay(0.1 + self.get_time_base() * 10 * 1.2)
            if scope_was_not_stopped:
                self._set_runmode('STOP')
            result = float(
                self.get_interface().ask(
                    f":MEASure:VAVerage? CHANnel{number}"))
            if scope_was_not_stopped:
                self._set_runmode('RUN')
            return result
        new_channel = channel(
            name, read_function=lambda: _get_vaverage_measurement(number))
        self._add_channel(new_channel)
        new_channel.set_attribute('dependent_physical_channels', (number,))
        return new_channel
...
##########################################################################
    ...
    def add_Ychannel(self, name, number):
        ...
        self.Ychannels[number]["main_channel"] = channel(
            name, read_function=lambda: self._read_scope_channel(number))
        self.Ychannels[number]["probe_gain_channel"] = self.add_channel_probe_gain(
            name=f"{name}_probe_gain", number=number)
        self.Ychannels[number]["BWLimit_channel"] = self.add_channel_BWLimit(
            name=f"{name}_BWlimit", number=number)
        self.Ychannels[number]["invert_channel"] = self.add_channel_invert(
            name=f"{name}_invert", number=number)
        self.Ychannels[number]["Yrange_channel"] = self.add_channel_Yrange(
            name=f"{name}_Yrange", number=number)
        self.Ychannels[number]["Yoffset_channel"] = self.add_channel_Yoffset(
            name=f"{name}_Yoffset", number=number)
        self.Ychannels[number]["Yrange_readback"] = self.add_channel_Yrange_readback(
            name=f"{name}_Yrange_readback", number=number)
        self.Ychannels[number]["Yoffset_readback"] = self.add_channel_Yoffset_readback(
            name=f"{name}_Yoffset_readback", number=number)
        self.Ychannels[number]["impedance_channel"] = self.add_channel_impedance(
            name=f"{name}_Impedance", number=number)
        self.Ychannels[number]["units_channel"] = self.add_channel_units(
            name=f"{name}_units", number=number)
        self.Ychannels[number]["coupling_channel"] = self.add_channel_coupling(
            name=f"{name}_coupling", number=number)
        self.Ychannels[number]["display_channel"] = self.add_channel_display(
            name=f"{name}_display", number=number)
        self.Ychannels[number]["main_channel"].set_delegator(self)
        main_channel = self._add_channel(
            self.Ychannels[number]["main_channel"])
        main_channel.set_attribute('dependent_physical_channels', (number,))
        self.Ychannels[number]["display_channel"].write(
            True)  # legacy script support
    ...
    def purge_all_Ychannels(self):
        ...
        for channel_number in self.Ychannels:
            for channel_name in self.Ychannels[channel_number]:
                self.remove_channel(
                    self.Ychannels[channel_number][channel_name])
                remove_channels.append(
                    {"channel_number": channel_number, "channel_name": channel_name})
        # This can't be done in the loop
        for remove_channel in remove_channels:
            # Iterator size not allowed to change dynamically.
            del self.Ychannels[remove_channel["channel_number"]
    ...
    def add_Xchannels(self, prefix):
        ...
        self.Xchannels["Xposition"] = self.add_channel_Xposition(
            name=f"{prefix}_Xposition")
        self.Xchannels["Xreference"] = self.add_channel_Xreference(
            name=f"{prefix}_Xreference")
        self.Xchannels["Xrange_readback"] = self.add_channel_Xrange_readback(
            name=f"{prefix}_Xrange_readback")
        self.Xchannels["Xposition_readback"] = self.add_channel_Xposition_readback(
            name=f"{prefix}_Xposition_readback")
        self.Xchannels["Xreference_readback"] = self.add_channel_Xreference_readback(
            name=f"{prefix}_Xreference_readback")
        self.Xchannels["triggerlevel"] = self.add_channel_triggerlevel(
            name=f"{prefix}_trigger_level")
        self.Xchannels["triggermode"] = self.add_channel_triggermode(
            name=f"{prefix}_trigger_mode")
        self.Xchannels["triggerslope"] = self.add_channel_triggerslope(
            name=f"{prefix}_trigger_slope")
        self.Xchannels["triggersource"] = self.add_channel_triggersource(
            name=f"{prefix}_trigger_source")
        self.Xchannels["triggertype"] = self.add_channel_triggertype(
            name=f"{prefix}_trigger_type")
        self.Xchannels["acquire_type"] = self.add_channel_acquire_type(
            name=f"{prefix}_acquire_type")
        self.Xchannels["acquire_count"] = self.add_channel_acquire_count(
            name=f"{prefix}_acquire_count")
        self.Xchannels["pointcount"] = self.add_channel_pointcount(
            name=f"{prefix}_points_count")
        self.Xchannels["pointcount_readback"] = self.add_channel_pointcount_readback(
            name=f"{prefix}_points_count_readback")
        self.Xchannels["runmode"] = self.add_channel_runmode(
            name=f"{prefix}_run_mode")
        self.Xchannels["time"] = self.add_channel_time(
            name=f"{prefix}_timedata")
        self.Xchannels["trigger_glitch_range"] = self.add_channel_trigger_glitch_range(
            name=f"{prefix}_glitch_range")
        self.Xchannels["trigger_glitch_level"] = self.add_channel_trigger_glitch_level(
            name=f"{prefix}_glitch_level")
        self.Xchannels["trigger_glitch_source"] = self.add_channel_trigger_glitch_source(
            name=f"{prefix}_glitch_source")
        self.Xchannels["trigger_glitch_lessthan"] = self.add_channel_trigger_glitch_lessthan(
            name=f"{prefix}_glitch_lessthan")
        self.Xchannels["trigger_glitch_polarity"] = self.add_channel_trigger_glitch_polarity(
            name=f"{prefix}_glitch_polarity")
        self.Xchannels["trigger_glitch_qualifier"] = self.add_channel_trigger_glitch_qualifier(
            name=f"{prefix}_glitch_qualifier")
        self.Xchannels["trigger_glitch_greaterthan"] = self.add_channel_trigger_glitch_greaterthan(
            name=f"{prefix}_glitch_greaterthan")
        self.Xchannels["trigger_runt_polarity"] = self.add_channel_trigger_runt_polarity(
            name=f"{prefix}_runt_polarity")
        self.Xchannels["trigger_runt_qualifier"] = self.add_channel_trigger_runt_qualifier(
            name=f"{prefix}_runt_qualifier")
        self.Xchannels["trigger_runt_source"] = self.add_channel_trigger_runt_source(
            name=f"{prefix}_runt_source")
        self.Xchannels["trigger_runt_time"] = self.add_channel_trigger_runt_time(
            name=f"{prefix}_runt_time")
        self.Xchannels["trigger_runt_level_high"] = self.add_channel_trigger_runt_level_high(
            name=f"{prefix}_trigger_runt_level_high")
        self.Xchannels["trigger_runt_level_low"] = self.add_channel_trigger_runt_level_low(
            name=f"{prefix}_trigger_runt_level_low")
        self.Xchannels["trigger_HFReject"] = self.add_channel_trigger_HFReject(
            name=f"{prefix}_trigger_HFReject")
        ...
        def compute_x_points(self):
            ...
            xpoints = [
                (x -
                 self.time_info["reference"]) *
                self.time_info["increment"] +
                self.time_info["origin"] for x in range(
                    self.time_info["points"])]
            return xpoints
