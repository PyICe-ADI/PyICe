...
class htx9011(scpi_instrument):
    ...
    def __init__(self, interface_visa, calibrating=False, serializing=False):
        ...
        Initializes 18 instance attributes that configure the object's
        behavior.

        Calls the parent constructor to inherit base behavior, and initializes 18 instance attributes that configure the object's behavior.

        Args:
            calibrating: If True, the device is in calibration mode.
            interface_visa: VISA interface instance.
            serializing: Serializing to use.
        """
        ...
        # self._write_gpio(list(self.gpio_pins.keys()),len(self.gpio_pins)*'Z')
        # #HiZ all GPIO Pins
        self.pwm_duty_cycle = {}
        self.pwm_frequency = {}
        self.prescale = {}
    ...
    def add_channel_relay_bypass(self, channel_name, relay_number):
        ...
        if self.relay_pins_bypass[relay_number] in self.initialized_pins:
            raise Exception(
                f'relay number {relay_number} already used in another channel!')
        new_channel = integer_channel(
            channel_name,
            size=1,
            write_function=lambda data: self._write_relay_bypass(
                relay_number,
                data))
        new_channel.set_description(
            f'Closes or opens bypass relay of channel {relay_number} when written to True or False, respectively')
        new_channel.add_preset(
            'True', True, f"Close bypass relay of channel {relay_number}.")
        new_channel.add_preset(
            'False',
            False,
            f"Open bypass relay of channel {relay_number}.")
    ...
    def add_channel_master_relay_bias(self, channel_name):
        ...
        new_channel = integer_channel(
            channel_name,
            size=1,
            write_function=lambda value: self._write_master_relay_bias(value))
        self._add_channel(new_channel)
        new_channel.set_write_delay(0.2)
    ...
    def add_channel_relay_connect(self, channel_name, relay_number):
        ...
        if self.relay_pins_connect[relay_number] in self.initialized_pins:
            raise Exception(
                f'relay connect{relay_number} already used in another channel!')
        new_channel = integer_channel(
            channel_name,
            size=1,
            write_function=lambda data: self._write_relay_connect(
                relay_number,
                data))
        self._add_channel(new_channel)
        new_channel.set_write_delay(0.2)
    ...
    def add_channel_relay_range_H(self, channel_name, relay_number):
        ...
        if self.relay_pins_range_H[relay_number] in self.initialized_pins:
            raise Exception(
                f'relay range_H{relay_number} already used in another channel!')
        new_channel = integer_channel(
            channel_name,
            size=1,
            write_function=lambda data: self._write_relay_range_H(
                relay_number,
                data))
        self._add_channel(new_channel)
        new_channel.set_write_delay(0.2)
    ...
    def add_channel_relay_range_M_LB(self, channel_name, relay_number):
        ...
        if self.relay_pins_range_M_LB[relay_number] in self.initialized_pins:
            raise Exception(
                f'relay range_M_LB{relay_number} already used in another channel!')
        new_channel = integer_channel(
            channel_name,
            size=1,
            write_function=lambda data: self._write_relay_range_M_LB(
                relay_number,
                data))
        self._add_channel(new_channel)
        new_channel.set_write_delay(0.2)
    ...
    def add_channel_range(self, channel_name, channel_number):
        ...
        new_channel = channel(
            channel_name,
            write_function=lambda data: self._set_range(
                channel_number,
                data))
        new_channel.add_preset(
            "OPEN", f"Open all the relays of channel {channel_number}.")
        new_channel.add_preset(
            "5mA", f"Set channel {channel_number} to 5mA Range.")
        new_channel.add_preset(
            "500mA", f"Set channel {channel_number} to 500mA Range.")
        new_channel.add_preset(
            "10A", f"Set channel {channel_number} to 10A Range.")
    ...
    def add_channel_isense_remapper(self, channel_name, channel_number, vmeter_high_range_channel,
        ...
        readback_channel.set_attribute(
            'vmeter_high_range_channel',
            vmeter_high_range_channel)
        readback_channel.set_attribute(
            'vmeter_med_range_channel',
            vmeter_med_range_channel)
        readback_channel.set_attribute(
            'vmeter_low_range_channel',
            vmeter_low_range_channel)
        readback_channel.set_description(
    ...
    def _set_range(self, relay, irange):
        ...
        if relay not in range(1, 8 + 1):
            raise Exception(
                f'ConfiguratorXT: Invalid relay number:{relay}. Needs to be 1-8.')
        if irange.upper() == "10A":
            # Redundant path for high current mode - technically not needed
            self._write_relay_bypass(relay_number=relay, value=True)
            # Wraps the sense line around the sense resistors
            self._write_relay_connect(relay_number=relay, value=True)
            # Connects the force line (officially)
            self._write_relay_range_H(relay_number=relay, value=True)
            # This is a don't care in this mode
            self._write_relay_range_M_LB(relay_number=relay, value=False)
        elif irange.upper() == "500MA":
            # Keep the conducting path to prevent bad things
            self._write_relay_bypass(relay_number=relay, value=True)
            # Wraps the sense line around the sense resistors
            self._write_relay_connect(relay_number=relay, value=True)
            # Selects the path that allows the two low ranges
            self._write_relay_range_H(relay_number=relay, value=False)
            # Sets the medium range relay for medium mode
            self._write_relay_range_M_LB(relay_number=relay, value=False)
            # Remove the bypass line that prevented glitches in the previous
            # changes
            self._write_relay_bypass(relay_number=relay, value=False)
            ...
            # Selects the path that allows the two low ranges
            self._write_relay_range_H(relay_number=relay, value=False)
            # Sets the low range relay for low mode
            self._write_relay_range_M_LB(relay_number=relay, value=True)
            # Remove the bypass line that prevented glitches in the previous
            # changes
            self._write_relay_bypass(relay_number=relay, value=False)
        elif irange.upper() == "OPEN":
            # self._write_relay_bypass(relay_number=relay, value=True)        #
            # Maybe read state and transition through bypass if anybody else is
            # on
            # Wraps the sense line around the sense resistors
            self._write_relay_connect(relay_number=relay, value=False)
            # Selects the path that allows the two low ranges
            self._write_relay_range_H(relay_number=relay, value=False)
            self._write_relay_range_M_LB(
                relay_number=relay, value=False)     # Don't care
            # Do this last to preven blowing up low current channel if current
            # flow
            self._write_relay_bypass(relay_number=relay, value=False)
        else:
            raise Exception(
                f'ConfiguratorXT: Invalid range setting:{range}. Needs to be one of OPEN, 5mA, 500mA, 10A')
    ...
    def add_channel_gpio(self, channel_name, gpio_list,
        ...
        for gpio_pin in gpio_list:
            ...
            if self.gpio_pins[gpio_pin] in self.initialized_pins:
                ...
                raise Exception(
                    f'gpio number {gpio_pin} already used in another channel!')
        ...
        if output:
            if integer:
                new_channel = integer_channel(
                    name=channel_name,
                    size=len(gpio_list),
                    write_function=lambda value: self._write_gpio(
                        gpio_list,
                        value))
            else:
                new_channel = channel(
            ...
            if integer:
                new_channel = integer_channel(name=channel_name, size=len(
                    gpio_list), read_function=lambda: self._read_pins_values(gpio_list))
            else:
                new_channel = channel(
                    channel_name, read_function=lambda: self._read_pins_values(gpio_list))
            # Because you said it was an input
            self._write_gpio(gpio_list, "Z")
            # DJS TODO Z will need special handling for integer channels!
        return self._add_channel(new_channel)

    def _add_readback_channel(self, channel_name, gpio_list):
        new_channel = channel(
            f"{channel_name}_readback",
            read_function=lambda: self._read_pins_values(gpio_list))
        return self._add_channel(new_channel)
    ...
    def _write_pin(self, pin, value, tries=0, ret_str=None):
        ...
        if tries == self.tries:
            raise Exception(
                f"Failed to write pin {pin} to {value} after {tries} tries. Return value is {ret_str}.")
        if value not in [0, 1, 'z', 'Z', 'p', 'P', 'H', 'L']:
            raise Exception(
                f'HTX9011: Bad value requested for pin setting: {value}. Must be one of: [0,1,z,Z,p,P,H,L].')
        ...
        if not self._pin_response_valid(ret_str) or (
                ret_str[2] != str(value).upper()):
            self._write_pin(pin, value, tries + 1, ret_str)

    def _read_pin_setting(self, pin, tries=0, ret_str=None):
        if tries == self.tries:
            raise Exception(
                f"HTX9011: Failed to read pin: {pin} after {tries} tries. Return value is {ret_str}.")
    ...
    def _from_bit_list(self, bit_list):
        # attempt to build an integer from the bit list, otherwise return a
        # string
        out = 0
    ...
    def _write_relay_connect(self, relay_number, value):
        ...
        if value not in [0, 1]:
            raise Exception(
                f"Bad value for connect{relay_number} relay: {value}")
    ...
    def _write_relay_range_H(self, relay_number, value):
        ...
        if value not in [0, 1]:
            raise Exception(
                f'Bad value for range_H{relay_number} relay: {value}')
    ...
    def _write_relay_range_M_LB(self, relay_number, value):
        ...
        if value not in [0, 1]:
            raise Exception(
                f'Bad value for range_M_LB{relay_number} relay: {value}')
    ...
    def _read_pins_values(self, pins, invert=False):
        ...
        return self._read_pins_generic(
            pin_names, invert=invert, function=self._read_pin_value)
    ...
    def set_all_relays_bypass(self, value):
        ...
        Updates the all relays bypass in the object's internal state.
    ...
    def set_all_relays_connect(self, value):
        ...
        Updates the all relays connect in the object's internal state.
    ...
    def set_all_relays_range_H(self, value):
        ...
        Updates the all relays range H in the object's internal state.
    ...
    def set_all_relays_range_M_LB(self, value):
        ...
        Updates the all relays range M LB in the object's internal state.
    ...
    def _rip(self, write_list):
        ...
        for pin, value in write_list:
            ...
            # set the pin and read back its state to make sure there were no
    ...
    def add_channel_pwm(self, channel_name, pin):
        ...
        self._add_channel_pwm_freq_readback(
            channel_name + "_freq_readback", pin)
    ...
    def _add_channel_pwm_freq_readback(self, channel_name, pin):
        ...
        new_channel = channel(channel_name,
                              read_function=lambda: compute_f(pin))
        self._add_channel(new_channel)
    ...
    def _update_pwm_channel(self, pin):
        ...
        if self.pwm_enable[pin] == 1:
            self.get_interface().write(
                f'PWM:PREScale {int(self.prescale[pin])}')
            self.get_interface().write(f'PWM:TOP {self.top[pin]}')
            self.get_interface().write(
                f'PWM:COMPare ({self.pwm_pins[pin]},{compare})')
            self.get_interface().write(
                f'PWM:MODE ({self.pwm_pins[pin]},CLEAR)')
        else:
            self.get_interface().write(
    ...
    def add_channel_servo_enable(self, channel_name, servo_number):
        ...
        new_channel = channel(
            channel_name,
            write_function=lambda value: self._write_servo_enable(
                servo_number,
                value))
        return self._add_channel(new_channel)
    ...
    def add_channel_interrupt(self, channel_name, interrupt_number):
        ...
        def _read_interrupt_count(ch):
            ...
            ch.set_attribute(
                ...
                new_count +
                ch.get_attribute('INTERRUPT_COUNT_ACCUM'))
        ...
        control_channel._write = lambda value: _write_interrupt_control_channel(
            control_channel, value)  # self-reference
        control_channel.add_preset(
            'DISABLE', f"Disable interrupt {channel_name}.")
        control_channel.add_preset(
            'ANYEDGE', f"Set interrupt {channel_name} to count any detected edge.")
        control_channel.add_preset(
            'RISING', f"Set interrupt {channel_name} to detect all rising edges.")
        control_channel.add_preset(
            'FALLING', f"Set interrupt {channel_name} to detect all falling edges.")
        control_channel.set_attribute('INTERRUPT_NUMBER', interrupt_number)
        self._add_channel(control_channel)
        readback_channel = channel(
            name=f"{channel_name}_count",
            read_function=lambda: None)  # temporary
        readback_channel._read = lambda: _read_interrupt_count(
            readback_channel)  # self-reference
        readback_channel._write = lambda v: _write_interrupt_count(
            readback_channel, v)  # self-reference
    ...
    def add_channel_pcint(self, channel_name, pcint_number):
        ...
        control_channel.add_preset(
            'ANYEDGE',
            f"Set interrupt {channel_name} to count all rising and falling edges.")
        control_channel.set_attribute('INTERRUPT_NUMBER', pcint_number)
        self._add_channel(control_channel)
        readback_channel = channel(
            name=f'{channel_name}_count',
            read_function=lambda: None)
        readback_channel._read = lambda: _read_pcint_count(
            readback_channel)  # self-reference
        readback_channel._write = lambda v: _write_pcint_count(
            readback_channel, v)  # self-reference
        readback_channel.set_write_access(True)
        readback_channel.set_attribute('INTERRUPT_NUMBER', pcint_number)
        self._add_channel(readback_channel)
        capture_channel = channel(
            name=f"{channel_name}_capture_control",
            write_function=lambda: None)  # temporary
        ...
        capture_status_channel = channel(
            name=f'{channel_name}_capture_status',
            read_function=lambda: None)
    ...
    def _write_servo_enable(self, servo_number, value):
        ...
        if value:
            ...
            self.get_interface().write(
                f'PWM:COMPare ({self.pwm_pins[servo_number]},3000)')
            self.get_interface().write(
                f'PWM:MODE ({self.pwm_pins[servo_number]},CLEAR)')
        else:
            self.get_interface().write(
                f'PWM:MODE ({self.pwm_pins[servo_number]},DISABLE)')
    ...
    def get_system_version(self):
        ...
        Sends the ``SYSTem:VERSion`` SCPI command to the instrument.
        Returns the stored system version from the object's internal state.
    ...
    def get_firmware_version(self):
        ...
        Queries the instrument for its current firmware version and returns
        the parsed response.
        Queries the instrument for its current firmware version and returns
        the parsed response.
    ...
    def is_calibrateable(self):
        ...
        Returns:
            True if the calibrateable condition is met, False otherwise.
        """
    ...
    def set_serial_number(self, serialnum):
        ...
        Args:
            serialnum: Serialnum to use.
        """
        self.get_interface().write(f"CALibration:BOArdrev {serialnum}")

    def get_serialnum(self):
        """Return the current serialnum.
        Sends the ``CALibration:BOArdrev`` SCPI command to the instrument.
        Returns the stored serialnum from the object's internal state.

        Sends the corresponding SCPI command string to the instrument over the bus.

        Returns:
            The current serialnum.
        """
    ...
    def valid_serialnum(self):
        ...
        Checks that the input meets the expected constraints before proceeding.

        Returns:
            The valid serialnum result.
        """
        serialnum = self.get_serialnum()
        return len(serialnum) == 10 and serialnum.isdigit()

    def get_idn(self):
        """Return the current idn.
        Queries the instrument for its current idn and returns the parsed
        response.
        Returns the stored idn from the object's internal state.
    ...
    def get_error(self):
        ...
        Sends the ``SYStem:ERRor`` SCPI command to the instrument.
        Returns the stored error from the object's internal state.
    ...
    def set_resistor_calibration(self, resistor_number, value):
        ...
        Applies correction factors derived from known reference values to
        improve measurement accuracy.
        Sends the ``CALibration:DATA`` SCPI command to the instrument.
        ...
        # Memory location allows up to 16 digits.
        self.get_interface().write(
    ...
    def get_resistor_calibration(self, resistor_number):
        ...
        Sends the ``CALibration:DATA`` SCPI command to the instrument.
        Returns the stored resistor calibration from the object's internal
        state.
    ...
    def get_calibration_date(self):
        ...
        Applies correction factors derived from known reference values to
        improve measurement accuracy.
        Sends the ``CALibration:DATE`` SCPI command to the instrument.
    ...
    def check_calibration_valid(self, calibrating):
        ...
        if days is None and not calibrating:
            raise Exception(
                "Current sense resistor calibration date invalid.  Was board ever calibrated?")
...
class PCF8574_on_ConfiguratorXT(instrument):
    ...
    def add_readback_channel_covering_all_pins(self, channel_name):
        ...
        Creates and registers a new readback channel covering all pins.

        Appends a new readback channel covering all pins entry to the object's internal collection.
    ...
    def add_one_channel_per_pin(self, channel_names=None):
        ...
        # i.e. 8 for the PCF8574 on the ConfiguratorXT
        assert len(channel_names) == len(self.valid_pin_names)
        for pin_name, channel_name in zip(self.valid_pin_names, channel_names):
            new_channel = channel(
                channel_name,
                write_function=self.make_write_function(pin_name))
            new_channel.set_description(
                f"Write PCF8574 GPIO expander pin {pin_name} to logic low or high")
            self._add_channel(new_channel)
    ...
    def add_one_readback_channel_per_pin(self, channel_names=None):
        ...
        if channel_names is None:
            channel_names = [
                f"{pin_name}_readback" for pin_name in self.valid_pin_names]
        assert isinstance(channel_names, list)
        # i.e. 8 for the PCF8574 on the ConfiguratorXT
        assert len(channel_names) == len(self.valid_pin_names)
        for pin_name, channel_name in zip(self.valid_pin_names, channel_names):
            ...
            new_channel.set_description(
                f"Read logic state of ConfiguratorXT GPIO expander pin {pin_name}")
            self._add_channel(new_channel)
    ...
    def write_port(self, data8):
        ...
        Sends the ``GPIOX:WRI`` SCPI command to the instrument.
    ...
    def make_write_function(self, pin_name):
        ...
        Supports the ``PCF8574_on_ConfiguratorXT`` workflow by performing the described operation.

        Args:
            pin_name: Pin name to use.

        Returns:
            The make write function result.
        """
    ...
    def set_pins(self, pin_list):
        ...
        Sends the ``GPIOX:SETP:H`` SCPI command to the instrument.
        Sends the appropriate SCPI command to configure the instrument's pins.

        Sends the corresponding SCPI command string to the instrument over the bus.

        Args:
            pin_list: Pin list to use.
        """
    ...
    def clear_pins(self, pin_list):
        ...
        Sends the ``GPIOX:SETP:L`` SCPI command to the instrument.
        Removes all entries from the pins.
    ...
    def validate_pin_list(self, pin_list):
        ...
        Validates the pin list and raises an exception if invalid.
