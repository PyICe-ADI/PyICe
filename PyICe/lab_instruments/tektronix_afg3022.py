...
>>> from PyICe.lab_instruments.tektronix_afg3022 import tektronix_afg3022
...
class tektronix_afg3022(scpi_instrument):
    ...
    def add_channel_burst(self, channel_name, channel_number):
        ...
        self.add_channel_burstwave_pulse_hold(
            channel_name + "_pulse_hold", channel_number)
    ...
    def add_channel_continuous(self, channel_name, channel_number):
        ...
        self.add_channel_continuouswave_period(
            channel_name + "_period", channel_number)
        self.add_channel_continuouswave_pulse_hold(
            channel_name + "_pulse_hold", channel_number)
    ...
    def add_generic_channels(self, channel_name, channel_number):
        ...
        enable_channel = self.add_channel_enable(
            channel_name + "_enable", channel_number)
    ...
    def add_channel_enable(self, channel_name, channel_number):
        ...
        def _write_output_enable(channel_number, value):
            ...
            if str(value).upper() not in ['ON', 'OFF']:
                # Best guess but at least no SCPI error.
                value = 'ON' if value else 'OFF'
    ...
    def add_channel_outputz(self, channel_name, channel_number):
        ...
        def _write_outputz(channel_number, value):
            ...
            self.low_voltage = self.get_interface().ask(
                f"SOURce{channel_number}:VOLTage:LEVel:IMMediate:LOW?")
            if value in ["INF", "INFinity",
                         "HZ"]:  # 'HZ' siglent_SDG1000X valid argument
                value = "INFinity"
            self.get_interface().write(
    ...
    def add_channel_burstwave_pulse_hold(self, channel_name, channel_number):
        ...
        new_channel = channel(
            channel_name,
            write_function=lambda value: _write_burstwave_pulse_hold(
                channel_number,
                value))
    ...
    def add_channel_burstwave_trigger(self, channel_name, channel_number):
        ...
        def _send_burstwave_trigger(channel_number, value):
            if value.upper() == "TRIGGER":
                ...
                raise Exception(
                    f"Valid value for Pulse trigger channel on channel #{channel_number} is TRIGGER, don't know what {value} is supposed to do.")
    ...
    def add_channel_continuouswave_pulse_hold(
        ...
        new_channel = channel(
            channel_name,
            write_function=lambda value: _write_continuouswave_pulse_hold(
                channel_number,
                value))
        new_channel.add_preset("WIDTh", "Hold Pulse Width")
        new_channel.add_preset("DUTY", "Hold Duty Cycle")
