...
class pxie_4081(instrument):  # noqa: F405
    ...
    def get_measurement(self):
        """ Read the measurement from the instrument.
            ...
            Returns:
                float: Measurement value. The unit depends on the meter
                    configuration.
        """
    ...
    def add_channel(self, channel_name):
        new_channel = channel(channel_name, read_function=lambda: self.get_measurement())  # noqa: F405
        self._add_channel(new_channel)
        return new_channel

    def add_channel_nplc(self, channel_name):
        new_channel = channel(f"{channel_name}_nplc", write_function=lambda value: self.set_nplc(value))  # noqa: F405
    ...
    def add_channel_digits(self, channel_name):
        new_channel = channel(f"{channel_name}_digits", write_function=lambda value: self.set_digits(value))  # noqa: F405
    ...
    def add_channel_range(self, channel_name):
        new_channel = channel(f"{channel_name}_range", write_function=lambda value: self.set_range(value))  # noqa: F405
    ...
    def add_channel_range_readback(self, channel_name):
        new_channel = channel(f"{channel_name}_range_readback", read_function=lambda: self.get_range())  # noqa: F405
        self._add_channel(new_channel)
        return new_channel
