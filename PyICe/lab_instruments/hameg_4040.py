...
class hameg_4040(scpi_instrument):
    ...
    def add_channel(self, channel_name, num, ilim=1, delay=0.5,
        ...
        if add_extended_channels:
            current_channel = self.add_channel_current(
                channel_name + "_ilim", num)
            enable_channel = self.add_channel_enable(
                channel_name + "_enable", num)
    ...
    def add_channel_ovp_status(self, channel_name, num):
        new_channel = integer_channel(
            channel_name,
            size=1,
            read_function=lambda: self._read_ovp_status(num))
        new_channel.set_attribute('hameg_number', num)
        return self._add_channel(new_channel)
    ...
    def add_channel_fuse_status(self, channel_name, num):
        new_channel = integer_channel(
            channel_name,
            size=1,
            read_function=lambda: self._read_fuse_status(num))
        new_channel.set_attribute('hameg_number', num)
        return self._add_channel(new_channel)
    ...
    def add_channel_enable(self, channel_name, num):
        new_channel = integer_channel(
            channel_name,
            size=1,
            write_function=lambda state: self._write_enable(
                num,
                state))
        new_channel.set_attribute('hameg_number', num)
        return self._add_channel(new_channel)
    ...
    def add_channel_fuse_enable(self, channel_name, num, fuse_delay=0):
        new_channel = integer_channel(
            channel_name,
            size=1,
            write_function=lambda state: self._write_fuse_enable(
                num,
                state,
                fuse_delay))
        new_channel.set_attribute('hameg_number', num)
    ...
    def add_channel_AWG(self, channel_name, num):
        ...
        pattern_channel = channel(
            channel_name + "_pattern",
            write_function=lambda pattern: self._set_arb_pattern(
                pattern,
                num))
        pattern_channel.set_attribute('hameg_number', num)
        reps_channel = channel(
            channel_name + "_reps",
            write_function=lambda arb_cycles: self._set_arb_reps(arb_cycles))
    ...
    def _set_arb_pattern(self, pattern, num):
        ...
        if len(pattern) > 128 * \
                3:  # 128 points times [voltage,current,time] sets
            raise ValueError(
    ...
    def _read_vsense(self, num):
        ...
        while (retry <= self.retries):
            try:
                ...
                time.sleep(self.hameg_suck_time)
                data = float(
                    self.get_interface().ask("MEASure:SCALar:VOLT:DC?"))
                time.sleep(self.hameg_suck_time)
                return data
            except Exception as e:
                this_error = e
    ...
    def _read_isense(self, num):
        ...
        while (retry <= self.retries):
            try:
                ...
                # needed for hw serial on fast linux
                time.sleep(self.hameg_suck_time)
                data = float(self.get_interface().ask(
