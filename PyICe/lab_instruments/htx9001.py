...
class htx9001(scpi_instrument):
    ...
    def add_channel_relay(self, channel_name, relay_number):
        ...
        new_channel = integer_channel(
            channel_name,
            size=1,
            write_function=lambda data: self._write_relay(
                relay_number,
                data))
    ...
    def add_channel_test_hook(self, channel_name, test_hook_number):
        ...
        if self.test_hook_pins[test_hook_number] in self.initialized_pins:
            raise Exception(
                f"test hook number {test_hook_number} already used in another channel!")
        new_channel = integer_channel(
            channel_name,
            size=1,
            write_function=lambda data: self._write_test_hook(
                test_hook_number,
                data))
        self._add_channel(new_channel)
    ...
    def add_channel_gpio(self, channel_name, gpio_list,
        ...
        for gpio_pin in gpio_list:
            ...
            if self.gpio_pins[gpio_pin] in self.initialized_pins:
                print(
                    f"HTX9001(A) Warning: Non GPIO pin {gpio_pin} being redefined as a GPIO pin.")
