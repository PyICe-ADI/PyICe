...
class agilent_35670a(scpi_instrument):
    ...
    def add_channel_noise(self, channel_name, channel_num=1,
        ...
        if res is None:
            res = [800, 800, 800, 800, 800]
        if count is None:
            count = [150, 600, 600, 600, 600]
        self.freqs = freqs
        self.res = res
        self.count = count
        self.progress_chars = ['-', '\\', '|', '/']
    ...
    def read_channel(self, channel_num):
        ...
        for ii in range(0, len(self.count)):
            ...
            self.get_interface().write(
                ("SENSe:FREQuency:STARt " + str(self.freqs[ii])))
            self.get_interface().write(
                ("SENSe:FREQuency:STOP " + str(self.freqs[ii + 1])))
            self.get_interface().write(
                ("SENSe:FREQuency:RESolution " + str(self.res[ii])))
            self.get_interface().write(
                ("SENSe:AVERage:COUNt " + str(self.count[ii])))
            ...
            while test != 0:
                ...
                time.sleep(1)
                print(
                    f"\r MEASURING {self.progress_chars[self.timer % len(self.progress_chars)]}", end=' ')
                sys.stdout.flush()
                # will only equal zero if averaging is complete
                test = int(status) & int(self.mask)
                self.timer += 1
            xdata = (
                self.get_interface().ask(
                    ("CALC" + str(channel_num) + ":X:DATA?")))
            ydata = (
                self.get_interface().ask(
                    ("CALC" + str(channel_num) + ":DATA?")))
            xdata = xdata.replace('+', '')
            xlist = xdata.split(',')
            ylist = ydata.split(',')
