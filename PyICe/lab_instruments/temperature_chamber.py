...
class temperature_chamber(instrument, metaclass=ABCMeta):
    ...
    def add_channel_temp(self, channel_name):
        ...
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_temp.__doc__)
    ...
    def add_channel_sense(self, channel_name):
        ...
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_sense.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_soak(self, channel_name):
        ...
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_soak.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_window(self, channel_name):
        ...
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_window.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_settle_time_limit(self, channel_name):
        ...
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_settle_time_limit.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_blocking(self, channel_name):
        ...
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_blocking.__doc__)
    ...
    def add_channel_enable(self, channel_name):
        ...
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_enable.__doc__)
    ...
    def _wait_settle(self):
        ...
        while (settled <= self.soak):
            ...
            # print f"\rSettling {settled}/{self.soak}s  Current
            # Temp:{temp_current}°C  Target
            # Temp:{self.setpoint}±{self.window/2.0}°C Total time this
            # setting:{self.time}s
            # {progress_chars[self.time%len(progress_chars)]}" #comma supresses
            # newline
            ...
            print(
                f"\rSettling {settled}/{
                    self.soak}s Current Temp:{
                    temp_current:3.1f}C Target Temp:{
                    self.setpoint:3.1f}+/-{
                    self.window / 2.0:3.1f}C Total time this setting:{
                        self.time:3d}s {
                            progress_chars[
                                self.time %
                                len(progress_chars)]}", end=' ')  # comma supresses newline
            ...
            if (float(temp_current) < window_upper) and (
                    float(temp_current) > window_lower):
                settled += 1
            else:
                settled = 0
                if (self.settle_time_limit is not None and self.time >
                        self.settle_time_limit):
                    self._enable(False)
                    print()
                    raise Exception(
                        f'Oven failed to settle to {
                            self.setpoint}C in {
                            self.time} seconds. Final Temp: {temp_current}C.\n Oven Disabled. Test aborted.')
            self.time += 1
