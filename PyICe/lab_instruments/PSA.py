...
class keysight_e4440a(scpi_SA):
    ...
    def __init__(self, interface_visa, minimum_frequency,
        ...
        self.flush_errors()
        if reset:
            # Skip for collection of manually configured sweep
            self.get_interface().write('*RST')
            # BASIC|CDMA|CDMA1XEV|CDMA2K|EDGEGSM|NADC|NFIGURE|PDC|PNOISE|SA|WCDMA|WLAN|DMODULATION|MRECEIVE|TDSCDMA|TDDEMOD|EMC
            self.get_interface().write(':INSTrument:SELect SA')
    ...
    def add_channel_system_preset(self, channel_name):
        ...
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_system_preset.__doc__)
    ...
    def _read_trace_data(self, trace_number):
        ...
        resp = self.get_interface().ask(
            f':TRACE:DATA? TRACE{trace_number}').split(',')
    ...
    def _compute_x_axis(self):
        ...
        if stop > start:
            freqs = numpy.linspace(
                ...
                stop=stop,
                num=point_count,
                endpoint=True)  # assume linear sweep
            assert len(
                freqs) == point_count, f'Swept-span point count error: {len(freqs)} vs. {point_count}.'
            return freqs
        elif start == stop:
            # Zero span
            ...
            # oscilloscope. See Application Note 150 for more information on
            ...
            # and VBW, so changing those parameters may change the sweep time.
            # When the analyzer has been set to
            ...
            times = numpy.linspace(
                ...
                endpoint=True)
            assert len(
                times) == point_count, f'Zero-span point count error: {len(times)} vs. {point_count}.'
            return times
        else:
            raise Exception(
    ...
    def add_channel_sweep_control(self, channel_name):
        ...
        return (start_channel, stop_channel, center_channel,
                span_channel, point_count_channel)
    ...
    def add_channel_sweep_type(self, channel_name):
        ...
        type_channel = channel(
            f'{channel_name}',
            write_function=lambda t: self.get_interface().write(
                f':SENSe:SWEep:TYPE {t}'))
    ...
    def add_channel_RBW(self, channel_name):
        ...
        set_channel._set_value(
            float(self.get_interface().ask(':SENSe:BANDwidth:RESolution?')))
        set_channel._read = lambda: int(
            float(self.get_interface().ask(':SENSe:BANDwidth:RESolution?')))
        set_channel.set_attribute('channel_type', 'x_control')
        set_channel.set_description(
            self.get_name() + ': ' + self.add_channel_RBW.__doc__)
    ...
    def add_channel_RBW_auto(self, channel_name):
        ...
        new_channel.set_attribute('channel_type', 'y_control')
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_RBW_auto.__doc__)
        return self._add_channel(new_channel)
...
determined as indicated in Table 2-1 on page 70'''
        ...
        set_channel._set_value(
            float(self.get_interface().ask(':SENSe:BANDwidth:VIDeo?')))
        set_channel._read = lambda: int(
            float(self.get_interface().ask(':SENSe:BANDwidth:VIDeo?')))
        ...
        set_channel.set_attribute('channel_type', 'x_control')
        set_channel.set_description(
            self.get_name() + ': ' + self.add_channel_VBW.__doc__)
        return self._add_channel(set_channel)
    ...
    def add_channel_VBW_auto(self, channel_name):
        ...
        auto_channel.set_description(
            self.get_name() + ': ' + self.add_channel_VBW_auto.__doc__)
        self._add_channel(auto_channel)

        ratio_channel = channel(
            name=f'{channel_name}_RBW_VBW_ratio',
            write_function=lambda r: self.get_interface().write(
                f':SENSe:BANDwidth:VIDeo:RATio {r}'))
        ratio_channel._read = lambda: float(
            self.get_interface().ask(':SENSe:BANDwidth:VIDeo:RATio?'))
        ratio_channel.set_attribute('channel_type', 'y_control')
        ratio_channel.set_description(
            self.get_name() + ': ' + self.add_channel_VBW_auto.__doc__)
        self._add_channel(ratio_channel)

        auto_ratio_channel = integer_channel(
        ...
        auto_ratio_channel.set_description(
            self.get_name() + ': ' + self.add_channel_VBW_auto.__doc__)
    ...
    def add_channel_average(self, channel_name):
        ...
        count_channel = channel(
            f'{channel_name}_count',
            write_function=_write_avg)
        ...
        count_channel.set_description(
            self.get_name() + ': ' + self.add_channel_average.__doc__)
        self._add_channel(count_channel)

        type_channel = channel(
            f'{channel_name}_type',
            write_function=lambda typ: self.get_interface().write(
                f':SENSe:AVERage:TYPE {typ}'))
...
If the detector has been manually selected, a # appears next to it.'''
        new_channel = channel(
            ...
            write_function=lambda det_type: self.get_interface().write(
                f':SENSe:DETector:FUNCtion {det_type}'))
        new_channel.set_attribute('channel_type', 'y_control')
        ...
        new_channel.add_preset(
            'RMS',
            'RMS (alias). The query returns a name that corresponds to the detector mode. The RMS selection is an alias which selects the Average detector and Power Averaging. Therefore, if RMS has been selected, the query will return the AVER string.')
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_detector.__doc__)
        return self._add_channel(new_channel)
        ...
        def _set_attenuator(value):
            if value in range(0, 72, 2):
                ...
                self.get_interface().write(
                    f':SENSe:POWer:RF:ATTenuation {value}')
            else:
                raise Exception(
                    f"\nPSA: Sorry, can't set attentuator level to {value}.\nValue must be a multiple of 2 between 0dB and 70dB inclusive.")
        self.attenuator_channel = channel(
            channel_name, write_function=_set_attenuator)
    ...
    def add_channel_attenuator_auto(self, channel_name):
        ...
        new_channel.set_attribute('channel_type', 'y_control')
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_attenuator_auto.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_max_power(self, channel_name):
        ...
        ************************************************************************************************'''
        ...
        self.max_power_ch = channel(
            channel_name,
            write_function=lambda value: self.attenuator_channel.write(
                attenuation_level(
                    pmax_of_freqs(),
                    value)))
...
mixing.'''
        ...
        self.preamp_channel.set_description(
            self.get_name() + ': ' + self.add_channel_preamp.__doc__)
    ...
    #
        ...
        # self.get_interface().write(f':CALCulate:MARKer{marker_number}:CPEak:STATe
        # ON') # OFF|ON|0|1 (or :CALCulate:MARKer[1]|2|3|4:MAXimum)
        ...
        # return (peak_position_channel, amplitude_channel,
        # search_position_channel, search_threshold_channel,
        # search_excursion_channel)
    ...
    def add_channel_max_marker(
        ...
        # marker.
        self.get_interface().write(
            f':CALCulate:MARKer{marker_number}:TRACe {trace_number}')
        # POSition|DELTa|BAND|SPAN|OFF
        self.get_interface().write(
            f':CALCulate:MARKer{marker_number}:MODE POSition')
        self.get_interface().write(
            # BPOWer|NOISe|OFF
            f':CALCulate:MARKer{marker_number}:FUNCtion OFF')

        def _single_abort_trigger_wait(run_mode):
            if run_mode == 'FIND':
                self.get_interface().write(
                    f':CALCulate:MARKer{marker_number}:MAX')
            elif run_mode == 'STDBY':
                pass
            else:
                raise Exception(
                    f'''PSA Max Marker Trigger, sorry don't know how to: {run_mode}. Try "FIND"?''')
        ...
        frequency_channel = channel(
            f'{channel_name}_frequency',
            read_function=lambda marker_number=marker_number: self.get_interface().ask(
                f':CALCulate:MARKer{marker_number}:X?'))
        frequency_channel.set_attribute('channel_type', 'x_data')
        frequency_channel.set_description(
            self.get_name() + ': ' + self.add_channel_max_marker.__doc__)
        self._add_channel(frequency_channel)

        amplitude_channel = channel(
            f'{channel_name}_amplitude',
            read_function=lambda marker_number=marker_number: float(
                self.get_interface().ask(
                    f':CALCulate:MARKer{marker_number}:Y?')))
        amplitude_channel.set_attribute('channel_type', 'y_data')
        amplitude_channel.set_description(
            self.get_name() + ': ' + self.add_channel_max_marker.__doc__)
        self._add_channel(amplitude_channel)

        return (trigger_channel, frequency_channel, amplitude_channel)

    def add_channel_noise_marker(
            self, channel_name, marker_number=1, trace_number=1):
        '''spot measurmeent scalar at given frequency
        ...
        self.get_interface().write(
            # BPOWer|NOISe|OFF
            f':CALCulate:MARKer{marker_number}:FUNCtion NOISe')
        position_channel = channel(
            f'{channel_name}_frequency',
            write_function=lambda freq,
            marker_number=marker_number: self.get_interface().write(
                f':CALCulate:MARKer{marker_number}:X {freq}'))
        # todo - readback in case somebody touches front panel???
        position_channel.set_attribute('channel_type', 'meas_control')
        position_channel.set_description(
            self.get_name() + ': ' + self.add_channel_noise_marker.__doc__)
    ...
    def add_channel_trigger(self, channel_name):
        ...
        source_channel = channel(f'{channel_name}_source', write_function=lambda s: self.get_interface(
            # IMMediate|VIDeo|LINE|EXTernal[1]|EXTernal2|RFBurst
        ).write(':TRIGger:SEQuence:SOURce {s}'))
        ...
        source_channel.add_preset(
            'LINE', 'Line – Triggers on the power line signal')
        source_channel.add_preset(
            'EXTernal1',
            'External Front – Enables you to trigger on an externally connected trigger source')
        source_channel.add_preset(
            'EXTernal2',
            'External Rear – Enables you to trigger on an externally connected trigger source')
        source_channel.add_preset(
            'RFBurst',
        ...
        # Remote Command Notes: Other trigger-related commands are found in the
        ...
        level_channel.set_description(
            self.get_name() + ': ' + self.add_channel_trigger.__doc__)
        self._add_channel(level_channel)

        slope_channel = channel(
            f'{channel_name}_slope',
            write_function=lambda s: self.get_interface().write(':TRIGger:SEQuence:SLOPe {s}'))
    ...
    def add_channel_sweep_time(self, channel_name):
        ...
        force_channel = channel(
            f'{channel_name}',
            write_function=_set_sweep_time)
        force_channel.set_attribute('channel_type', 'meas_control')
        force_channel._set_value(
            float(self.get_interface().ask(':SENSe:SWEep:TIME?')))
        force_channel.set_description(
            self.get_name() + ': ' + self.add_channel_sweep_time.__doc__)
        force_channel._read = lambda: int(
            float(self.get_interface().ask(':SENSe:SWEep:TIME?')))
    ...
    def add_channel_sweep_time_auto(self, channel_name):
        ...
        new_channel.set_attribute('channel_type', 'y_control')
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_sweep_time_auto.__doc__)
        return self._add_channel(new_channel)
...
'''
        reference_level_channel = channel(
            ...
            write_function=lambda ampl: self.get_interface().write(
                f':DISPlay:WINDow1:TRACe:Y:SCALe:RLEVel {ampl}'))
        ...
        reference_level_channel._read = lambda: float(
            self.get_interface().ask(':DISPlay:WINDow1:TRACe:Y:SCALe:RLEVel?'))
        reference_level_channel.set_description(
            self.get_name() + ': ' + self.add_channel_y_disp.__doc__)
        self._add_channel(reference_level_channel)

        units_channel = channel(
            f'{channel_name}_units',
            write_function=lambda unit: self.get_interface().write(
        ...
        units_channel.set_description(
            self.get_name() + ': ' + self.add_channel_y_disp.__doc__)
        self._add_channel(units_channel)

        spacing_channel = channel(
            f'{channel_name}_spacing',
            write_function=lambda spac: self.get_interface().write(
                f':DISP:WIND:TRAC:Y:SPAC {spac}'))
        spacing_channel.set_attribute('channel_type', 'y_disp')
        spacing_channel._set_value(
            self.get_interface().ask(':DISP:WIND:TRAC:Y:SPAC?'))
        ...
        spacing_channel.set_description(
            self.get_name() + ': ' + self.add_channel_y_disp.__doc__)
        self._add_channel(spacing_channel)

        scale_channel = channel(
            f'{channel_name}_scale',
            write_function=lambda power: self.get_interface().write(
                f':DISPlay:WINDow1:TRACe:Y:SCALe:PDIVision {power}'))
        ...
        scale_channel._read = lambda: float(
            self.get_interface().ask(':DISPlay:WINDow1:TRACe:Y:SCALe:PDIVision?'))
        scale_channel.set_description(
            self.get_name() + ': ' + self.add_channel_y_disp.__doc__)
        self._add_channel(scale_channel)

        offset_channel = channel(
            f'{channel_name}_reference_level_offset',
            write_function=lambda dB: self.get_interface().write(
                f':DISPlay:WINDow1:TRACe:Y:SCALe:RLEVel:OFFSet {dB}'))
        offset_channel.set_attribute('channel_type', 'y_disp')
        offset_channel.add_preset(0)
        offset_channel.add_preset(20)  # 450 ohm divider
        offset_channel._read = lambda: float(self.get_interface().ask(
        ...
        offset_channel.set_description(
            f'{self.get_name()}: {self.add_channel_y_disp.__doc__}\n\n{offset_doc_str}')
        ...
        return (reference_level_channel, units_channel,
                spacing_channel, scale_channel, offset_channel)
...
+30 dBm'''
        ...
        coupling_channel._set_value(
            self.get_interface().ask(':INPut:COUPling?'))
        coupling_channel.add_preset('AC')
        coupling_channel.add_preset('DC')
        coupling_channel.set_description(
            self.get_name() + ': ' + self.add_channel_coupling.__doc__)
    ...
    def add_channel_1st_IF_overload(self, channel_name):
        ...
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_1st_IF_overload.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_final_IF_overload(self, channel_name):
        ...
        new_channel._set_value(get_status())
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_final_IF_overload.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_measurement_uncalibrated(self, channel_name):
        ...
        new_channel._set_value(get_status())
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_measurement_uncalibrated.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_calibration_error(self, channel_name):
        ...
        new_channel._set_value(get_status())
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_calibration_error.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_questionable(self, channel_name):
        ...
        new_channel._set_value(get_status())
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_questionable.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_fullscreen(self, channel_name):
        ...
        new_channel.set_attribute('channel_type', 'y_control')
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_fullscreen.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channel_syst_error(self, channel_name):
        ...
        new_channel.set_attribute('channel_type', 'y_control')
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_syst_error.__doc__)
        return self._add_channel(new_channel)
    ...
    def add_channels(self, channel_name):
        ...
        channels.append(
            self.add_channel_ydata(
                f'{channel_name}_ypoints',
                trace_number=1))
        # _start, _stop, _center, _span, _point_count
        channels.extend(self.add_channel_sweep_control(f'{channel_name}'))
        channels.extend(
            self.add_channel_sweep_type(
                f'{channel_name}_sweep_type'))
        # _reference_level, _units, _spacing, _scale, _reference_level_offset
        ...
        channels.append(
            self.add_channel_attenuator(
                f'{channel_name}_attenuator'))
        channels.append(
            self.add_channel_attenuator_auto(
                f'{channel_name}_attenuator_auto'))
        channels.append(
            self.add_channel_max_power(
                f'{channel_name}_max_power'))
        channels.append(self.add_channel_preamp(f'{channel_name}_preamp'))
        # _mode, _source, _video_level, _slope
        channels.extend(self.add_channel_trigger(f'{channel_name}_trigger'))
        channels.append(
            self.add_channel_sweep_time(
                f'{channel_name}_sweep_time'))
        channels.append(
            self.add_channel_sweep_time_auto(
                f'{channel_name}_sweep_time_auto'))
        ...
        channels.append(
            self.add_channel_1st_IF_overload(
                f'{channel_name}_input_overload'))
        channels.append(
            self.add_channel_final_IF_overload(
                f'{channel_name}_final_overload'))
        channels.append(
            self.add_channel_calibration_error(
                f'{channel_name}_uncalibrated'))
        channels.append(
            self.add_channel_fullscreen(
                f'{channel_name}_fullscreen'))
        channels.append(
            self.add_channel_syst_error(
                f'{channel_name}_syst_error'))
        #
        # TODO:
    ...
    # On receiving the :INITiate[:IMMediate] command, it will go through a
    ...
    # :CALCulate:MARKer[1]|2|3|4:STATe OFF|ON|0|1
    # :CALCulate:MARKer[1]|2|3|4:STATe?
    # Sets or queries the state of a marker. Setting a marker to state ON or 1
    # selects that marker. Setting a marker which is OFF to state ON or 1 puts
    # it in Normal mode and places it at the center of the display. Setting a
    # marker to state OFF or 0 selects that marker and turns it off. The
    # response to the query will be 0 if OFF, 1 if ON.
    ...
    # choices of detector and Avg/VBW type will usually cause measurement inaccuracy. The active marker
    # pair indicate the edges of the band. Only Delta Pair and Span Pair marker control modes can be used
    # while in this function, selecting any other mode (for example, Normal or
    # Delta) turns off this function

    # amplitude units :UNIT:POWer
    # DBM|DBMV|DBMA|V|W|A|DBUV|DBUA|DBUVM|DBUAM|DBPT|DBG  :UNIT:POWer?
