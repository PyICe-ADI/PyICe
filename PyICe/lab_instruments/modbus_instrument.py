...
register_description = collections.namedtuple('Register_description',
                                              ('name',
                                               ...
                                               'writeable',
                                               'reg_type',
                                               # (bit, register, long, float, string)
                                               'signed',
                                               'number_of_decimals',
                                               ...
                                               'documentation'),
                                              defaults=(
                                                  modbus_reg_type.register, None, None, None, None, None),
                                              )
...
class modbus_instrument(instrument, minimalmodbus.Instrument):
    ...
    def __init__(self, interface_raw_serial,
        ...
        instrument.__init__(
            self, f"Modbus instrument @ {interface_raw_serial}:{modbus_address}")
        minimalmodbus.Instrument.__init__(
            self,
            interface_raw_serial,
            modbus_address,
            mode=minimalmodbus.MODE_RTU if mode.lower() == 'rtu' else minimalmodbus.MODE_ASCII,
            debug=False)
    ...
    def _read_register(self, ch):
        return self.read_register(registeraddress=ch.get_attribute('address'),
                                  number_of_decimals=ch.get_attribute(
                                      'number_of_decimals'),
                                  functioncode=ch.get_attribute(
                                      'read_functioncode'),
        ...
        for reg_description in register_descriptions:
            if reg_description.reg_type == modbus_reg_type.bit:
                ch = register(name=reg_description.name,
                              ...
                              read_function=None,
                              # lambda addr=reg_description.address: self.read_bit(registeraddress=addr, functioncode=2) if reg_description.readable else None,
                              write_function=None,
                              # lambda v, addr=reg_description.address: self.write_bit(registeraddress=addr, value=v, functioncode=5) if reg_description.writeable else None,
                              )
                ...
                ch = modbus_register(name=reg_description.name,
                                     read_function=None,
                                     # lambda reg=reg_description: self._read_register(reg) if reg_description.readable else None,
                                     write_function=None,
                                     # lambda v, reg=reg_description: self._write_register(v, reg) if reg_description.writeable else None,
                                     )
            ...
            for attr in reg_description._fields:
                if attr == 'number_of_decimals' and getattr(
                        reg_description, attr) is None:
