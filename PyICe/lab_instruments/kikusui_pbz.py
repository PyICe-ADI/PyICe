...
class kikusui_pbz(scpi_instrument):
    ...
    def add_channel(self, channel_name, ilim=1, delay=0.5,
        ...
        if add_extended_channels:
            current_source_channel = self.add_channel_current_source(
                channel_name + "_ilim_source")
            current_source_channel.set_write_delay(delay)
            current_sink_channel = self.add_channel_current_sink(
                channel_name + "_ilim_sink")
            current_sink_channel.set_write_delay(delay)
            output_enable_channel = self.add_channel_output_enable(
