...
class semiconductor_parameter_analyzer(scpi_instrument):
    ...
    def _set_smu_voltage(self, smu_number, v_output=None,
        ...
        if v_output_range is not None:
            if isinstance(v_output_range,
                          str) and v_output_range.upper() == 'AUTO':
                self._smu_configuration[smu_number]['v_output_range'] = 0
            else:
                self._smu_configuration[smu_number]['v_output_range'] = self._lookup_output_range(
                    v_output_range, self._smu_voltage_range)
    ...
    def _set_smu_current(self, smu_number, i_output=None,
        ...
        if i_output_range is not None:
            if isinstance(i_output_range,
                          str) and i_output_range.upper() == 'AUTO':
                self._smu_configuration[smu_number]['i_output_range'] = 0
            else:
                self._smu_configuration[smu_number]['i_output_range'] = self._lookup_output_range(
                    i_output_range, self._smu_current_range)
        if 'i_output' in list(self._smu_configuration[smu_number].keys(
            ...
            print(
                f'SMU{smu_number} disabled.  Write both output_current and voltage_compliance channels to enable output')
            self._disable_smu(smu_number)

    def _disable_smu(self, smu_number):
        self.get_interface().write(
            (f"DV {self._smu_voltage_force_channels[smu_number]}"))
    ...
    def _lookup_output_range(self, max, range_dict):
        ...
        print(
            f'range select chose +/-{ranges[index]}:{range_dict[ranges[index]]} to match input of {max}')
    ...
    def _set_integration_time(self, time):
        if (isinstance(time, str) and time.upper() == "SHORT") or time == 1:
            ...
            raise Exception(
                'Valid integration times are "SHORT", "MEDIUM" or "LONG"')

    def _add_channels_smu_voltage(
            self, smu_number, voltage_force_channel_name, current_compliance_channel_name):
        voltage_force_channel = channel(
            voltage_force_channel_name,
            write_function=lambda output: self._set_smu_voltage(
                smu_number,
                v_output=output))
        voltage_force_channel.set_attribute('type', 'voltage_force')
        current_compliance_channel = channel(
            current_compliance_channel_name,
            write_function=lambda compliance: self._set_smu_voltage(
                smu_number,
    ...
    def _add_channel_smu_voltage_output_range(
            self, smu_number, output_range_channel_name):
        output_range_channel = channel(
            output_range_channel_name,
            write_function=lambda output_range: self._set_smu_voltage(
                smu_number,
                v_output_range=output_range))
        output_range_channel.set_attribute('type', 'voltage_range')
        self._add_channel(output_range_channel)
        return output_range_channel

    def _add_channels_smu_current(
            self, smu_number, current_force_channel_name, voltage_compliance_channel_name):
        current_force_channel = channel(
            current_force_channel_name,
            write_function=lambda output: self._set_smu_current(
                smu_number,
                i_output=output))
        current_force_channel.set_attribute('type', 'current_force')
        voltage_compliance_channel = channel(
            voltage_compliance_channel_name,
            write_function=lambda compliance: self._set_smu_current(
                smu_number,
    ...
    def _add_channel_smu_current_output_range(
            self, smu_number, output_range_channel_name):
        output_range_channel = channel(
            output_range_channel_name,
            write_function=lambda output_range: self._set_smu_current(
                smu_number,
                i_output_range=output_range))
        output_range_channel.set_attribute('type', 'current_range')
        self._add_channel(output_range_channel)
        return output_range_channel

    def _add_channel_smu_voltage_sense(
            self, smu_number, voltage_sense_channel_name):
        voltage_sense_channel = channel(
            voltage_sense_channel_name,
            read_function=lambda: self._read_voltage(
                self._smu_voltage_measure_channels[smu_number]))
    ...
    def _add_channel_smu_current_sense(
            self, smu_number, current_sense_channel_name):
        current_sense_channel = channel(
            current_sense_channel_name,
            read_function=lambda: self._read_current(
                self._smu_current_measure_channels[smu_number]))
    ...
    def _add_channel_vsource(self, vsource_number, vsource_channel_name):
        vsource_channel = channel(
            vsource_channel_name,
            write_function=lambda output: self._set_vsource_voltage(
                vsource_number,
                output))
        # should be different from SMU vsource???
        vsource_channel.set_attribute('type', 'voltage_force')
        self._add_channel(vsource_channel)
        return vsource_channel

    def _add_channel_vmeter(self, vmeter_number, vmeter_channel_name):
        vmeter_channel = channel(
            vmeter_channel_name,
            read_function=lambda: self._read_voltage(
                self._vm_voltage_measure_channels[vmeter_number]))
