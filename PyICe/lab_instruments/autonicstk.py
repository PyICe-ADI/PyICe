...
class autonicstk(instrument):
    def __init__(self, interface_raw_serial, modbus_address):
        ...
        instrument.__init__(
            self, f"Autonics PID @ {interface_raw_serial}:{modbus_address}")
        ...
        self.modbus_pid = minimalmodbus.Instrument(
            interface_raw_serial, modbus_address)
    ...
    def _read_temperature_sense(self):
        return self.modbus_pid.read_register(
        ...
        new_register = modbus_register(channel_name,
                                       read_function=lambda: self.retry(
                                           lambda: self.modbus_pid.read_register(
                                               0,
                                               number_of_decimals=self.get_decimal(),
                                               functioncode=3,
                                               signed=True),
                                           retry_count=1),
    ...
    def add_channels_tuning(self, channel_name):
        ...
        new_register = modbus_register(f'{channel_name}_Heat_Proportional',
                                       read_function=lambda: self.modbus_pid.read_register(
                                           101, number_of_decimals=1, functioncode=3),
                                       write_function=lambda data: self.modbus_pid.write_register(101, float(data), number_of_decimals=1, functioncode=6))
        new_register.set_category('PID_tuning')
        new_register.set_description(
            'Reciprocal heat proportional gain. Number of degrees over which the heat output transistion from 0%-100% output due to proportional error alone.')
        ...
        new_register = modbus_register(f'{channel_name}_Cool_Proportional',
                                       read_function=lambda: self.modbus_pid.read_register(
                                           102, number_of_decimals=1, functioncode=3),
                                       write_function=lambda data: self.modbus_pid.write_register(102, float(data), number_of_decimals=1, functioncode=6))
        new_register.set_category('PID_tuning')
        new_register.set_description(
            'Reciprocal cool proportional gain. Number of degrees over which the heat output transistion from 100%-0% output due to proportional error alone.')
        self._add_channel(new_register)
        new_channels.append(new_register)
        ...
        new_register.set_category('PID_tuning')
        new_register.set_description(
            'Heat integral time. Number of seconds over which the (measured) process variable will change from proportial residual error to zero error.')
        self._add_channel(new_register)
        new_channels.append(new_register)
        ...
        new_register.set_category('PID_tuning')
        new_register.set_description(
            'Cool integral time. Number of seconds over which the (measured) process variable will change from proportial residual error to zero error.')
        self._add_channel(new_register)
        new_channels.append(new_register)
        ...
        new_register.set_category('PID_tuning')
        new_register.set_description(
            'Heat derivitive time. Scaling constant to map degrees/s (rate) into degrees of apparent error.')
        self._add_channel(new_register)
        new_channels.append(new_register)
        ...
        new_register.set_category('PID_tuning')
        new_register.set_description(
            'Cool derivitive time. Scaling constant to map degrees/s (rate) into degrees of apparent error.')
        self._add_channel(new_register)
        new_channels.append(new_register)
        new_register = modbus_register(f'{channel_name}_Heat_Hysteresis',
                                       read_function=lambda: self.modbus_pid.read_register(
                                           109, number_of_decimals=self.get_decimal(), functioncode=3),
        ...
        new_register.set_category('PID_tuning')
        new_register.set_description(
            'Heat hysteresis. Dead band for heat loop only. Not used with PID proportional output mode.')
        self._add_channel(new_register)
        new_channels.append(new_register)
        new_register = modbus_register(f'{channel_name}_Cool_Hysteresis',
                                       read_function=lambda: self.modbus_pid.read_register(
                                           111, number_of_decimals=self.get_decimal(), functioncode=3),
        ...
        new_register.set_category('PID_tuning')
        new_register.set_description(
            'Cool hysteresis. Dead band for cool loop only. Not used with PID proportional output mode.')
        self._add_channel(new_register)
        new_channels.append(new_register)
        new_register = modbus_register(f'{channel_name}_Deadband',
                                       read_function=lambda: self.modbus_pid.read_register(
            107, number_of_decimals=self.get_decimal(), functioncode=3, signed=True),
        ...
        new_register.set_description(
            'Dead band between heat and cool loops. Negative value causes class-A operation.')
        self._add_channel(new_register)
        new_channels.append(new_register)
        ...
        new_register.set_category('PID_tuning')
        new_register.set_description(
            "Heat PWM mode. 'Standard' uses 'Heating_Control_Time' setting. 'Cycle' powers/unpowers the heater over whole power line cycles. 'Phase' synchronously dims the heater by delaying turn-on during each power line cycle.")
        self._add_channel(new_register)
        new_channels.append(new_register)
        new_register = modbus_register(f'{channel_name}_Heating_Control_Time',
                                       read_function=lambda: self.modbus_pid.read_register(
                                           170, number_of_decimals=1, functioncode=3),
                                       write_function=lambda data: self.modbus_pid.write_register(170, float(data), number_of_decimals=1, functioncode=6))
        new_register.set_category('PID_tuning')
        new_register.set_description(
            "Heat control PWM period when in 'Standard' SSR1_Mode.")
        self._add_channel(new_register)
        new_channels.append(new_register)
        new_register = modbus_register(f'{channel_name}_Cooling_Control_Time',
                                       read_function=lambda: self.modbus_pid.read_register(
                                           171, number_of_decimals=1, functioncode=3),
    ...
    def add_channels_alarm_config(self, channel_name):
        ...
        new_register = modbus_register(f'{channel_name}_Alarm1_high_limit',
                                       read_function=lambda: self.modbus_pid.read_register(
            54, number_of_decimals=self.get_decimal(), functioncode=3, signed=True),
        ...
        new_register = modbus_register(f'{channel_name}_Alarm1_hysteresis',
                                       read_function=lambda: self.modbus_pid.read_register(
                                           202, number_of_decimals=self.get_decimal(), functioncode=3),
        ...
        new_register = modbus_register(f'{channel_name}_Alarm2_low_limit',
                                       read_function=lambda: self.modbus_pid.read_register(
            55, number_of_decimals=self.get_decimal(), functioncode=3, signed=True),
        ...
        new_register = modbus_register(f'{channel_name}_Alarm2_high_limit',
                                       read_function=lambda: self.modbus_pid.read_register(
            56, number_of_decimals=self.get_decimal(), functioncode=3, signed=True),
        ...
        new_register = modbus_register(f'{channel_name}_Alarm2_hysteresis',
                                       read_function=lambda: self.modbus_pid.read_register(
                                           208, number_of_decimals=self.get_decimal(), functioncode=3),
