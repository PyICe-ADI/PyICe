...
class tektronix_3054(scpi_instrument, delegator):
    ...
    def add_channel_dvm(self, channel_name,
        ...
        dvm_channel = channel(
            channel_name,
            read_function=lambda measurement_time=measurement_time: self._read_dvm_channel(
                scope_channel_number,
                measurement_time,
                mode))
        dvm_channel.set_delegator(self)
        # set up DMM configuration so that first reading isn't bogus with
        # measurement_time=0 (single channel usage)
        dvm_channel.read_without_delegator(measurement_time=0)
