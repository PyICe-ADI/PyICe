...
class pxie_4051(instrument):
    ...
    def set_current_level(self, value: float) -> None:
    ...
    def set_current_level_range(self, value) -> None:
        if value not in ['AUTO', 4, 40]:
            raise Exception(f'{Fore.CYAN}Not a valid current range setting, valid settings are 4, 40, AUTO{Fore.RESET}')
    ...
    def get_current_level_range(self) -> float:
        return float(self.session.current_level_range)

    def set_voltage_level_range(self, value) -> None:
        if value not in ['AUTO', 6, 60]:
            raise Exception(f'{Fore.CYAN}Not a valid voltage range setting, valid settings are 6, 60, AUTO{Fore.RESET}')
        ...
        if value == "AUTO":
            ...
            try:
                ...
                if e.code == -1074097882:  # Requested value is not a supported
                    ...
                    raise Exception(f"{Fore.LIGHTRED_EX}Error setting voltage level range: {e}{Fore.RESET}")
        self.init_session()
    ...
    def set_current_level_rise_slew_rate(self, value: float) -> None:
        # Default is 2.4A/microsecond
        self.session.abort()
        self.session.current_level_rising_slew_rate = value
        self.init_session()

    def get_current_level_rise_slew_rate(self) -> float:
        return self.session.current_level_rising_slew_rate

    def set_current_level_fall_slew_rate(self, value: float) -> None:
        # Default is 2.4A/microsecond
        self.session.abort()
        self.session.current_level_falling_slew_rate = value
        self.init_session()

    def get_current_level_fall_slew_rate(self) -> float:
    ...
    def add_channel_current_level(self, channel_name) -> channel:
        new_channel = channel(channel_name + "_current", write_function=lambda value: self.set_current_level(value))
        new_channel._write_max_value = 40.0
        new_channel._set_value(self.get_current_level())
        self._add_channel(new_channel)
        return new_channel

    def add_channel_current_level_range(self, channel_name) -> channel:
        new_channel = channel(channel_name + "_current_level_range", write_function=lambda value: self.set_current_level_range(value))
        new_channel.add_preset("AUTO", "")
        new_channel.add_preset(4.0, "")
        new_channel.add_preset(40.0, "")
        new_channel._set_value(self.get_current_level_range())
        self._add_channel(new_channel)
        return new_channel
    ...
    def add_channel_voltage_level_range(self, channel_name) -> channel:
        ...
        new_channel.add_preset("AUTO", "")
        # new_channel.add_preset(6.0, "") # Disabled the 6V range, is not supported on PXIe-4051, but showing on InstrumentStudio GUI.
    ...
    def add_channel_current_rise_slew_rate(self, channel_name) -> channel:
        new_channel = channel(channel_name + "_current_rise_slew_rate", write_function=lambda value: self.set_current_level_rise_slew_rate(value))
        self._add_channel(new_channel)
        return new_channel

    def add_channel_current_fall_slew_rate(self, channel_name) -> channel:
        new_channel = channel(channel_name + "_current_fall_slew_rate", write_function=lambda value: self.set_current_level_fall_slew_rate(value))
        self._add_channel(new_channel)
        return new_channel
