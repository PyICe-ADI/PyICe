...
class ADT7410(instrument):
    ...
    def enable(self, enabled=True):
        ...
        if enabled:
            ...
            self.twi.write_register(
                addr7=self.addr7,
                commandCode=self.registers['config'],
                data=(
                    0b1 << 7),
                data_size=8,
                use_pec=False)
        else:  # shutdown
            # self.twi.write_byte(addr7=self.addr7, self.registers['config'],
            # (0b1<<7 + 0b11<<5))
            self.twi.write_register(
                addr7=self.addr7,
                commandCode=self.registers['config'],
    ...
    def read_temp(self):
        ...
        data = self.twi.read_register(
            addr7=self.addr7,
            commandCode=self.registers['t_msb'],
            data_size=16,
            use_pec=False)
        # msb comes out first, not SMBus compatible!
        data = swap_endian(data, elementCount=2)
