...
>>> from PyICe.lab_instruments.chroma_63600 import chroma_63600
...
class chroma_63600(scpi_instrument):
    ...
    def add_channel_resistance(self, channel_name, num):
        ...
        new_channel = channel(
            ...
            write_function=lambda resistance: self._write_resistance(
                ...
                resistance))
        new_channel.set_attribute('chroma_number', num)
        # only achievable in H (high) mode
        new_channel.set_max_write_limit(12000)
        # only achievable in L (low) mode
        new_channel.set_min_write_limit(0.04)
        return self._add_channel(new_channel)
    ...
    def _read_measured_voltage(self, num):
        self._select_load_channel(num)
        # UUT sets it to the sense terminals
        self.get_interface().write(("MEAS:INP UUT"))
        # returns a string by default
