...
>>> from PyICe.lab_instruments.pyice_arduino_tool import pyice_arduino_tool
...
class pyice_arduino_tool(instrument):
    ...
    def __init__(self, interface, base_name='pat'):
        ...
        Args:
            ...
            interface: Communication interface object.
        """
    ...
    def init_channels(self, base_name):  # ADD BASENAME INPUT ARG
        ...
        Supports the ``pyice_arduino_tool`` workflow by performing the described operation.
        ...
        self.chip_register_size_channel = self._add_channel_chip_register_size(
            f'{base_name}_chip_register_size')
        self.chip_address_channel = self._add_channel_chip_address(
            f'{base_name}_chip_address')
        self.chip_watchdog_q_command_code_channel = self._add_channel_chip_watchdog_q_command_code(
            f'{base_name}_chip_wd_q_cc')
        self.chip_watchdog_a_command_code_channel = self._add_channel_chip_watchdog_a_command_code(
            f'{base_name}_chip_wd_a_cc')

        # control_settings
        self.control_command_code_channel = self._add_channel_control_command_code(
            f'{base_name}_control_command_code')
        self.control_command_write_data_channel = self._add_channel_control_command_write_data(
            f'{base_name}_control_command_write_data')
        self.control_command_read_data_channel = self._add_channel_control_command_read_data(
            f'{base_name}_control_command_read_data')
        self.control_pin_number_channel = self._add_channel_control_pin_number(
            f'{base_name}_control_pin_number')
        self.control_pin_edge_channel = self._add_channel_control_pin_edge(
            f'{base_name}_control_pin_edge')
        self.control_command_write_channel = self._add_channel_control_command_write(
            f'{base_name}_control_command_write')
        self.control_command_read_channel = self._add_channel_control_command_read(
            f'{base_name}_control_command_read')
        self.control_pin_write_channel = self._add_channel_control_pin_write(
            f'{base_name}_control_pin_write')
        self.control_pin_read_channel = self._add_channel_control_pin_read(
            f'{base_name}_control_pin_read')
        self.control_test_hook_enable = self._add_channel_control_test_hook_enable(
            f'{base_name}_control_test_hook_enable')

        # test_settings
        self.test_timeout_channel_ms = self._add_channel_test_timeout_ms(
            f'{base_name}_test_timeout_ms')
        self.test_arm_channel = self._add_channel_test_arm(
            f'{base_name}_test_arm')
        self.test_run_channel = self._add_channel_test_run(
            f'{base_name}_test_run')
        self.test_data_ready_channel = self._add_channel_test_data_ready(
            f'{base_name}_test_data_ready')
        self.test_data_channel = self._add_channel_test_data(
            f'{base_name}_test_data')

        # trigger_settings
        self.trigger_select_channel = self._add_channel_trigger_select(
            f'{base_name}_trigger_select')
        self.trigger_type_channel = self._add_channel_trigger_type(
            f'{base_name}_trigger_type')
        self.trigger_command_code_channel = self._add_channel_trigger_command_code(
            f'{base_name}_trigger_command_code')
        self.trigger_command_write_data_channel = self._add_channel_trigger_command_write_data(
            f'{base_name}_trigger_command_write_data')
        self.trigger_command_read_data_channel = self._add_channel_trigger_command_read_data(
            f'{base_name}_trigger_command_read_data')
        self.trigger_pin_number_channel = self._add_channel_trigger_pin_number(
            f'{base_name}_trigger_pin_number')
        self.trigger_pin_edge_channel = self._add_channel_trigger_pin_edge(
            f'{base_name}_trigger_pin_edge')

        # actions
        self.action_queue_channel = self._add_channel_queue_action(
            f'{base_name}_action_queue')
        self.action_run_all_channel = self._add_channel_run_all_actions(
            f'{base_name}_action_run_all')
        self.action_flush_channel = self._add_channel_flush_actions(
            f'{base_name}_action_flush')
        self.action_type_channel = self._add_channel_action_type(
            f'{base_name}_action_type')
        self.action_delay_channel_us = self._add_channel_action_delay_us(
            f'{base_name}_action_delay_us')
        self.action_select_channel = self._add_channel_action_select(
            f'{base_name}_action_select')
        self.action_timestamp_channel_us = self._add_channel_action_timestamp_us(
            f'{base_name}_action_timestamp_us')
        self.action_command_write_data_channel = self._add_channel_action_command_write_data(
            f'{base_name}_action_command_write_data')
        self.action_command_code_channel = self._add_channel_action_command_code(
            f'{base_name}_action_command_code')
        self.action_pin_number_channel = self._add_channel_action_pin_number(
            f'{base_name}_action_pin_number')
        self.action_pin_edge_channel = self._add_channel_action_pin_edge(
            f'{base_name}_action_pin_edge')

        # pwm
        self.pwm_gclk_channel = self._add_channel_pwm_gclk(
            f'{base_name}_pwm_gclk_div')
        self.pwm_turbo_channel = self._add_channel_pwm_turbo(
            f'{base_name}_pwm_turbo')
        self.pwm_enable_channel = self._add_channel_pwm_enable(
            f'{base_name}_pwm_enable')
        self.pwm_output_dutycycle_channel = self._add_channel_pwm_output_dutycycle(
            f'{base_name}_pwm_duty')
        self.pwm_output_pin_channel = self._add_channel_pwm_output_pin(
            f'{base_name}_pwm_output_pin')
        self.pwm_tcc_div_channel = self._add_channel_pwm_tcc_div(
            f'{base_name}_pwm_tcc_div')
        self.pwm_tcc_steps_channel = self._add_channel_pwm_tcc_steps(
            f'{base_name}_pwm_steps')
        self.pwm_mode_channel = self._add_channel_pwm_mode(
            f'{base_name}_pwm_mode')

        # pat_settings
        self.enable_board_channel = self._add_channel_enable_board(
            f'{base_name}_board_enable')

        # run_channel
        # self.run_channel = channel(f'{base_name}_run_test',write_function=lambda x :self.run_test(x))
        # self._add_channel(self.run_channel)
        self.run_channel = self._add_channel(
            channel(
                f'{base_name}_run_test',
                write_function=lambda x: self.run_test(x)))

        # second unit corrections
        self.test_timeout_channel = self._add_channel(
            channel(
                f'{base_name}_test_timeout',
                write_function=lambda x: self.test_timeout_channel_ms.write(
                    x * 1e3)))
        self.action_delay_channel = self._add_channel(
            channel(
                f'{base_name}_action_delay',
                write_function=lambda x: self.action_delay_channel_us.write(
                    x * 1e6)))
        self.action_timestamp_channel = self._add_channel(
            channel(
                f'{base_name}_action_timestamp',
                read_function=lambda: float(
                    self.action_timestamp_channel_us.read()) / 1e6))
    ...
    def reset_pins(self):
        ...
        """
        print(
            'YOU CAN OVERRIDE reset_pins(). Use this to set the default state for each pin.')
...
# ADD CHANNEL HELPERS ----------------------------------------------------
    ...
    def _add_read_channel(self, channel_name, command):
        ...
        Args:
            ...
            command: Command string to send to the instrument.

        Returns:
            The add read channel result.
        """
        ...
        # self.get_interface().ask(command))#.decode(str_encoding))
        # .decode(str_encoding))
        new_channel = channel(channel_name,
                              read_function=lambda: read_mumbo(command))
    ...
    def _add_read_write_channel_with_param(self, channel_name, command):
        ...
        Returns:
            The add read write channel with param result.
        """
        ...
        new_channel = channel(
            channel_name,
            write_function=lambda param: write_magic(
                f'{command} {param}'))
        new_channel._read = lambda: self.get_interface().ask(f'{command}?')
        # new_channel.set_description(self.get_name() + ': ' + self.add_channel.__doc__)
        return self._add_channel(new_channel)
    # We need a write-only channel, for the types that don't have a '?'
    # equivalent.
    ...
    def _add_write_channel(self, channel_name, command):
        ...
        new_channel = channel(
            channel_name,
            write_function=lambda param: write_jumbo(
                f'{command} {param}'))
        return self._add_channel(new_channel)
...
# CHIP_DATA CHANNEL DEFS -------------------------------------------------
    def _add_channel_chip_uses_pec(self, channel_name):
        ...
        Returns:
            The add channel chip uses pec result.
        """
    ...
    def _add_channel_chip_register_size(self, channel_name):
        ...
        Returns:
            The add channel chip register size result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_chip_address(self, channel_name):
        ...
        Returns:
            The add channel chip address result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_chip_watchdog_q_command_code(self, channel_name):
        ...
        Returns:
            The add channel chip watchdog q command code result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_chip_watchdog_a_command_code(self, channel_name):
        ...
        Returns:
            The add channel chip watchdog a command code result.
        """
        new_channel = self._add_read_write_channel_with_param(
...
# CONTROL_SETTINGS CHANNEL DEFS ------------------------------------------
    ...
    def _add_channel_control_command_code(self, channel_name):
        ...
        Returns:
            The add channel control command code result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_control_command_write_data(self, channel_name):
        ...
        Returns:
            The add channel control command write data result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_control_command_read_data(self, channel_name):
        ...
        Returns:
            The add channel control command read data result.
        """
    ...
    def _add_channel_control_command_write(self, channel_name):
        ...
        Returns:
            The add channel control command write result.
        """
    ...
    def _add_channel_control_command_read(self, channel_name):
        ...
        Returns:
            The add channel control command read result.
        """
        new_channel = self._add_read_channel(
    ...
    def _add_channel_control_pin_number(self, channel_name):
        ...
        Returns:
            The add channel control pin number result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_control_pin_edge(self, channel_name):
        ...
        Returns:
            The add channel control pin edge result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_control_pin_write(self, channel_name):
        ...
        Returns:
            The add channel control pin write result.
        """
        new_channel = self._add_write_channel(
    ...
    def _add_channel_control_pin_read(self, channel_name):
        ...
        Returns:
            The add channel control pin read result.
        """
    ...
    def _add_channel_control_test_hook_enable(self, channel_name):
        ...
        Returns:
            The add channel control test hook enable result.
        """
        new_channel = self._add_read_write_channel_with_param(
...
# TEST CHANNEL DEFS ------------------------------------------------------
    ...
    def _add_channel_test_timeout_ms(self, channel_name):
        ...
        Returns:
            The add channel test timeout ms result.
        """
    ...
    def _add_channel_test_arm(self, channel_name):
        ...
        Returns:
            The add channel test arm result.
        """
        new_channel = self._add_read_write_channel_with_param(
        ...
        new_channel.add_preset(
            'A->T1->T2',
            'Perform queued actions, then measure the time delta between the start and stop triggers.')
        new_channel.add_preset(
            'T1->A->T2',
            'Measure the time delta between the start and stop triggers, peforming actions after start trigger fires.')
        new_channel.add_preset(
            'A->T2?',
            'Enable a non-blocking stop trigger (pin input only) and run all actions. Reports back time between start of actions and stop trigger.')
        new_channel.add_preset(
            'T1->A->T2?',
            'Await start trigger, then enable a non-blocking stop trigger (pin input only) and run all actions. Reports back time between triggers.')
        return new_channel
    ...
    def _add_channel_test_run(self, channel_name):
        ...
        Returns:
            The add channel test run result.
        """
    ...
    def _add_channel_test_data_ready(self, channel_name):
        ...
        Returns:
            The add channel test data ready result.
        """
    ...
    def _add_channel_test_data(self, channel_name):
        ...
        Returns:
            The add channel test data result.
        """
        return self._add_read_write_channel_with_param(
...
# TRIGGER_SETTINGS CHANNEL DEFS ------------------------------------------
    ...
    def _add_channel_trigger_select(self, channel_name):
        ...
        Returns:
            The add channel trigger select result.
        """
        new_channel = self._add_read_write_channel_with_param(
            channel_name, 'TRIGger:SELect')
        new_channel.add_preset(
            'START', 'Select the trigger which begins the measurement time.')
        new_channel.add_preset(
            'STOP', 'Select the trigger which ends the measurement time.')
        return new_channel
    ...
    def _add_channel_trigger_type(self, channel_name):
        ...
        Returns:
            The add channel trigger type result.
        """
        new_channel = self._add_read_write_channel_with_param(
        ...
        new_channel.add_preset(
            'PIN_INPUT',
            'Wait for pin input to go to the value defined by the pin edge setting, then timestamp (interrupt-based).')
        new_channel.add_preset(
            'PIN_OUTPUT',
            'Set the selected pin to the pin edge setting and timestamp.')
        new_channel.add_preset(
            'WRITE', 'Write the write data value to the command code and timestamp.')
        new_channel.add_preset(
            'READ_MATCH',
            'Timestamp once the read data matches the value stored in write data.')
        new_channel.add_preset(
            'READ_MISMATCH',
            'Timestamp once the read data does not match the value stored in write data.')
        new_channel.add_preset(
            'SERVICE_WATCHDOG',
            'Service the watchdog and then timestamp.')
        new_channel.add_preset(
            'BAD_SERVICE_WATCHDOG',
            'Service the watchdog incorrectly (wrong CRC) and then timestamp.')
        return new_channel
    ...
    def _add_channel_trigger_command_code(self, channel_name):
        ...
        Returns:
            The add channel trigger command code result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_trigger_command_write_data(self, channel_name):
        ...
        Returns:
            The add channel trigger command write data result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_trigger_command_read_data(self, channel_name):
        ...
        Returns:
            The add channel trigger command read data result.
        """
        new_channel = self._add_read_channel(
    ...
    def _add_channel_trigger_pin_number(self, channel_name):
        ...
        Returns:
            The add channel trigger pin number result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_trigger_pin_edge(self, channel_name):
        ...
        Returns:
            The add channel trigger pin edge result.
        """
        new_channel = self._add_read_write_channel_with_param(
...
# ACTION CHANNEL DEFS ----------------------------------------------------
    ...
    def _add_channel_queue_action(self, channel_name):
        ...
        Returns:
            The add channel queue action result.
        """
    ...
    def _add_channel_run_all_actions(self, channel_name):
        ...
        Returns:
            The add channel run all actions result.
        """
    ...
    def _add_channel_flush_actions(self, channel_name):
        ...
        Returns:
            The add channel flush actions result.
        """
    ...
    def _add_channel_action_type(self, channel_name):
        ...
        Returns:
            The add channel action type result.
        """
        new_channel = self._add_read_write_channel_with_param(
            channel_name, 'ACTIon:TYPE')
        new_channel.add_preset('NONE', 'No action, just a dummy placeholder.')
        new_channel.add_preset(
            'WAIT', 'Wait for the specified action delay in microseconds.')
        new_channel.add_preset('PIN_OUTPUT',
                               'Set the selected pin to the pin edge setting.')
        new_channel.add_preset(
            'WRITE', 'Write the write data value to the command code.')
        new_channel.add_preset('SERVICE_WATCHDOG', 'Service the watchdog.')
        new_channel.add_preset('BAD_SERVICE_WATCHDOG',
                               'Service the watchdog incorrectly (wrong CRC).')
        return new_channel
    ...
    def _add_channel_action_delay_us(self, channel_name):
        ...
        Returns:
            The add channel action delay us result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_action_select(self, channel_name):
        ...
        Returns:
            The add channel action select result.
        """
        new_channel = self._add_read_write_channel_with_param(
        ...
        new_channel.add_preset(
            'NEXT', 'Select the next action, not in queue yet.')
        return new_channel
    ...
    def _add_channel_action_command_code(self, channel_name):
        ...
        Returns:
            The add channel action command code result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_action_command_write_data(self, channel_name):
        ...
        Returns:
            The add channel action command write data result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_action_pin_number(self, channel_name):
        ...
        Returns:
            The add channel action pin number result.
        """
        new_channel = self._add_read_write_channel_with_param(
    ...
    def _add_channel_action_pin_edge(self, channel_name):
        ...
        Returns:
            The add channel action pin edge result.
        """
        new_channel = self._add_read_write_channel_with_param(
...
# UTILITY CHANNELS -------------------------------------------------------
    ...
    def _add_channel_enable_board(self, channel_name):
        new_channel = self._add_read_write_channel_with_param(
            channel_name, 'BOARd:ENABled')
        new_channel.add_preset(
            'True',
            'The adapter board is allowed to drive the ENABLE, WD_DISABLE, and SYNC pins.')
        new_channel.add_preset(
            'False', 'ENABLE, WD_DISABLE, and SYNC pins are configured as high-Z inputs.')
        return new_channel

# PWM CHANNELS -----------------------------------------------------------
    def _add_channel_pwm_gclk(self, channel_name):
        new_channel = self._add_read_write_channel_with_param(
            channel_name, 'PWM:GCLKDiv')
    ...
    def _add_channel_pwm_turbo(self, channel_name):
        new_channel = self._add_read_write_channel_with_param(
            channel_name, 'PWM:TURBo')
        new_channel.add_preset(
            True, 'Enable 96MHz base clock in ATSAMD21Gx chip')
        new_channel.add_preset(
            False, 'Enable 48MHz base clock in ATSAMD21Gx chip')
        return new_channel

    def _add_channel_pwm_enable(self, channel_name):
        new_channel = self._add_read_write_channel_with_param(
            channel_name, 'PWM:ENABle')
    ...
    def _add_channel_pwm_tcc_steps(self, channel_name):
        new_channel = self._add_read_write_channel_with_param(
            channel_name, 'PWM:TCC:STEPS')
        return new_channel

    def _add_channel_pwm_mode(self, channel_name):
        new_channel = self._add_read_write_channel_with_param(
            channel_name, 'PWM:TCC:MODE')
    ...
    def set_chip_settings(self, uses_pec='True', register_size='8',
        ...
        Sends the appropriate SCPI command to configure the instrument's chip
        settings.
        Sends the appropriate SCPI command to configure the instrument's chip
        settings.

        Updates the chip settings in the object's internal state.

        Args:
            address: Network hostname or IP address string.
            register_size: Register size to use.
            uses_pec: Uses pec to use.
            wd_a_cc: Wd a cc to use.
            wd_q_cc: Wd q cc to use.
        """
    ...
    def write_chip(self, command_code, write_data):
        ...
        Args:
            ...
            write_data: Data payload to write to the device.
    ...
    def get_bf_writeback_data(self, channels, channel_name, write_data):
        ...
        def size_to_bitmask(size):
            ...
            Returns:
                The size to bitmask result.
            """
            return 2**size - 1
        # bitmask the size of the register with relevant bits cleared
        bitmask = size_to_bitmask(word_size) - \
            (size_to_bitmask(size) << offset)
        # writeback data with only relevant bits changed
        write_data = (register_read_data & bitmask) + (write_data << offset)
        return write_data
    ...
    def set_pin_mode(self, pin_num, mode, output_state='LOW'):
        ...
        Updates the pin mode in the object's internal state.

        Args:
            mode: Operating mode.
            output_state: Output state to use.
            pin_num: GPIO pin number (zero-based).
        """
        if mode == 'OUTPUT':
            self.write_pin(pin_num, output_state)
        else:
            # this sets it as an input, just throw away the read data
            self.read_pin(pin_num)

    def enable_test_hook(self, enable=True):
        """Enable test hook.

        Activates test hook so that subsequent operations include it.
    ...
    def set_trigger(self, trigger, type, cc=None,
        ...
        Updates the trigger in the object's internal state.

        Args:
            cc: Condition code for trigger or action.
            pin_edge: Pin edge to use.
            pin_num: GPIO pin number (zero-based).
            trigger: Trigger source or mode.
    ...
    def set_trigger_cc(self, trigger, type, cc, write_data):
        ...
        Updates the trigger cc in the object's internal state.
    ...
    def set_trigger_pin_num(self, trigger, type, pin_num, pin_edge):
        ...
        Updates the trigger pin num in the object's internal state.
    ...
    def queue_action(self, type, delay_us=0, cc=None,
        ...
        if delay_us != 0:
            self.action_delay_channel_us.write(delay_us)
        if cc is not None:
            self.action_command_code_channel.write(cc)
        if write_data is not None:
            self.action_command_write_data_channel.write(write_data)
        if pin_num is not None:
            self.action_pin_number_channel.write(pin_num)
        if pin_edge is not None:
            self.action_pin_edge_channel.write(pin_edge)
        self.action_queue_channel.read()

    def run_all_actions(self):
        """Perform run all actions operation.
    ...
    def get_action_complete_timestamp_us(self, action_queue_index):
        ...
        Queries the instrument for its current action complete timestamp us
        and returns the parsed response.
        Queries the instrument for its current action complete timestamp us
        and returns the parsed response.

        Returns the stored action complete timestamp us from the object's internal state.

        Args:
            action_queue_index: Action queue index to use.

        Returns:
            The current action complete timestamp us.
        """
    ...
    def add_channels_pwm(self, logger_instance,
        ...
        new_channel = self._add_channel_pwm_output_dutycycle(
            channel_name + '_duty_cycle')
        ...
        logger_instance.add(new_channel)
        # self._add_channel(new_channel)
        new_channel = self._add_channel_pwm_pulse_width(
            channel_name + '_pulse_width')
        logger_instance.add(new_channel)
