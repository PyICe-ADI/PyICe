    ...
    def read_delegated_channel_list(self, channel_list):
        ...
        if False and self._automatic_monitor \
           ...
           and channel_list[0].get_attribute('34970_type') in ['volts_dc', 'current_dc', 'thermocouple']:
            ...
            # print f'WARNING: 3497x monitor channel:
            # {channel_list[0].get_name()} read timed out without value change.
            # This might be normal if quantization noise exceeds thermal
            # noise.'
...
class agilent_3497xa_20ch_40ch(a3497xa_instrument):
    ...
    def __init__(self, interface_visa, bay, automatic_monitor=True):
        ...
        a3497xa_instrument.__init__(
            self,
            f'34970a_mux bay: {bay} @ {interface_visa}',
            automatic_monitor=automatic_monitor)
        self.add_interface_visa(interface_visa)
    ...
    def warn_about_relays(self, bay):
        ...
        self.relay_cycle_counts = self._get_all_relay_cycles(
            channel_count=20 if plugin_type == "34901A" else 40)
        if any(count > 10e6 for count in self.relay_cycle_counts):
            print_banner(
                f"One or more relays in the 3497xa bay {bay} plugin have more than 10M cycles.",
    ...
    def add_channel_rtd(self, channel_name, channel_num, rtd_type=85,
        ...
        self._configure_channel_autozero(new_channel, disable_autozero)
        new_channel.set_description(
            self.get_name() + ': ' + self.add_channel_rtd.__doc__)
        return new_channel
    ...
    def _config_rtd(self, internal_address, rtd_type, ptype, nom_res):
        if rtd_type not in [85, 91]:
            raise Exception(
                'Invalid RTD type. Acceptable values are 85 and 91')
        if ptype == 4:
            self.get_interface().write(
                f"CONFigure:TEMPerature FRTD,{rtd_type}, (@{internal_address})")
            self.get_interface().write(
                f"SENS:TEMP:TRAN:FRTD:RES {nom_res}, (@{internal_address})")
        elif ptype == 2:
            self.get_interface().write(
                f"CONFigure:TEMPerature RTD,{rtd_type}, (@{internal_address})")
            self.get_interface().write(
                f"SENS:TEMP:TRAN:RTD:RES {nom_res}, (@{internal_address})")
        else:
            raise Exception(
        ...
        counts = [
            int(count) for count in (
                self.get_interface().ask(
                    f"DIAGnostic:RELay:CYCLes? ({channel_list})")).split(",")]
        return counts
...
class agilent_3497xa_20ch(agilent_3497xa_20ch_40ch):
    ...
    def add_channel_res(self, channel_name, channel_num, NPLC=1, res_range='AUTO',
        ...
        def _set_range(value):
            if value == "AUTO" or value is None:
                self.get_interface().write(
                    f'SENSe:RESistance:RANGe:AUTO ON, (@{channel_num + self.bay * 100})')
            else:
                '''TODO set presets for MIN and MAX'''
                '''Presumably AUTO OFF not needed if range being set to MIN, MAX or value.'''
                self.get_interface().write(
                    f'SENSe:RESistance:RANGe {value}, (@{channel_num + self.bay * 100})')

        def _get_range():
            return float(self.get_interface().ask(
                f'SENSe:RESistance:RANGe? (@{channel_num + self.bay * 100})'))

        def _add_channel_range():
            range_channel = channel(
                channel_name + '_range',
                write_function=_set_range)
            range_channel._read = _get_range
            range_channel.set_display_format_function(
                function=lambda float_data: eng_string(
                    float_data, fmt=':0.5g', si=True) + 'Ω')
        ...
        def _set_NPLC(value):
            self.get_interface().write(
                f'SENSe:RESistance:NPLC {value}, (@{channel_num + self.bay * 100})')

        def _get_NPLC():
            return float(self.get_interface().ask(
                f'SENSe:RESistance:NPLC? (@{channel_num + self.bay * 100})'))

        def _add_channel_NPLC():
            NPLC_channel = channel(
                channel_name + '_NPLC',
                write_function=_set_NPLC)
            NPLC_channel._read = _get_NPLC
            NPLC_channel.set_display_format_function(
                function=lambda float_data: eng_string(
                    float_data, fmt=':0.5g', si=True) + '')
        ...
        def _set_aperature(value):
            self.get_interface().write(
                f'SENSe:RESistance:APERture {value}, (@{channel_num + self.bay * 100})')

        def _get_aperature():
            return float(self.get_interface().ask(
                f'SENSe:RESistance:APERture? (@{channel_num + self.bay * 100})'))

        def _add_channel_aperature():
            aperature_channel = channel(
                channel_name + '_aperature',
                write_function=_set_aperature)
            aperature_channel._read = _get_aperature
            aperature_channel.set_display_format_function(
                function=lambda float_data: eng_string(
                    float_data, fmt=':0.5g', si=True) + 's')
        ...
        def _set_resolution(value):
            self.get_interface().write(
                f'SENSe:RESistance:RANGe:AUTO OFF, (@{channel_num + self.bay * 100})')
            self.get_interface().write(
                f'SENSe:RESistance:RESolution {value}, (@{channel_num + self.bay * 100})')

        def _get_resolution():
            return float(self.get_interface().ask(
                f'SENSe:RESistance:RESolution? (@{channel_num + self.bay * 100})'))

        def _add_channel_resolution():
            resolution_channel = channel(
                channel_name + '_resolution',
                write_function=_set_resolution)
            resolution_channel._read = _get_resolution
            resolution_channel.set_display_format_function(
                function=lambda float_data: eng_string(
                    float_data, fmt=':0.5g', si=True) + 'Ω')
        ...
        def _get_offset_compensated():
            return True if int(self.get_interface().ask(
                f'SENSe:RESistance:OCOMpensated? (@{channel_num + self.bay * 100})')) else False

        def _add_channel_offset_compensated():
            offset_compensated_channel = channel(
                channel_name + '_offscomp',
                write_function=_set_offset_compensated)
        ...
        main_channel.set_description(
            self.get_name() + ': ' + self.add_channel_fres.__doc__)
        main_channel.set_attribute('34970_type', 'Ohmmeter')
        main_channel.set_display_format_function(
            function=lambda float_data: eng_string(
                float_data, fmt=':0.5g', si=True) + 'Ω')
        self.get_interface().write("CONFigure:RESistance " +
                                   f"(@{channel_num + self.bay * 100})")
        if delay is not None:
    ...
    def add_channel_fres(self, channel_name, channel_num, NPLC=1, range='AUTO',
        ...
        ####################################
        CONFigure
          :FRESistance [{<range>|AUTO|MIN|MAX|DEF} [,<resolution>|MIN|MAX|DEF}],] (@<scan_list>)
        CONFigure? [(@<ch_list>)]
        [SENSe:] (implied)
        ...
        def _set_range(value):
            if value == "AUTO" or value is None:
                self.get_interface().write(
                    f'SENSe:FRESistance:RANGe:AUTO ON, (@{channel_num + self.bay * 100})')
            else:
                '''TODO set presets for MIN and MAX'''
                '''Presumably AUTO OFF not needed if range being set to MIN, MAX or value.'''
                self.get_interface().write(
                    f'SENSe:FRESistance:RANGe {value}, (@{channel_num + self.bay * 100})')

        def _get_range():
            return float(self.get_interface().ask(
                f'SENSe:FRESistance:RANGe? (@{channel_num + self.bay * 100})'))
        ...
        def _set_NPLC(value):
            self.get_interface().write(
                f'SENSe:FRESistance:NPLC {value}, (@{channel_num + self.bay * 100})')

        def _get_NPLC():
            return float(self.get_interface().ask(
                f'SENSe:FRESistance:NPLC? (@{channel_num + self.bay * 100})'))
        ...
        def _set_aperature(value):
            self.get_interface().write(
                f'SENSe:FRESistance:APERture {value}, (@{channel_num + self.bay * 100})')

        def _get_aperature():
            return float(self.get_interface().ask(
                f'SENSe:FRESistance:APERture? (@{channel_num + self.bay * 100})'))
        ...
        def _set_resolution(value):
            self.get_interface().write(
                f'SENSe:FRESistance:RANGe:AUTO OFF, (@{channel_num + self.bay * 100})')
            self.get_interface().write(
                f'SENSe:FRESistance:RESolution {value}, (@{channel_num + self.bay * 100})')

        def _get_resolution():
            return float(self.get_interface().ask(
                f'SENSe:FRESistance:RESolution? (@{channel_num + self.bay * 100})'))
        ...
        def _get_offset_compensated():
            return True if int(self.get_interface().ask(
                f'SENSe:FRESistance:OCOMpensated? (@{channel_num + self.bay * 100})')) else False
        ...
        main_channel.set_display_format_function(
            function=lambda float_data: eng_string(
                float_data, fmt=':0.5g', si=True) + 'Ω')
        self.get_interface().write("CONFigure:FRESistance " +
                                   f"(@{channel_num + self.bay * 100})")
        if delay is not None:
            self._config_channel_delay(main_channel, delay)
        # self._configure_channel_autozero(main_channel, disable_autozero)
...
class agilent_3497xa_actuator(a3497xa_instrument):
    ...
    def __init__(self, interface_visa, bay):
        ...
        a3497xa_instrument.__init__(
            self,
            f'34970a_actuator: {bay} @ {interface_visa} ',
