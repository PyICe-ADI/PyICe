...
class AD5667R(instrument):
    ...
    def __init__(self, interface_twi, addr7):
        ...
        instrument.__init__(
            self,
            f'Analog Devices AD5667R Digital to Ananlog Converter at 0x{
                addr7:X}')
        ...
        self._setup_LDAC_mode(
            dac_a_ldac_setting=LDAC_AUTOMATIC,
            dac_b_ldac_setting=LDAC_AUTOMATIC)
    ...
    def _set_power_state(self, power_state, dac):
        if power_state in [0, "0"]:
            ...
            raise Exception(
                f'AD5667R: Power setting must be one of: ["0", "1k", "100k", "z"], got: "{power_state}".')
    ...
    def _sync_channels(self, channel, value):
        ...
        if channel.get_attribute('ch_type') == 'voltage':
            ...
            raise Exception(
                "AD5667R Dual Dac driver has confused itself. Contact PyICe-developers@analog.com for more information.")
