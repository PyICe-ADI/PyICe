...
class rigol_DG800(scpi_instrument):
    ...
    def add_channel(self, channel_name, channel_number,
        ...
        if add_extended_channels:
            ...
            self.add_channel_low_voltage(
                channel_name + "_low_voltage", channel_number)
            self.add_channel_high_voltage(
                channel_name + "_high_voltage", channel_number)
            self.add_channel_pulse_width(
                channel_name + "_pulse_width", channel_number)
            self.add_channel_pulse_period(
                channel_name + "_period", channel_number)
            self.add_channel_rise_time(
                channel_name + "_risetime", channel_number)
            self.add_channel_fall_time(
                channel_name + "_falltime", channel_number)
            self.add_channel_cycle_count(
                channel_name + "_cycle_count", channel_number)
        else:
            print(
                'Manually add channels for high_voltage, low_voltage, pulse_width, period, and/or transition')
    ...
    def _config_pulse_func(self, channel_number, function, high_voltage=3.3,
        ...
        self.instrument.write(
            f"SOURce{channel_number}:BURSt:TRIGger:SOURce MAN")
    ...
    def _write_output_enable(self, channel_number, output_enable):
        self.instrument.write(
            (f"OUTPut{channel_number}:STATE {output_enable}").encode())

    def _write_low_voltage(self, channel_number, low_voltage):
        self.instrument.write(
            (f"SOURce{channel_number}:VOLTage:LEVel:IMMediate:LOW {low_voltage}").encode())

    def _write_high_voltage(self, channel_number, high_voltage):
        self.instrument.write(
            (f"SOURce{channel_number}:VOLTage:LEVel:IMMediate:HIGH {high_voltage}").encode())

    def _write_pulse_width(self, channel_number, pulse_width):
        self.instrument.write(
            (f"SOURce{channel_number}:PULSe:WIDTh {pulse_width}").encode())

    def _write_pulse_period(self, channel_number, period):
        self.instrument.write(
            (f"SOURce{channel_number}:PULSe:PERiod {period}").encode())

    def _write_pulse_transition_leading(self, channel_number, transition):
        self.instrument.write(
            (f"SOURce{channel_number}:PULSe:TRANsition:LEADing {transition}").encode())

    def _write_pulse_transition_trailing(self, channel_number, transition):
        self.instrument.write(
            (f"SOURce{channel_number}:PULSe:TRANsition:TRAiling {transition}").encode())

    def _write_cycle_count(self, channel_number, cycle_count):
        self.instrument.write(
            (f"SOURce{channel_number}:BURSt:NCYCles {cycle_count}").encode())
