...
├── tests/
│   └── example/
│       ├── run.py              Entry point. Run this to execute a test.
│       └── test.py             Example test class (customize, collect, plot).
└── infrastructure/
    ├── benches/
    │   └── {this_machine}.py   Bench file for this computer. Defines VISA
    │                           addresses and interfaces for local instruments.
    ├── hardware_drivers/
    │   └── example_HAMEG.py    Example driver. Shows how to wrap an instrument,
    │                           create channels, and set safety limits.
    └── plugin_dependencies/
        ├── project_settings.py Central config: project path, enabled plugins,
        │                       and plugin-specific parameters.
        ├── test_template.py    Project-wide base class for all tests.
        │                       Sits between Master_Test_Template and your tests.
'''
    if 'traceability' in plugins_to_add:
        readme += '''        ├── metadata_gathering_fns.py
        │                       Defines traceability fields (who, where, when)
        │                       logged alongside every test result.
'''
    if 'notifications' in plugins_to_add:
        readme += '''        ├── user_notifications/
        │   └── example_user.py Per-user notification targets (email, text).
'''
    if 'bench_config_management' in plugins_to_add:
        readme += '''        ├── bench_configuration_components.py
        │                       Custom component definitions (boards, fixtures).
