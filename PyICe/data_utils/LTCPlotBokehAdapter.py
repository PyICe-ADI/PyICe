...
class plot(LTCPlotBokehAdapter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        xselect = Select(
            title="X Scale:",
            value="log" if args_map['logx'] else "linear",
            options=[
                "linear",
                "log"])
        yselect = Select(
            title="Y Scale:",
            value="log" if args_map['logy'] else "linear",
            options=[
                "linear",
                "log"])
    ...
    def add_note(self, *args, **kwargs):
        ...
        # I don't know. Different than matplotlib...
        label.x_offset = -0.1 * self._fig.width
        # I don't know. Different than matplotlib...
        label.y_offset = -0.1 * self._fig.height
    ...
    def add_trace(self, *args, **kwargs):
        ...
        color = tuple(map(lambda zero_one: int(
            round(255 * zero_one)), args_map['color']))
        self._fig.line(
            x=x_data,
            y=y_data,
            alpha=1,
            color=color,
            legend_label=args_map['legend'])
        # TODO line style, etc?
...
class Page(LTCPlotBokehAdapter):
    ...
    def create_svg(self, *args, **kwargs):
        ...
        if args_map['file_basename'] is not None:
            filepath = './plots/' if args_map['filepath'] is None else os.path.join(
                args_map['filepath'], 'plots')
            try:
    ...
    def add_plot(self, *args, **kwargs):
        ...
        args_map = bind_to_base(
            ...
            **kwargs)
        plt = args_map['plot']
        # other options: 'top_right', 'bottom_left', etc.
        plt._fig.legend.location = "bottom_left"
...
def install(calling_module):
    ...
    def store_and_replace(class_name):
        ...
        setattr(
            calling_module.LTC_plot,
            class_name,
            getattr(
                this_module,
                class_name))
    print(
        f'INFO: Replacing LTCPlot calls with Bokeh adapter from {this_module}')
