...
class waveform(object):
    ...
    def __init__(self, data, trigger_sigma=10, trigger_level=None,
        ...
        # This is pretty hacky, but promises trememdous speed improvement when
        # passing data straight from sqlite and numpy to here, compared with
        # list(zip) operations to meet the legacy calling argument format, but
        # are then immediately undone.
        def _zip(data):
            global warned_1_already
            # Let the nagging begin
            if not warned_1_already:
                print_banner(
                    "WARNING: The waveform class instantiated with (x,y) pair data (legacy format) N x 2.",
                    "This is both expensive to zip together from independent database columns",
                    "and expensive to unzip on the other side of the function call, for no net benefit.",
                    "Consider sending data in instead as (x_data, y_data) 2 x N column tuple.",
                    length=160)
                warned_1_already = True
            self.data = data
            # TODO numpy array, not list!
            self.xdata, self.ydata = list(zip(*data))
        try:
            if len(data) == 2 and len(data[0]) > 2:
                # it's in columns
                # list? necessary? Trying not to do too much work here.
                self.data = zip(data)
        ...
        if not isinstance(self.xdata, type(self.ydata)) != numpy.ndarray:
            if not warned_2_already:
                print_banner(f"WARNING: The waveform class initialized with non-numpy arrays ({type(self.xdata)},{type(self.ydata)}).", "They will be converted here to enable more computationally efficient internal methods, at some expense.",
                             "Consider whether this data can travel from its source (SQLite?) to here without needing to become a Python list at any point in its journey.", "Contact pyice-developers@analog.com for help in converting column types and/or data.", "TODO box_filter, etc.", length=160)
                self.xdata = numpy.array(self.xdata)
                self.ydata = numpy.array(self.ydata)
                warned_2_already = True

        # Leadin and leadout.
        self.index_size = int(len(self.xdata) * LEADER_SIZE)
        # Rules require stationarity in and out...
        leadin = self.ydata[:self.index_size]
        # for at least 10% of the record size.
        leadout = self.ydata[-self.index_size:]
        self.warning = 0
        ...
        if stationarity_check:
            ...
            if self.stationarity_in < 1 - MAX_NONSTATIONARITY:
                print_banner(
                    f"Waveform Analyser ** WARNING **: Waveform is not stationary leading in, stationarity: {
                        self.stationarity_in:0.5e}")
                self.warning = -1
            if self.stationarity_out < 1 - MAX_NONSTATIONARITY:
                print_banner(
                    f"Waveform Analyser ** WARNING **: Waveform is not stationary leading out, stationarity: {
                        self.stationarity_out:0.5e}")
                self.warning = -1
        else:
    ...
    def _plot(self):
        ...
        # Show data that waveform analyzer sees (possibly hidden inside SCITL
        # loop)
        self.plt.line(x=self.xdata, y=self.ydata)
        data_txt = ''
        for param_name in ['_average_in', '_average_out', 'stdev_in', 'stdev_out',
                           '_trigger_sigma', '_trigger_level', 'stationarity_in', 'stationarity_out']:
    ...
    def trigger_10_90(self):
        ...
        for index, value in enumerate(self.ydata):
            if self.trigger_10_90_polarity == 1 and value > 0.1 * \
                    (self._average_out - self._average_in) + self._average_in:    # Use average_out b'cos that's what is servoed for a load step
                ...
                return
            # Use average_in b'cos that's what is servoed for a load release
            elif self.trigger_10_90_polarity == -1 and value < 0.9 * (self._average_in - self._average_out) + self._average_out:
                self.trigger_10_90_index = index
                self.trigger_10_90_value = value
                self.trigger_10_90_time = self.xdata[index]
                self.plt.add_layout(Span(location=self.xdata[index],
    ...
    def settling_time(self, low_limit, high_limit):
        ...
        for idx, value in enumerate(yreverse):
            if value >= high_limit or value <= low_limit:
                if idx == 0:
                    print_banner(
                        "Waveform Analyser: Warning, The waveform did not settle to the tolerance requested...", f"({low_limit}, {high_limit})")
                    return -1
                return xreverse[idx] - self.xdata[self._trigger_index]
        print_banner("Waveform Analyser: Warning, waveform was never outside the tolerance region requested...",
    ...
    def settling_time_from_max_deviation(self, limit, deviation):
        ...
        for idx, value in enumerate(yreverse):
            if value - \
                ...
                if idx == 0:
                    print_banner(
                        "Waveform Analyser: Warning, The waveform did not settle to the tolerance requested...",
                        f"{limit}")
                    return -1
        ...
        print_banner(
            "Waveform Analyser: Warning, The waveform was never outside the tolerance region requested...",
            f"{limit}")
        return -1
        ...
        for idx, value in enumerate(self.ydata):
            if value - \
                ...
                if idx == len(self.ydata) - 1:
                    print_banner(
                        "Waveform Analyser: Warning, The waveform was always outside the tolerance requested...",
                        f"{limit}")
                    return -1
                start_index = idx
                break
    ...
    def slew_rate(self):
        ...
        if self.trigger_10_90_polarity == 1:
            rampend_index = next(idx for (idx, data) in enumerate(
                self.ydata) if data > 0.9 * (self._average_out - self._average_in) + self._average_in)
        elif self.trigger_10_90_polarity == -1:
            rampend_index = next(idx for (idx, data) in enumerate(
                self.ydata) if data < 0.1 * (self._average_in - self._average_out) + self._average_out)
        else:
            raise ValueError(
    ...
    def rise_time(self, low_percentage=0.1, high_percentage=0.9):
        ...
        hline1 = Span(
            ...
            line_dash='dashed',
            line_width=1)
        vline1 = Span(location=self.xdata[self.index_10],
                      dimension='height',
                      line_color='red',
                      line_dash='dashed',
                      line_width=1)
        ...
        hline2 = Span(
            ...
            line_dash='dashed',
            line_width=1)
        vline2 = Span(location=self.xdata[self.index_90],
                      dimension='height',
                      line_color='red',
                      line_dash='dashed',
                      line_width=1)
    ...
    def find_first_rising_edge(self, vhigh, vlow=0, lvl=0.5, start_index=0):
        ...
        if vhigh is None:
            ...
            raise ValueError(
                f"\nWaveform Analyser: start_index is outside the range of the waveform data. Enter a value between 0 and {
                    len(
                        self.ydata) -
                    1}\n")
        ...
        if self.ydata[0] > vth:
            start_index = self.find_less_than_or_equal_to(
                vth=vth, start_index=start_index, stop_index=len(
                    self.ydata) - 1, increment=1)
            if start_index == -1:
                return -1
            index = self.find_grt_than_or_equal_to(
                vth=vth, start_index=start_index, stop_index=len(
                    self.ydata) - 1, increment=1)
        else:
            index = self.find_grt_than_or_equal_to(
    ...
    def find_first_falling_edge(self, vhigh, vlow=0, lvl=0.5, start_index=0):
        ...
        if self.ydata[0] < vth:
            ...
            if start_index == -1:
                return -1
            index = self.find_less_than_or_equal_to(
                vth=vth, start_index=start_index, stop_index=len(
                    self.ydata) - 1, increment=1)
        else:
            index = self.find_less_than_or_equal_to(
    ...
    def sw_rise_time(self, lo_lvl, hi_lvl, vhigh, vlow=0):
        ...
        if index_50 == -1:
            print_banner(
                "Waveform Analyser: The 50% level of a rising edge was not found in the data record")
            return (-1, -1, -1)
        ...
        if lo_lvl >= real_50_lvl:
            ...
            # To be consistent with when lo_lvl< 50%, decreased the index by
            # one
            index_lo = self.find_grt_than_or_equal_to(
                vth=vth_lo, start_index=index_50, stop_index=len(
                    self.ydata) - 1, increment=1) - 1
        else:
            index_lo = self.find_less_than_or_equal_to(
                vth=vth_lo, start_index=index_50, stop_index=0, increment=-1)
            ...
            print_banner(
                ...
                f"The {
                    round(
                        lo_lvl *
                        100,
                        1)}% level of the waveform was not found in the data record")
            return (-1, -1, -1)

        vth_hi = vlow + hi_lvl * amplitude
        if hi_lvl >= real_50_lvl:
            index_hi = self.find_grt_than_or_equal_to(
                vth=vth_hi, start_index=index_50, stop_index=len(
                    self.ydata) - 1, increment=1)
        else:
            ...
            index_hi = self.find_less_than_or_equal_to(
                vth=vth_hi, start_index=index_50, stop_index=0, increment=-1) + 1
        ...
        return (round((self.ydata[index_lo] - vlow) / amplitude, 4), round(
            (self.ydata[index_hi] - vlow) / amplitude, 4), self.xdata[index_hi] - self.xdata[index_lo])
    ...
    def sw_fall_time(self, lo_lvl, hi_lvl, vhigh, vlow=0):
        ...
        index_50 = self.find_first_falling_edge(
            vhigh=vhigh, vlow=vlow, lvl=0.5)
        if index_50 == -1:
            print_banner(
                "Waveform Analyser",
                "The 50% level of a falling  edge is not found in the data record")
            return (-1, -1, -1)
        real_50_lvl = round((self.ydata[index_50] - vlow) / amplitude, 4)
        ...
        if lo_lvl <= real_50_lvl:
            ...
            # When lo_lvl is less than 50_lvl, the index returned is when the signal drops below the lo_lvl.
            # To be consistent with when lo_lvl< 50%, increased the index by
            # one
            index_lo = self.find_grt_than_or_equal_to(
                vth=vth_lo, start_index=index_50, stop_index=0, increment=-1) + 1
        ...
        if hi_lvl <= real_50_lvl:
            ...
            index_hi = self.find_grt_than_or_equal_to(
                vth=vth_hi, start_index=index_50, stop_index=0, increment=-1)
        ...
        return (round((self.ydata[index_lo] - vlow) / amplitude, 4), round(
            (self.ydata[index_hi] - vlow) / amplitude, 4), self.xdata[index_lo] - self.xdata[index_hi])
    ...
    def sw_nol_rise(self, vth, vhigh, vlow=0):
        ...
        if index_50 == -1:
            print_banner(
                "Waveform Analyser",
                "The 50% level of a rising edge is not found in the data record")
            return (-1, -1)

        nol_low_side_start_index = self.find_less_than_or_equal_to(
            vth=vlow - vth, start_index=index_50, stop_index=0, increment=-1)
        if nol_low_side_start_index == -1:
            nol_low_side = 0
        else:
            nol_low_side_stop_index = self.find_grt_than_or_equal_to(
                vth=vlow - vth, start_index=nol_low_side_start_index, stop_index=0, increment=-1)
            if nol_low_side_stop_index == -1:
                print_banner(
                    "Waveform Analyser",
                    "The waveform was below vlow - vth from the beginning of the scope shot to the first rising edge")
                nol_low_side = -1
            else:
                nol_low_side = self.xdata[nol_low_side_start_index] - \
                    self.xdata[nol_low_side_stop_index]
        ...
        nol_high_side_start_index = self.find_grt_than_or_equal_to(
            vth=vhigh + vth,
            start_index=index_50,
            stop_index=len(
                self.ydata) - 1,
            increment=1)
        if nol_high_side_start_index == -1:
            ...
            nol_high_side_stop_index = self.find_less_than_or_equal_to(
                vth=vhigh + vth,
                start_index=nol_high_side_start_index,
                stop_index=len(
                    self.ydata) - 1,
                increment=1)
            if nol_high_side_stop_index == -1:
                print_banner(
                    "Waveform Analyser",
                    "The waveform was above vhigh +vth at the end of the scope shot")
                nol_high_side = -1
            else:
                nol_high_side = self.xdata[nol_high_side_stop_index] - \
                    self.xdata[nol_high_side_start_index]
    ...
    def sw_nol_fall(self, vth, vhigh, vlow=0):
        ...
        if nol_low_side_start_index == -1:
            ...
            nol_low_side_stop_index = self.find_grt_than_or_equal_to(
                vth=vlow - vth,
                start_index=nol_low_side_start_index,
                stop_index=len(
                    self.ydata) - 1,
                increment=1)
            if nol_low_side_stop_index == -1:
                ...
                nol_low_side = self.xdata[nol_low_side_stop_index] - \
                    self.xdata[nol_low_side_start_index]
                self.plt.add_layout(Span(location=vlow - vth,
                                         dimension='width',
                                         line_color='orange',
                                         line_dash='dotdash',
        ...
        nol_high_side_start_index = self.find_grt_than_or_equal_to(
            vth=vhigh + vth, start_index=index_50, stop_index=0, increment=-1)
        if nol_high_side_start_index == -1:
            nol_high_side = 0
        else:
            nol_high_side_stop_index = self.find_less_than_or_equal_to(
                vth=vhigh + vth, start_index=nol_high_side_start_index, stop_index=0, increment=-1)
            if nol_high_side_stop_index == -1:
                print_banner(
                    "Waveform Analyser",
                    "The waveform was above vhigh +vth at the end of the scope shot")
                ...
                nol_high_side = self.xdata[nol_high_side_start_index] - \
                    self.xdata[nol_high_side_stop_index]
                self.plt.add_layout(Span(location=vhigh + vth,
                                         dimension='width',
                                         line_color='green',
                                         line_dash='dotdash',
