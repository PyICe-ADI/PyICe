...
class touchstone_utils():
    ...
    def output_touchstone(self, output_file_name, output_file_dir,
        ...
        Args:
            ...
            input_network: Optional input network to output a touchstone of, if none given, the current touchstone utils network is used.
            start: Start frequency for the data of the network. If none given or lower than output_network.f[0] it is ignored.
            stop: Stop frequency for the data of the network. If none given or higher than network.f[-1] it is ignored.
            output_form: Output form based on skrf.network.Network.write_touchstone. Common forms are 'ri' for real & imaginary and 'ma' for magnitude & angle (degrees).
        """
        ...
        output_network.write_touchstone(filename=output_file_name,
                                        ...
                                        skrf_comment=False,
                                        form=output_form,
                                        # Real Imaginary, could also use mag
                                        # for Magnitude Phase(deg)
                                        )
    ...
    def network_Nport_to_Mport(self, port_nums):
        ...
        output_network = skrf.network.Network(
            frequency=freqs,
            s=capped_circuit.network.s,
            z0=z0,
            name="M Port Network")
        # TODO: See about using the Network constructor the way it is used in
        # test_make_Nport_Mport_reference_networks
    ...
    def network_Nport_to_1port(self, source_port_num, load_port_num):
        ...
        output_network = skrf.network.Network(
            frequency=freqs,
            s=circuit_1port.network.s,
            z0=z0,
            name="Test 1 Port Network")
        return output_network
...
###################################################################
    ...
    def dev_make_series_LR_model(self, outputFileName=None, R=1e3, L=1e-3):
        ...
        mplt.figure(2)
        # Plot Z(1,1) magnitude over frequency
    ...
    def dev_plot_rational_coefficients(self, y, x):
        ...
        mplt.figure()
        mplt.plot(self.network.f, np.abs(
            self.network.z[:, 0, 0]), label='touchstone')
        mplt.plot(
            np.transpose(freqs2),
            np.transpose(
                np.abs(Z11)),
    ...
    def plot_residue_impedance(self, r, p, k, fstop=10e6):
        ...
        mplt.figure()
        mplt.plot(self.network.f, np.angle(
            self.network.z[:, 0, 0], deg=True), label='touchstone')
        mplt.plot(
            np.transpose(freqs2),
            np.transpose(
                np.angle(
        ...
        db_error = 20 * np.log10(np.transpose(np.abs(z11[:index_f10_mega_hz]))) - 20 * np.log10(
            np.abs(self.network.z[:index_f10_mega_hz, 0, 0]))
        mplt.figure()
        mplt.plot(self.network.f[:index_f10_mega_hz],
                  db_error, label='magnitude_error')
        ...
        phase_error = np.transpose(np.angle(z11[:index_f10_mega_hz], deg=True)) - np.angle(
            self.network.z[:index_f10_mega_hz, 0, 0], deg=True)
        mplt.figure()
        mplt.plot(self.network.f[:index_f10_mega_hz],
                  phase_error, label='phase_error')
    ...
    def adice_rpc_to_impedance(a, b, k, z0, model_type=1):
        ...
        Args:
            ...
            k: DC term of M(s) equation.
            z0: Characteristic impedance of S or Y parameter from RPC file.
            model_type: 0 for Y parameter RPC coefficients, 1 for S parameter RPC coefficients.

        Returns:
            Tuple of (z_num_coeff, z_den_coeff, r, p, k) impedance coefficients.
        """
        r = np.multiply(a, b) * (-1)
    ...
    def adice_rpc_to_rational(a, b, k):
        ...
        Coefficients of the form T(s) = y(s)/x(s), where y[0] corresponds to s**n and n=degree.
        RPC coefficients are of the form M(s) = k + n_Σ {a[n] / (1 - s/b[n])}
        ...
        Returns:
            Tuple of (y, x) numerator and denominator coefficient lists.
        """
        r = np.multiply(a, b) * (-1)
        y, x = signal.invres(r=r, p=b, k=k)
        return y, x
    ...
    def dev_make_N_port_touchstone(self, output_file_name, num_ports):
        ...
        for i in range(num_ports):
            ...
            connections.append(
                [(make_port(f"port{i + 1}"), 0), (resistors[i], 0)])
    ...
    def LR_modelfit(self, f_max=30e6):  # This is Dave's algorithm
        ...
        # Find first index of f array that is greater than fmax
        i_max = np.where(self.network.frequency.f >= f_max)[0][0]
        r_tot_max = model_z_thru.real[i_max]
        model_params['R_skin_effect'] = (
            r_tot_max - model_params['Rdc']) / f_max**0.5  # 2-point at min/max
        model_params['L'] = (
            model_z_thru.imag /
            self.network.frequency.w)[
            i_max //
            2]  # middle freq fit
        print(model_params)
