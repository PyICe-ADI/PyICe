...
class plot(object):
    ...
    def add_horizontal_line(self, value, xrange=None, note=None,
        ...
        if self.xlims in [None, "auto"] and xrange is None:
            ...
            if note is not None:
                print(
                    "LTC_plot: Discarding horizontal line note, inability to locate on autoscale.")
            note = None
        # Use the range given (wierd but whatever)
        elif self.xlims in [None, "auto"] and xrange is not None:
            print(
                "LTC_plot: Requesting a horizontal line range without graph limits may affect autoscaling!")
            # Be advised this may autoscale the graph
            hxline = False
            xrange0 = xrange[0]
            xrange1 = xrange[1]
            if ylims not in [None, "auto"]:
                text_location = [
                    xrange[0] + 0.015 * (xrange[1] - xrange[0]), value + 0.015 * (ylims[1] - ylims[0])]
            else:
                if note is not None:
                    print(
                        "LTC_plot: Discarding horizontal line note, inability to locate on autoscale.")
                    note = None
        # Best guess on the location from xlims as before
        elif self.xlims not in [None, "auto"] and xrange is None:
            hxline = False
            xrange0 = self.xlims[0]
            xrange1 = self.xlims[1]
            if ylims not in [None, "auto"]:
                text_location = [self.xlims[0] +
                                 0.015 *
                                 (self.xlims[1] -
                                  self.xlims[0]), value +
                                 0.015 *
    ...
    def add_vertical_line(self, value, yrange=None, note=None,
        ...
        if axis_params['ylims'] in [None, "auto"] and yrange is None:
            ...
            if note is not None:
                print(
                    "LTC_plot: Discarding vertical line note, inability to locate on autoscale.")
                note = None
        # Use the range given (wierd but whatever)
        elif axis_params['ylims'] in [None, "auto"] and yrange is not None:
            print(
                "LTC_plot: Requesting a vertical line range without graph limits may affect autoscaling!")
            # Be advised this may autoscale the graph
            vxline = False
            yrange0 = yrange[0]
            yrange1 = yrange[1]
            if self.xlims not in [None, "auto"]:
                text_location = [value, yrange0 -
                                 3 * (yrange1 - yrange0) / 100]
            else:
                if note is not None:
                    print(
                        "LTC_plot: Discarding vertical line note, inability to locate on autoscale.")
                    note = None
        # Best guess on the location from xlims as before
        elif axis_params['ylims'] not in [None, "auto"] and yrange is None:
            vxline = False
    ...
    def create_csv(self, file_basename, filepath=None, dialect='excel'):
        ...
        file_basename = os.path.join(
            filepath, f"{file_basename}.csv".replace(
                " ", "_"))

        trace_dict = {}
        max_len = 0
        for ax in (self.y1_axis_params["trace_data"],
                   self.y2_axis_params["trace_data"]):
            for trace in ax:
                legend = trace['legend'] if len(
                    trace['legend']) else 'UNLABELED'
...
class Page():
    def __init__(self, rows_x_cols=None, page_size=None, plot_count=None):
        ...
        if rows_x_cols is not None and plot_count is None:
            ...
            self.rows_x_cols = (
                (plot_count - 1) // 3 + 1,
                plot_count if plot_count <= 3 else 3)
        else:
            raise Exception(
                'Specify exactly one of rows_x_cols or plot_count arguments. rows_x_cols should be a two-element list or tuple.')
        ...
        # Prevents characters from being converted to paths.
        matplotlib.rcParams['svg.fonttype'] = "none"
        matplotlib.rcParams['text.usetex'] = False     # Not Sure
        # Prevents subscripts from changing fonts.
    ...
    def add_plot(self, plot,
                 ...
                 x_gap=1,
                 # in inches. LTC marcom uses 0.8 but places only individual
                 # plots anyway so this is not used for datasheet placement.
                 y_gap=1,
                 # in inches. LTC marcom uses 0.8 but places only individual
                 # plots anyway so this is not used for datasheet placement.
        ...
        plot_sizex = 11.0 / 6.0 if plot_sizex is None else float(plot_sizex)
        if plot_sizey is None:
            plot_sizey = float(
                8.8 / 6.0) if plot.plot_type == "scope_plot" else 11.0 / 6.0
        ...
        if self.page_size is not None:
            ...
            x_total = left_border + right_border + \
                plot_sizex * columns + x_gap * (columns - 1)
            y_total = bottom_border + top_border + \
                plot_sizey * rows + y_gap * (rows - 1)
        ...
        # average Axes width.
        hspace = y_gap / plot_sizey
        # The height of the padding between subplots, as a fraction of the
        # average Axes height. For example, wspace = 1 makes gaps the size of
        # the x dimension of a plot
        wspace = x_gap / plot_sizex
        self.Figure.set_size_inches(x_total, y_total)
        graph.figure.subplots_adjust(
            left=left,
            right=right,
            bottom=bottom,
            top=top,
        ...
        if plot.plot_type == "scope_plot":
            ...
            note_props = dict(
                ...
                edgecolor="white",
                alpha=1)
            graph.text(0.4, -0.02, plot.xaxis_label, fontsize=7, transform=graph.transAxes,
                       horizontalalignment='left', verticalalignment='top', bbox=note_props)
        ...
        if plot.y2_axis_params["axis_is_used"]:
            ...
            if plot.plot_type == "scope_plot":
                ...
                twin.set_ylabel(
                    plot.y2_axis_params["yaxis_label"],
                    fontsize=7,
                    rotation=-90)
            # TODO, this should be a function of the number of digits in the
            # axes numbers
            twin.yaxis.labelpad = 17
        ...
        if plot.y1_axis_params["ylims"] in [None, "auto"]:
            ...
            graph.axis(
                ymin=plot.y1_axis_params["ylims"][0],
        ...
        if plot.logx:
            ...
            if plot.xminor != 0:
                ...
                graph.xaxis.set_minor_locator(matplotlib.ticker.LogLocator(
                    subs=(2, 3, 4, 5, 6, 7, 8, 9), numticks=None))
            else:
                graph.xaxis.set_minor_locator(
                    matplotlib.ticker.LogLocator(
                        subs=[], numticks=None))
        else:
            ...
            if plot.plot_type == "regular":
                ...
                if plot.xminor not in [None, 0]:
                    graph.xaxis.set_minor_locator(
                        matplotlib.ticker.AutoMinorLocator(plot.xminor))
        if plot.y1_axis_params["logy"]:
                ...
                graph.yaxis.set_minor_locator(
                    matplotlib.ticker.LogLocator(
                        ...
                        subs=(
                            ...
                            0.9),
                        numticks=None))
                graph.yaxis.set_minor_formatter(
            ...
            graph.yaxis.set_major_locator(
                matplotlib.ticker.LinearLocator(
                    plot.y1_axis_params["ydivs"] + 1))
            if plot.y1_axis_params["yminor"] not in [None, 0]:
                graph.yaxis.set_minor_locator(
                    matplotlib.ticker.AutoMinorLocator(
                        plot.y1_axis_params["yminor"]))
        if plot.plot_type == "scope_plot":
        ...
        if plot.plot_type == "scope_plot":
            ...
            for major in range(8):  # 0..7
                for minor in range(1, 5):  # 1..4
                    line = [(0.5 - TIC_LENGTH, major / 8. + minor / 40.),
                            (0.5 + TIC_LENGTH, major / 8. + minor / 40.)]
            ...
            for major in range(10):  # 0..9
                for minor in range(1, 5):  # 1..4
                    line = [(major / 10. + minor / 50., 0.5 - TIC_LENGTH),
                            (major / 10. + minor / 50., 0.5 + TIC_LENGTH)]
                    (line_xs, line_ys) = list(zip(*line))
                    graph.add_line(
                        Line2D(
                            line_xs,
            ...
            if y_axis_params["axis_is_used"]:
                for trace in y_axis_params["trace_data"]:
                    if trace["vxline"]:
                        ...
                        if isinstance(trace['data'], np.ndarray) and not isinstance(
                                trace['data'], np.recarray):
                            # Numpy views may be slightly faster.  Also, views
                            # could be used to also handle recarray coming in,
                            # instead of list,zip
                            unzip = trace['data'].T
                            x = unzip[0]
                            y = unzip[1]
                ...
                if y_axis_params["place_legend"]:
                    ...
                    if y_axis_params == plot.y2_axis_params:
                        if plot.y2_axis_params["use_axes_scale"]:
                            ...
                            coordinate_system = graph.transAxes
                    legend = y_axis_params["axis"].legend(
                        frameon=True,
                        fontsize=y_axis_params["legend_fontsize"],
                        loc=y_axis_params["legend_justification"],
                        bbox_to_anchor=y_axis_params["legend_loc"],
                        framealpha=1,
                        borderpad=0.25,
                        bbox_transform=coordinate_system)
                ...
                if plot.plot_type == "scope_plot":
                    ...
                    for ref_marker in plot.ref_markers:
                        if ref_marker["use_axes_scale"]:
                            ...
                            x = plot.xlims[0] - marker_x_offset * \
                                (plot.xlims[1] - plot.xlims[0])
                            ...
                            # marker = u"◄"
                            # Not sure why offset is needed, text arrow not
                            # centered with its bbox?
                            y = ref_marker["ylocation"] - 0.0038
                        note_props = dict(
                            boxstyle='square, pad = 0.125',
                            facecolor='white',
                            edgecolor="white",
        ...
        for note_dict in plot.notes:
            ...
            if note_dict["axis"] == 1:
                ...
                graph.text(
                    ...
                    y,
                    note,
                    fontsize=note_dict["fontsize"],
                    transform=coordinate_system,
                    horizontalalignment=note_dict["horizontalalignment"],
                    verticalalignment=note_dict["verticalalignment"],
                    bbox=note_props)
    ...
    def create_svg(self, file_basename=None, filepath=None):
        ...
        output = figdata.getvalue().replace("Linear Helv Cond",
                                            "LinearHelvCond").replace("font-size:9.5px;font-style:normal",
        ...
        if file_basename is not None:
            ...
            try:
                ...
                pass
            file_basename = os.path.join(
                filepath, "{}.svg".format(file_basename).replace(
                    " ", "_"))
    ...
    def create_pdf(self, file_basename, filepath=None):
        ...
        try:
            ...
            pass
        file_basename = os.path.join(
            filepath, "{}.pdf".format(file_basename).replace(
                " ", "_"))
...
class Multipage_pdf():
    ...
    def create_pdf(self, file_basename, filepath=None):
        ...
        filename = os.path.join(
            filepath,
            "{}.pdf".format(file_basename).replace(
                " ",
                "_"))
        try:
            os.makedirs(filepath)
    ...
    def kit_datasheet(self, file_basename="datasheet_kit"):
        ...
        for page in self.page_list:
            for plot in range(len(page.plot_list)):
                plot_name = "{}".format(
                    page.plot_list[plot].plot_name).replace(
                    " ", "_")
                DummyPage = Page(rows_x_cols=(1, 1))
...
def list_markers():
    ...
    print(
        "Valid markers are: " +
        "[':' '.' ',' 'o' 'v' '^' '<' '>' '1' '2' '3' '4' '8' 's' 'p' '*' 'h' 'H' '+' 'x' 'D' 'd' '|' '_' TICKLEFT TICKRIGHT TICKUP TICKDOWN CARETLEFT CARETRIGHT CARETUP CARETDOWN]")
    print()
...
# Bob also had the text color and black trace color as 231f20 which is
# kind of grey.
REAY_COLORS = [
    "900027",
    "3a52a4",
...
# The CMYK values are included here just for reference. They come from the
# document "LT Graph Color Palette and Styles.pdf"
# CMYK_to_RGB([0.00,   1.00,   0.65,   0.47])
LT_RED_1 = webRGB_to_fracRGB(MARCOM_COLORS[0])
# CMYK_to_RGB([0.88,   0.77,   0.00,   0.00])
LT_BLUE_1 = webRGB_to_fracRGB(MARCOM_COLORS[1])
# CMYK_to_RGB([0.80,   0.25,   0.90,   0.10])
LT_GREEN_1 = webRGB_to_fracRGB(MARCOM_COLORS[2])
# CMYK_to_RGB([0.24,   0.58,   0.92,   0.09])
LT_COPPER_1 = webRGB_to_fracRGB(MARCOM_COLORS[3])
# CMYK_to_RGB([0.00,   0.00,   0.00,   1.00])
LT_BLACK = webRGB_to_fracRGB(MARCOM_COLORS[4])
# CMYK_to_RGB([0.75,   0.05,   1.00,   0.00])
LT_GREEN_2 = webRGB_to_fracRGB(REAY_COLORS[9])
# CMYK_to_RGB([0.20,   0.60,   0.80,   0.00])
LT_COPPER_2 = webRGB_to_fracRGB(REAY_COLORS[8])
# CMYK_to_RGB([0.70,   0.45,   0.01,   0.01])??
LT_BLUE_2 = webRGB_to_fracRGB(REAY_COLORS[7])
# CMYK_to_RGB([0.22,   0.25,   0.92,   0.00])
LT_YELLOW_2 = webRGB_to_fracRGB(REAY_COLORS[6])
# CMYK_to_RGB([0.10,   1.00,   0.90,   0.00])
LT_RED_2 = webRGB_to_fracRGB(REAY_COLORS[5])
...
# This comes out as #333333 whereas Bob Reay's value is #323232
LT_GRID = CMYK_to_fracRGB([0, 0, 0, 0.8])
LT_SCOPE_GRID = CMYK_to_fracRGB([0, 0, 0, 0.2])       #
# This makes the most sense - Black
LT_TEXT = webRGB_to_fracRGB("000000")
